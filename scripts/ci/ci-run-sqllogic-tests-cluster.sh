...
if [ $# -gt 0 ]; then
	if [ "$#" -eq 1 ] && [ "$1" = "concurrent_tpch" ]; then
		if ! [[ "$TEST_PARALLEL" =~ ^[1-9][0-9]*$ ]]; then
			echo "TEST_PARALLEL must be a positive integer for concurrent_tpch, got: ${TEST_PARALLEL}"
			exit 1
		fi

		source_file="tests/sqllogictests/suites/concurrent_tpch/queries.test"
		generated_dir="tests/sqllogictests/suites/concurrent_tpch/generated"
		rm -rf "${generated_dir}"
		...
		EXTRA_ARGS=(--http-ports "${CONCURRENT_TPCH_PORTS:-8000,8002,8003}")
