...
./target/release/bendctl --databend_dir ./.databend --group local query ./tests/suites/0_stateful/01_ontime/ddl/create_table.sql
if [ $? -eq 0 ]; then
...
for i in $(find ./tests/suites/0_stateful/01_ontime/sql -name "*.sql" -type f); do
