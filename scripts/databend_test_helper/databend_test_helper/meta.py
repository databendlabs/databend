...
from .utils import BinaryFinder, ConfigManager, ProcessManager, CommandBuilder, LogConfigHelper
...
class DatabendMeta:
    ...
    def __init__(self, binary_path: Optional[str] = None, config_path: Optional[str] = None, 
                 profile: Optional[str] = None, args: Optional[MetaArgs] = None):
    ...
    def _print_start_info(self) -> None:
        ...
        if self.args:
            ...
            if overrides:
                ...
                import json
                ProgressReporter.print_message(f"     {json.dumps(overrides, indent=4)}")
        ...
        ProgressReporter.print_message(f"   Raft: node_id={node_id}, port={raft_port}, dir={raft_dir}")
    ...
    def start(self, wait_for_ready: bool = True, dry_run: bool = False) -> None:
        ...
        # Build command
        cmd = CommandBuilder.build_command(self.binary_path, self.config_path, self.args)
    ...
    def get_raft_config(self) -> dict:
        ...
        if raft_config is None:
            raise ValueError(f"Config file {self.config_path} missing [raft_config] section")
        return raft_config
