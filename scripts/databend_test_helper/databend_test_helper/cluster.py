...
class DatabendCluster:
    ...
    def __init__(self, meta_configs: List[str], query_configs: List[str], 
                 meta_binary: Optional[str] = None, query_binary: Optional[str] = None,
    ...
    def start(self, meta_delay: float = 2.0, cluster_delay: float = 5.0, dry_run: bool = False) -> None:
        """
        ...
        if dry_run:
            ProgressReporter.print_message(f"🔄 Would wait {cluster_delay}s between meta and query clusters")
        else:
    ...
    def print_status(self) -> None:
        ...
        ProgressReporter.print_message(f"Meta nodes: {meta_status['running']}/{meta_status['total']} running")
        ProgressReporter.print_message(f"Query nodes: {query_status['running']}/{query_status['total']} running")
