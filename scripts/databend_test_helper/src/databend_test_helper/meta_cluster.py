...
class MetaCluster:
    ...
    def __init__(
        ...
        for config_path in config_paths:
            node = DatabendMeta(
                binary_path=binary_path, config_path=config_path, profile=profile
            )
    ...
    def status(self) -> Dict[int, Dict[str, Any]]:
        ...
        for node_id, node in self.nodes.items():
            ...
            status_dict[node_id] = {
                ...
                "log_dir": file_log_config.get("dir")
                if file_log_config.get("on")
                else None,
                "log_level": file_log_config.get("level")
                if file_log_config.get("on")
                else None,
            }
