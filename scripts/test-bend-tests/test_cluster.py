...
def main():
    ...
    if not meta_config_dir.exists():
        ProgressReporter.print_message(f"❌ Meta config directory not found: {meta_config_dir}")
        sys.exit(1)
        
    if not query_config_dir.exists():
        ProgressReporter.print_message(f"❌ Query config directory not found: {query_config_dir}")
        sys.exit(1)
    ...
    if not meta_configs:
        ProgressReporter.print_message(f"❌ No meta config files found in: {meta_config_dir}")
        sys.exit(1)
        
    if not query_configs:
        ProgressReporter.print_message(f"❌ No query config files found in: {query_config_dir}")
        sys.exit(1)
    ...
    def signal_handler(signum, frame):
        ProgressReporter.print_message(f"\n🔄 Received signal {signum}, shutting down...")
        query_cluster.stop()
        meta_cluster.stop()
        sys.exit(0)
    ...
    try:
        ...
        meta_running = sum(1 for node_info in meta_status.values() if node_info.get("running", False))
        ...
        ProgressReporter.print_message(f"Meta nodes: {meta_running}/{meta_total} running")
        ProgressReporter.print_message(f"Query nodes: {query_running}/{query_total} running")
        ...
        if connections:
            ...
            for conn in connections:
                ProgressReporter.print_message(f"   Node {conn['node']}: HTTP={conn['http_url']}, MySQL={conn['mysql_url']}")
        ...
        while True:
            ...
            if restart_counter % 10 == 0:
                ...
                if not (meta_running and query_running):
                    ProgressReporter.print_message("❌ Some processes have stopped unexpectedly!")
                    ProgressReporter.print_message(f"Meta cluster running: {meta_running}")
                    ProgressReporter.print_message(f"Query cluster running: {query_running}")
                    break
