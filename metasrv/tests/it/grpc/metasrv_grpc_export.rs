...
async fn test_export() -> anyhow::Result<()> {
    ...
    async {
        ...
        let want = vec![
            ...
            r#"["log",1,"sledks::Logs",2,{"log_id":{"term":1,"index":2},"payload":{"Normal":{"txid":null,"cmd":{"AddNode":{"node_id":0,"node":{"name":"","address":"localhost:29000"}}}}}}]"#, //
            ...
            r#"["sm",2,"sledks::Nodes",0,{"name":"","address":"localhost:29000"}]"#, //
        ...
        let lines = lines
            .iter()
            .map(|x| {
                Regex::new(r"localhost:29\d\d\d")
                    .unwrap()
                    .replace(x, "localhost:29000")
                    .to_string()
            })
            .collect::<Vec<_>>();
