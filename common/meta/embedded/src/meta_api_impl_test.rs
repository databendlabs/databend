...
async fn test_meta_embedded_database_list() -> anyhow::Result<()> {
    ...
    {
        let res = create_database(&mt, "db1").await?;
        assert_eq!(1, res.database_id);

        let res = create_database(&mt, "db2").await?;
...
async fn test_meta_embedded_table_list() -> anyhow::Result<()> {
    ...
    tracing::info!("--- prepare db");
    {
        let res = create_database(&mt, db_name).await?;
        assert_eq!(1, res.database_id, "first database id is 1");
    }
...
async fn create_database(mt: &MetaEmbedded, db_name: &str) -> anyhow::Result<CreateDatabaseReply> {
