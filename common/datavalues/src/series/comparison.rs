...
macro_rules! impl_compare {
    ($self:expr, $rhs:expr, $method:ident) => {{
        ...
        match $self.data_type() {
            ...
            DataType::String(_) => $self.string().unwrap().$method($rhs.string().unwrap()),
            DataType::UInt8(_) => $self.u8().unwrap().$method($rhs.u8().unwrap()),
            DataType::UInt16(_) => $self.u16().unwrap().$method($rhs.u16().unwrap()),
            DataType::UInt32(_) => $self.u32().unwrap().$method($rhs.u32().unwrap()),
            DataType::UInt64(_) => $self.u64().unwrap().$method($rhs.u64().unwrap()),
            DataType::Int8(_) => $self.i8().unwrap().$method($rhs.i8().unwrap()),
            DataType::Int16(_) => $self.i16().unwrap().$method($rhs.i16().unwrap()),
            DataType::Int32(_) => $self.i32().unwrap().$method($rhs.i32().unwrap()),
            DataType::Int64(_) => $self.i64().unwrap().$method($rhs.i64().unwrap()),
            DataType::Float32(_) => $self.f32().unwrap().$method($rhs.f32().unwrap()),
            DataType::Float64(_) => $self.f64().unwrap().$method($rhs.f64().unwrap()),
            DataType::Date16(_) => $self.u16().unwrap().$method($rhs.u16().unwrap()),
            DataType::Date32(_) => $self.i32().unwrap().$method($rhs.i32().unwrap()),
            _ => unimplemented!(),
        }
    }};
}
