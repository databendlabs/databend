...
impl SeriesHelper {
    ...
    pub fn merge_series(lhs: &Series, rhs: &Series, indices: &[bool]) -> Result<Series> {
        let (lhs, rhs) = (lhs.get_array_ref(), rhs.get_array_ref());
        let result = DataArrayMerge::merge_array(&lhs, &rhs, indices)?;
    ...
    pub fn merge_indices(
        ...
        let lhs: Vec<ArrayRef> = lhs.iter().map(|s| s.get_array_ref()).collect();
        let rhs: Vec<ArrayRef> = rhs.iter().map(|s| s.get_array_ref()).collect();
