...
impl<T: DFPrimitiveType> DFPrimitiveArray<T> {
    ...
    pub unsafe fn try_get(&self, index: usize) -> Result<DataValue> {
        ...
        let d = match T::data_type() {
            DataType::UInt8 => v.map(|v| DataValue::UInt8(v.to_u8())),
            DataType::UInt16 => v.map(|v| DataValue::UInt16(v.to_u16())),
            DataType::UInt32 => v.map(|v| DataValue::UInt32(v.to_u32())),
            DataType::UInt64 => v.map(|v| DataValue::UInt64(v.to_u64())),
            DataType::Int8 => v.map(|v| DataValue::Int8(v.to_i8())),
            DataType::Int16 => v.map(|v| DataValue::Int16(v.to_i16())),
            DataType::Int32 => v.map(|v| DataValue::Int32(v.to_i32())),
            DataType::Int64 => v.map(|v| DataValue::Int64(v.to_i64())),
            DataType::Float32 => v.map(|v| DataValue::Float32(v.to_f32())),
            DataType::Float64 => v.map(|v| DataValue::Float64(v.to_f64())),
            _ => unreachable!(),
        };
