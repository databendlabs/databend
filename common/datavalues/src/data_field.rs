...
use crate::types2::data_type::from_arrow_field;
use crate::types2::data_type::DataTypePtr;

#[derive(serde::Serialize, serde::Deserialize, Clone, MallocSizeOf)]
...
impl DataField {
    ...
    pub fn contains(&self, other: &DataField) -> bool {
        if self.name != other.name || self.data_type().type_id() != other.data_type().type_id() {
            return false;
        }
