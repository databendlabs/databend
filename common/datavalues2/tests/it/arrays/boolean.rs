...
fn test_empty_boolean_array() {
    let mut builder = BooleanColumnBuilder::with_capacity(16);
    let data_array: BooleanColumn = builder.finish();
...
fn test_new_from_slice() {
    let data_array: BooleanColumn = NewColumn::new_from_slice(&[true, false]);
    let mut iter = data_array.into_iter();
    assert_eq!(Some(Some(true)), iter.next());
    assert_eq!(Some(Some(false)), iter.next());
    assert_eq!(None, iter.next());
...
fn test_new_from_opt_slice() {
    ...
    let data_array: BooleanColumn = NewColumn::new_from_opt_slice(&v);
    let mut iter = data_array.into_iter();
    assert_eq!(Some(Some(true)), iter.next());
    assert_eq!(Some(Some(false)), iter.next());
    assert_eq!(Some(None), iter.next());
...
fn test_new_from_opt_iter() {
    ...
    let data_array: BooleanColumn = NewColumn::new_from_opt_iter(iter);
    let mut iter = data_array.into_iter();
    assert_eq!(Some(Some(true)), iter.next());
    assert_eq!(Some(Some(false)), iter.next());
    assert_eq!(Some(None), iter.next());
