...
#[test]
fn test_empty_array() {
    let mut builder = MutableStringColumn::with_capacity(16);
...
fn test_new_from_opt_slice() {
    let data_array = StringColumn::new_from_opt_slice(&[Some("你好"), None]);
    let mut iter = data_array.into_iter();
...
fn test_new_from_opt_iter() {
    ...
    let data_array = StringColumn::new_from_opt_iter(iter);
    let mut iter = data_array.into_iter();
