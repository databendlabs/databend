...
#[test]
fn test_empty_array() {
    let mut builder = MutablePrimitiveColumn::<i64>::with_capacity(16);
    let data_array: Int64Column = builder.finish();
...
fn test_new_from_slice() {
    let data_array: Int64Column = NewColumn::new_from_slice(&[1, 2]);
    let mut iter = data_array.into_iter();

    // verify NewColumn::new_from_slice
    assert_eq!(Some(Some(&1)), iter.next());
    assert_eq!(Some(Some(&2)), iter.next());
    assert_eq!(None, iter.next());
}
...
fn test_new_from_opt_slice() {
    ...
    let data_array: Int64Column = NewColumn::new_from_opt_slice(&v);
    let mut iter = data_array.into_iter();

    // verify NewColumn::new_from_opt_slice
    assert_eq!(Some(Some(&1)), iter.next());
    assert_eq!(Some(Some(&2)), iter.next());
    assert_eq!(Some(None), iter.next());
    assert_eq!(None, iter.next());
...
fn test_new_from_opt_iter() {
    ...
    let data_array: Int64Column = NewColumn::new_from_opt_iter(iter);
    let mut iter = data_array.into_iter();
