...
use bytes::BytesMut;
use common_arrow::arrow::compute::aggregate::Sum;
...
use super::aggregate_function2::AggregateFunction2;
...
impl<T, SumT> AggregateFunction2 for AggregateSumFunction<T, SumT>
where
    T: PrimitiveType + AsPrimitive<SumT>,
    SumT: PrimitiveType + ToDataType + std::ops::Add<Output = SumT>,
    Option<SumT>: Into<DataValue>,
{
    ...
    fn accumulate(&self, place: StateAddr, arrays: &[ColumnRef], _input_rows: usize) -> Result<()> {
    ...
    fn merge_result(&self, place: StateAddr, array: &mut dyn MutableColumn) -> Result<()> {
        ...
        if let Some(val) = state.value {
            let mut array = array
                .as_mut_any()
                .downcast_mut::<MutablePrimitiveColumn<SumT>>()
                .ok_or_else(|| {
...
where
    T: PrimitiveType + AsPrimitive<SumT>,
    SumT: PrimitiveType + std::ops::Add<Output = SumT>,
    Option<SumT>: Into<DataValue>,
{
    pub fn try_create(
        display_name: &str,
...
pub fn try_create_aggregate_sum_function2(
    display_name: &str,
    _params: Vec<DataValue>,
    arguments: Vec<DataField>,
) -> Result<AggregateFunction2Ref> {
...
pub fn aggregate_sum_function_desc() -> AggregateFunctionDescription {
    AggregateFunctionDescription::creator(Box::new(try_create_aggregate_sum_function2))
}

fn sum_primitive<T, SumT>(column: &ColumnRef) -> Result<SumT>
where
    T: PrimitiveType + AsPrimitive<SumT>,
    SumT: PrimitiveType + std::ops::Add<Output = SumT> + std::ops::AddAssign,
{
    let nullable_col: Result<&NullableColumn> = Series::check_get(column);
    if let Ok(col) = nullable_col {
        let inner: &PrimitiveColumn<T> = Series::check_get(col.inner())?;
