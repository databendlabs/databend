...
pub struct Monotonicity2 {
    ...
    // Is the monotonicity2 from constant value
...
impl Monotonicity2 {
    /// Create a default monotonicity2 for non-monotonic and non-constant expression/function.
    ...
    /// Create a monotonicity2, with input parameter field. The left and right field are None.
    ...
    /// Create a monotonicity2 for constant expression, with 'is_constant' is true.
    ...
    /// Clone from a monotonicity2, but with left/right field None.
    pub fn clone_without_range(mono: &Monotonicity2) -> Self {
        Monotonicity2 {
    ...
    pub fn compare_with_zero(&self) -> Result<i8> {
        if !self.is_monotonic {
            return Err(ErrorCode::UnknownException(
                "Request monotonicity2 function",
            ));
        }
...
impl From<Monotonicity> for Monotonicity2 {
    fn from(m: Monotonicity) -> Self {
        let left = m.left.map(|f| convert2_new_column(&f));
        let right = m.right.map(|f| convert2_new_column(&f));
...
impl From<Monotonicity2> for Monotonicity {
    fn from(m: Monotonicity2) -> Self {
        let left = m.left.map(|f| convert2_old_column_with_field(&f));
        let right = m.right.map(|f| convert2_old_column_with_field(&f));
