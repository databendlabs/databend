...
In version v1.2.0, Databend has added features such as **BITMAP data type** , **direct query of CSV/TSV/NDJSON files using column position** , and **AI Functions** . It has also **N** to significantly improve **Join** performance. This release brings Databend closer to realizing the vision of LakeHouse, allowing it to directly read and analyze CSV/TSV/NDJSON/Parquet format files stored on object storage. You can also perform ETL operations on these files within Databend for higher performance OLAP analysis.

In addition, we have designed and implemented enterprise features such as **Computed Columns**, **VACUUM TABLE**, and **Serveless Background Service**. If you are interested in these features, you can contact [Databend Support](https://www.databend.com/contact-us) for upgrade information or visit [Databend Cloud](http://app.databend.cn/) for an instant experience.
