...
async fn test_interpreter_interceptor_for_insert() -> Result<()> {
    ...
    // Check.
    {
        ...
        let expected = vec![
            ...
            "| log_type | handler_type | cpu_usage | scan_rows | scan_bytes | scan_partitions | written_rows | written_bytes | result_rows | result_bytes | query_kind  | query_text | sql_user | sql_user_quota                 |",
            "+----------+--------------+-----------+-----------+------------+-----------------+--------------+---------------+-------------+--------------+-------------+------------+----------+--------------------------------+",
            "| 1        | Dummy        | 8         | 0         | 0          | 0               | 0            | 0             | 0           | 0            | CreateTable | create table t as select number from numbers_mt(1) | root     | UserQuota<cpu:0,mem:0,store:0> |",
            "| 2        | Dummy        | 8         | 1         | 8          | 0               | 1            | 8             | 0           | 0            | CreateTable | create table t as select number from numbers_mt(1) | root     | UserQuota<cpu:0,mem:0,store:0> |",
            "+----------+--------------+-----------+-----------+------------+-----------------+--------------+---------------+-------------+--------------+-------------+------------+----------+--------------------------------+",
        ];
        common_datablocks::assert_blocks_sorted_eq(expected, result.as_slice());
    }
