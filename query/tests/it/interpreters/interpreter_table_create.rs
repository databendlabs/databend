...
async fn test_create_table_interpreter() -> Result<()> {
    ...
    ";
        ...
        assert_eq!(
            format!("{:?}", field_a),
            r#"DataField { name: "a", data_type: Int64, nullable: false, default_expr: "{\"Literal\":{\"value\":{\"UInt64\":3},\"column_name\":null,\"data_type\":\"UInt8\"}}" }"#
        );

        let field_b = schema.field_with_name("b").unwrap();
        assert_eq!(
            format!("{:?}", field_b),
            r#"DataField { name: "a", data_type: Int64, nullable: false, default_expr: "{\\"Literal\\":{\\"value\\":{\\"UInt64\\":3},\\"column_name\\":null,\\"data_type\\":{\\"type\\":\\"UInt8Type\\"}}}" }"#
        );
    }
