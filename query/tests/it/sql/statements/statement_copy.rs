...
async fn test_statement_copy() -> Result<()> {
    ...
    let tests = vec![
        ...
        file_format = (type = csv field_delimiter = '|' skip_header = 1)",
            expect: r#"Copy into system.configs ,stage_plan:UserStagePlan { stage_info: UserStageInfo { stage_name: "s3://mybucket/data/files", stage_type: External, stage_params: StageParams { storage: S3(StageS3Storage { bucket: "mybucket", location_key: "/data/files", credentials_aws_key_id: "my_key_id", credentials_aws_secret_key: "my_key_id", encryption_master_key: "my_master_key" }) }, file_format_option: FileFormatOptions { format: Csv, skip_header: 1, field_delimiter: "|", record_delimiter: "", compression: None }, copy_option: CopyOptions, comment: "" } }"#,
            err: "",
        },
        TestCase {
