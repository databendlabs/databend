...
impl FuseTable {
    pub async fn do_optimize(
        ...
        let mut snapshots =
            SnapshotReader::read_snapshot_history(da.as_ref(), snapshot_loc, ctx.get_fuse_cache())
                .await?;
    ...
    async fn blocks_of(
        ...
        for x in locations {
            let res = SegmentReader::read(data_accessor.as_ref(), x, ctx.get_fuse_cache()).await?;
