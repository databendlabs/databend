...
impl ClickHouseSession for InteractiveWorker {
    async fn execute_query(
        ...
        let query_ctx = self.session.get_shared_query_context().await.map_err(|err| to_clickhouse_err(err))?;
        let format = query_ctx.get_format_settings().map_err(|err| to_clickhouse_err(err))?;
