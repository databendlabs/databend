...
pub fn prune_columns(expr: &SExpr) -> Result<SExpr> {
    match expr.plan() {
        ...
        RelOperator::Project(p) => Ok(SExpr::create_unary(
            ...
            keep_required_columns(expr.child(0)?, p.columns.clone())?,
        )),
        RelOperator::Aggregate(p) => {
            ...
            Ok(SExpr::create_unary(
                ...
                keep_required_columns(expr.child(0)?, used)?,
            ))
        }
...
fn keep_required_columns(expr: &SExpr, mut required: ColumnSet) -> Result<SExpr> {
    match expr.plan() {
        RelOperator::LogicalGet(p) => {
            let used = required.intersection(&p.columns).cloned().collect();
        ...
        RelOperator::LogicalInnerJoin(p) => {
            ...
            Ok(SExpr::create_binary(
                ...
                keep_required_columns(expr.child(0)?, left)?,
                keep_required_columns(expr.child(1)?, right)?,
            ))
        }
