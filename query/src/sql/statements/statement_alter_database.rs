...
impl AnalyzableStatement for DfAlterDatabase {
    ...
    async fn analyze(&self, ctx: Arc<QueryContext>) -> Result<AnalyzedResult> {
        ...
        let db = self.resolve_database(&self.database_name)?;

        match &self.action {
            AlterDatabaseAction::RenameDatabase(o) => {
                let mut entities = Vec::new();
                let new_db = self.resolve_database(o)?;
...
impl DfAlterDatabase {
    fn resolve_database(&self, database_name: &ObjectName) -> Result<String> {
        let idents = &database_name.0;
        match idents.len() {
            1 => Ok(idents[0].value.clone()),
            _ => Err(ErrorCode::SyntaxException(
                "Alter database name must be [`db`]",
            )),
        }
    }
}
