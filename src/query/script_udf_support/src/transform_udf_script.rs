...
"#,
    ...
    fn test_wasm_runtime_type_matrix() {
        ...
        for (handler, data_type, array) in scalar_cases {
            let declared_return_type = data_type.to_string();
        ...
        let decimals = [
            ...
            (
                Field::new("input", ArrowDataType::Decimal128(28, 28), true),
                Arc::new(
        ...
        let wide_field = Field::new("input", ArrowDataType::Decimal256(76, 30), true);
        let wide_input = RecordBatch::new_empty(Arc::new(Schema::new(vec![wide_field.clone()])));
        let err = call_wasm_function(
            &runtime,
            "wasm_identity_decimal",
            &wide_input,
            &wide_field.clone().with_name("result"),
            "Decimal(76, 30)",
        )
        .unwrap_err();
        assert!(err.message().contains("Decimal256(76, 30)"));
        ...
        let int_field = Field::new("input", ArrowDataType::Int32, true);
        let int_input = RecordBatch::new_empty(Arc::new(Schema::new(vec![int_field])));
        let err = call_wasm_function(
            &runtime,
            "wasm_identity_int32",
            &int_input,
            &Field::new("result", ArrowDataType::Decimal256(76, 30), true),
            "Decimal(76, 30)",
        )
        .unwrap_err();
        assert!(err.message().contains("Decimal256(76, 30)"));
