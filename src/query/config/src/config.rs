...
mod config_converters {
    ...
    impl From<InnerConfig> for Config {
        fn from(inner: InnerConfig) -> Self {
            ...
            let mut log: LogConfig = inner.log.into();
            if lineage.enabled() {
                log.history
                    .log_history_tables
                    .retain(|table| !table.table_name.eq_ignore_ascii_case("lineage_history"));
                if log.history.log_history_tables.is_empty() {
                    log.history.log_history_on = false;
                }
            }
...
mod test {
    ...
    #[test]
    fn test_lineage_config_applies_explicit_retention_independently() {
