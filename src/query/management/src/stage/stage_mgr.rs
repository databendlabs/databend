...
impl StageMgr {
    ...
    fn stage_dir_name(&self) -> DirName<StageIdent> {
        let ident = self.stage_ident("dummy");
        DirName::new(ident)
    }
    ...
    fn stage_file_dir_name(&self, stage: &str) -> DirName<StageFileIdent> {
        let si = StageIdent::new(self.tenant.clone(), stage);
        let fi = StageFileIdent::new(si, "dummy");
        DirName::new(fi)
    }
...
impl StageApi for StageMgr {
    ...
    async fn get_stages(&self) -> Result<Vec<StageInfo>> {
        let dir_name = self.stage_dir_name();
    ...
    async fn list_files(&self, name: &str) -> Result<Vec<StageFile>> {
        let dir_name = self.stage_file_dir_name(name);
