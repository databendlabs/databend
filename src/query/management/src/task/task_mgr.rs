...
use databend_common_meta_app::principal::task_run_ident::TaskRunIdent;
...
impl TaskChannel {
    pub fn instance() -> Arc<TaskChannel> {
        GlobalInstance::get()
    }
}

impl Deref for TaskChannel {
    type Target = mpsc::Sender<TaskMessage>;
...
impl TaskChannel {
    pub fn init(len: usize) -> Result<mpsc::Receiver<TaskMessage>, ErrorCode> {
        let (tx, rx) = mpsc::channel(len);

        GlobalInstance::set(Arc::new(TaskChannel { tx }));
        Ok(rx)
    }
}
    ...
    pub async fn alter_task(
        ...
        match alter_options {
            ...
            AlterTaskOptions::Set {
                ...
                task.suspend_task_after_num_failures = suspend_task_after_num_failures.clone();
