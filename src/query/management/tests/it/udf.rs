...
async fn test_add_udf() -> Result<()> {
    ...
    udf_api.add_udf(udf.clone(), &OnExist::Error).await??;
...
async fn test_already_exists_add_udf() -> Result<()> {
    ...
    let got = udf_api.add_udf(udf.clone(), &OnExist::Error).await?;
...
async fn test_successfully_get_udfs() -> Result<()> {
    ...
    udf_api
        .add_udf(lambda_udf.clone(), &OnExist::Error)
        .await??;

    udf_api
        .add_udf(udf_server.clone(), &OnExist::Error)
        .await??;
