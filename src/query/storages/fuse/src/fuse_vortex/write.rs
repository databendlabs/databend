...
pub fn write_vortex(
    ...
    serialize_vortex_footer(write_summary.footer())
}

fn serialize_vortex_footer(footer: &vortex::file::Footer) -> Result<Vec<u8>> {
    let serialized = footer.clone().into_serializer().serialize()?;
    let total_size = serialized.iter().map(|buf| buf.len()).sum();
