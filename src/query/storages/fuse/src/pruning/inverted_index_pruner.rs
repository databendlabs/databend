...
pub struct InvertedIndexFieldId(u32);

impl InvertedIndexFieldId {
    #[inline]
...
pub struct InvertedIndexPruner {
    ...
    query: Box<dyn Query>,
    inv_field_ids: HashSet<InvertedIndexFieldId>,
...
impl InvertedIndexPruner {
    pub fn try_create(
        ...
        if let Some(inverted_index_info) = inverted_index_info {
            ...
            let mut inv_field_ids = HashSet::new();
            query.query_terms(&mut |term, pos| {
                let inv_field_id = InvertedIndexFieldId::new(term.field().field_id());
                inv_field_ids.insert(inv_field_id);
                if pos {
                    need_position = true;
                }
            });
            for inv_field_id in &inv_field_ids {
                let field = inverted_index_info
                    .index_schema
                    .field(inv_field_id.as_usize());
