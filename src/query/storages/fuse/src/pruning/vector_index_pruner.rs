...
impl VectorIndexPruner {
    ...
    async fn vector_index_hnsw_topn_prune(
        ...
        for candidate in candidates {
            ...
            let vector_prune_result = self
                .process_vector_pruning_candidate(candidate, VectorReaderOperation::Prune { limit })
                .await?;
            processed_blocks += 1;
    ...
    async fn vector_index_topn_prune_by_stat_bounds(
        ...
        for candidate in candidates {
            ...
            let vector_prune_result = self
                .process_vector_pruning_candidate(candidate, VectorReaderOperation::GenerateScores)
                .await?;
            processed_blocks += 1;
    ...
    async fn process_vector_pruning_candidate(
        &self,
        candidate: VectorPruneCandidate,
        operation: VectorReaderOperation,
        ...
        let v: VectorPruningFuture = Box::new(move |permit: OwnedSemaphorePermit| {
            Box::pin(async move {
                let _permit = permit;
                let score_offsets = match operation {
                    VectorReaderOperation::Prune { limit } => {
                        vector_reader.prune(limit, row_count, &location).await?
                    }
                    VectorReaderOperation::GenerateScores => {
                        vector_reader.generate_scores(row_count, &location).await?
                    }
                };
        ...
        let mut join_handlers = self
            .pruning_ctx
            .pruning_runtime
            .try_spawn_batch_with_owned_semaphore(
                self.pruning_ctx.pruning_semaphore.clone(),
                std::iter::once(v),
            )
            .await?;
        let join_handler = join_handlers.pop().unwrap();
        join_handler
            .await
            .map_err(|e| ErrorCode::StorageOther(format!("vector pruning failure: {}", e)))?
    }
