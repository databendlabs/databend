...
mod tests {
    ...
    #[test]
    fn test_merge_statistics_ignores_empty_summaries() {
        let stats = ColumnStatistics::new(
            Scalar::Number(1_i64.into()),
            Scalar::Number(9_i64.into()),
            0,
        ...
        let mut empty_then_populated = empty.clone();
        merge_statistics_mut(&mut empty_then_populated, &populated, None);
        assert_eq!(empty_then_populated.col_stats, populated.col_stats);

        let mut populated_then_empty = populated.clone();
        merge_statistics_mut(&mut populated_then_empty, &empty, None);
        assert_eq!(populated_then_empty.col_stats, populated.col_stats);
    }
}
