...
use crate::statistics::HilbertRect;
use crate::statistics::hilbert_rect_from_stats;
use crate::statistics::rank_hilbert_rects;
...
impl ReclusterStrategy for HilbertReclusterStrategy {
    fn select_segments(
        ...
        for (loc, info) in compact_segments {
            ...
            bounds.push(hilbert_rect_from_stats(&stats)?);
        }
        ...
        let boxes = rank_hilbert_rects(&bounds);
    ...
    fn fetch_task_candidates(
        ...
        let bounds = ordered_indices
            .iter()
            .map(|idx| hilbert_rect_from_stats(blocks[*idx].stats()))
        ...
        for witness in witnesses {
            ...
            while tasks.len() < task_budget {
                ...
                // Every selected block contains this hotspot's exact XY witness. The full witness
                // was admitted above before memory-bounded splitting; do not reapply the depth
                // gate to this truncated chunk, or valid deep hotspots can make no progress when
                // the memory budget fits at most `depth_threshold` blocks. Keep the score pinned
                // to the chunk's actual core depth because growth below adds overlapping blocks
                // without deepening this hotspot.
                ...
                if task_bytes < properties.memory_threshold {
                    let core = HilbertRect::bounding(&boxes, &selected);
...
fn top_witnesses(boxes: &[HilbertRect<usize>], budget: usize) -> Vec<Witness> {
...
fn members_at_witness(boxes: &[HilbertRect<usize>], witness: Witness) -> Vec<usize> {
    boxes
        .iter()
        .enumerate()
        .filter_map(|(idx, item)| {
...
fn sweep_xy<F>(boxes: &[HilbertRect<usize>], transpose: bool, mut visit: F)
...
mod tests {
    ...
    fn bounds(stats: &[ClusterStatistics]) -> Vec<HilbertRect<Scalar>> {
        stats
            .iter()
            .map(hilbert_rect_from_stats)
            .collect::<Result<_>>()
            .unwrap()
    }
    ...
    fn test_top_witnesses_respect_mbrs_and_budget() {
        ...
        let boxes = rank_hilbert_rects(&bounds(&block_stats));
