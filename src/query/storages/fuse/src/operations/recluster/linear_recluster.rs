...
impl ReclusterMutator {
    ...
    pub(super) fn select_scalar_segments(
        &self,
        ...
        for (i, (loc, compact_segment)) in compact_segments.iter().enumerate() {
            ...
            if min.len() != self.cluster_key_types.len()
                || max.len() != self.cluster_key_types.len()
            {
                continue;
            }
            segment_points
    ...
    /// Build linear-cluster task candidates from one recluster group.
    pub(super) fn fetch_linear_task_candidates(
        &self,
