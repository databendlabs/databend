...
// Bound the expensive all-peak probing stage by accepted scored plans, not raw
// depth peaks. Peaks are sorted by depth, width, and position, but aggressive
// FINAL can later reject a deep peak by density. Continue past rejected peaks
// until enough eligible plans are found, so a lower-depth small cleanup is not
// hidden behind deeper low-value hotspots.
const MIN_LINEAR_PLAN_PROBE_COUNT: usize = 32;
const LINEAR_PLAN_PROBE_FACTOR: usize = 4;
...
impl ReclusterStrategy for LinearReclusterStrategy {
    ...
    fn fetch_task_candidates(
        ...
        let mut push_plan = |plans: &mut Vec<CandidatePlan>,
                             peak_pos: usize,
                             local_indices: Vec<usize>,
                             task_bytes: usize,
                             max_depth: usize| {
            ...
            if properties.mode == ReclusterMode::Aggressive
                && !score.passes_tail_efficiency_threshold()
            {
        ...
        let plan_probe_target = peaks.len().min(
            task_budget
                .saturating_mul(LINEAR_PLAN_PROBE_FACTOR)
                .max(MIN_LINEAR_PLAN_PROBE_COUNT),
        );
        let mut probed_peak_count = 0usize;
        'probe_peaks: for &(peak_pos, peak_depth, _) in &peaks {
            probed_peak_count += 1;
            ...
            for local_idx in 0..block_count {
                ...
                if should_split_for_memory {
                    if task_indices.len() >= 2 {
                        ...
                        if plans.len() >= plan_probe_target {
                            break 'probe_peaks;
                        }
                    } else {
                        task_indices.clear();
        ...
        debug!(
            "recluster: probed task candidates group={} block_count={} avg_depth={} depth_threshold={} max_depth={} peak_count={} probed_peak_count={} plan_probe_target={} task_count={} rejected_tail_count={} rejected_conflict_count={}",
            group,
            block_count,
            average_depth,
