    ...
    pub async fn do_recluster(
        ...
        let parts = loop {
            ...
            let (block_count, parts) = if pending_windows.is_empty() {
                ...
                // Step 3: choose task candidates. V1 keeps the early-accept
                // shortcut; v2 ranks all pending tasks by benefit density.
                let enable_task_selection_v2 = mutator.properties.enable_task_selection_v2;
                let early_accept_only =
                    !enable_task_selection_v2 && early_accept_count >= mutator.max_tasks;
                ...
                for (window_idx, window) in pending_windows.iter().enumerate() {
                    for (task_idx, task) in window.tasks.iter().enumerate() {
                        if enable_task_selection_v2
                            || !early_accept_only
                            || mutator.passes_early_accept(task)
                        {
                ...
                sorted_tasks.sort_by(|left, right| {
                    let ordering = if enable_task_selection_v2 {
                        right.2.score.cmp_desc_v2(&left.2.score)
                    } else {
                        right.2.score.cmp_desc(&left.2.score)
                    };
