        ...
        if candidate_window.tasks.is_empty() {
            ...
            if compactable_repack || unordered_repack {
                ...
                candidate_window.tasks.push(ReclusterTaskCandidate {
                    ...
                    output_level: 0,
                    all_ordered: false,
                    kind: ReclusterTaskKind::RepackOnly,
                });
            } else {
    ...
    fn build_tasks(
        ...
        if self.properties.mode == ReclusterMode::Aggressive {
            candidates.retain(|candidate| {
                let eligible = candidate.passes_aggressive_tail_filter();
                if !eligible {
                    debug!("recluster: skip inefficient task candidate {}", candidate);
                }
                eligible
            });
        }
    ...
    fn build_recluster_task_candidates_for_indices(
            ...
            && total_bytes as usize <= self.properties.memory_threshold
        {
            ...
            return Ok(vec![task_candidate(
                ...
                &indices,
                blocks,
                ReclusterTaskKind::PhysicalCompaction,
            )]);
        }
