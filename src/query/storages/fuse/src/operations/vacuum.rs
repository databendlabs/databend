...
impl FuseTable {
    pub async fn vacuum_table(
            ...
            .await
        {
            ...
            info!("[VACUUM] Vacuum table {} done", self.table_info.name);
        }
    }
    ...
    pub async fn list_files_until_prefix(
        ...
        info!("[FUSE-VACUUM] Listing files until prefix: {}", until);
        ...
        for snapshot_ref in table_info.meta.refs.values() {
            match &snapshot_ref.info {
                SnapshotRefInfo::Tag(location) => {
                    ...
                    match SnapshotsIO::read_snapshot_for_vacuum(operator.clone(), location).await? {
                        Some(snapshot) => {
                ...
                SnapshotRefInfo::Branch { head, .. } => {
                    // Read branch head snapshot to create branch-specific root_snapshot_lite
                    match SnapshotsIO::read_snapshot_for_vacuum(operator.clone(), head).await? {
                        Some(snapshot) => {
                            let branch_root_snapshot_lite = Arc::new(SnapshotLiteExtended {
                                format_version: TableMetaLocationGenerator::snapshot_version(head),
