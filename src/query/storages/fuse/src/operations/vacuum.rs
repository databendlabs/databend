...
impl FuseTable {
    pub async fn vacuum_table(
        ...
        if self.is_table_branch() {
            info!(
                "Skip automatic vacuum for branch table: {}, ident: {}",
    ...
    pub fn branch_table_from_meta(
        ...
        let table_info = TableInfo {
            ident: TableIdent {
                table_id: branch.branch_id.table_id,
            ...
            // otherwise follow-up metadata cleanup could run against the wrong catalog.
            catalog_info: self.table_info.catalog_info.clone(),
        };
        Self::create_without_refresh_table_info(table_info, *s3_storage_class)
    }
