...
mod tests {
    ...
    fn test_prepare_persistent_navigation_metadata() {
        ...
        let original = table_meta.clone();
        for row_policy in [true, false] {
            table_meta = original.clone();
            if row_policy {
                table_meta.column_mask_policy = None;
            } else {
                table_meta.row_access_policy = None;
            }
            let before = table_meta.clone();
            let err =
                FuseTable::prepare_persistent_navigation_metadata(&mut table_meta).unwrap_err();
            assert_eq!(err.code(), ErrorCode::INVALID_ARGUMENT);
            assert_eq!(
                table_meta, before,
                "legacy policy rejection must not mutate metadata"
            );
        }
        table_meta = original;
        table_meta.column_mask_policy = None;
        ...
        table_meta
            .column_mask_policy_columns_ids
            .insert(1, policy.clone());
        table_meta.row_access_policy_columns_ids = Some(policy.clone());
