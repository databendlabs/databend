            ...
            if let Some(part) = self.ctx.get_partition() {
                ...
                self.output
                    .push_data(Ok(DataBlock::empty_with_meta(AnalyzeNDVMeta::create(
                        ...
                        std::mem::take(&mut self.dropped_frequency_columns)
                            .into_iter()
                            .collect(),
    ...
    fn process(&mut self) -> Result<()> {
        match std::mem::replace(&mut self.state, State::Finish) {
            State::CollectNDV {
                ...
                if !indexes.is_empty() && self.no_scan && !collect_kll && !collect_frequency_stats {
                ...
                if indexes.is_empty() && !collect_kll && !collect_frequency_stats {
                    self.state = State::ReadData(None);
                } else {
                    assert!(self.segment_with_hll.is_none());
                    let block_indexes = if collect_kll || collect_frequency_stats {
            ...
            State::MergeHLL => {
                ...
                for (((new, mut top_n), mut count_min_sketch), idx) in new_hlls
                    .into_iter()
                    ...
                    .zip(new_indexes.into_iter())
                {
                        ...
                        (&mut self.top_n, self.top_n_size)
                    {
                        ...
                        top_n.retain(|column_id, _| !dropped_frequency_columns.contains(column_id));
                        merge_column_top_n_mut(block_top_n, top_n, top_n_size);
                    }
                    if let Some(block_count_min_sketch) = &mut self.count_min_sketch {
                        let dropped_frequency_columns = &self.dropped_frequency_columns;
                        count_min_sketch
                            .retain(|column_id, _| !dropped_frequency_columns.contains(column_id));
        ...
        match std::mem::replace(&mut self.state, State::Finish) {
            ...
            State::BuildHLL => {
                ...
                for (column_hlls, kll_histograms) in block_stats {
                    let (hll, top_n, count_min_sketch, dropped_frequency) =
                        split_block_stats(column_hlls);
                    for column_id in dropped_frequency {
...
mod tests {
    ...
    fn split_block_stats_keeps_frequency_only_out_of_hll_slots() {
        ...
        let (hll, top_n, count_min_sketch, dropped_frequency) =
            split_block_stats(Some(BlockStats {
                hll: HashMap::new(),
                top_n,
                count_min_sketch,
                dropped_frequency: vec![7],
            }));

        assert!(hll.is_none());
        assert_eq!(top_n.len(), 1);
        assert_eq!(count_min_sketch.len(), 1);
        assert_eq!(dropped_frequency, vec![7]);
    }
}
