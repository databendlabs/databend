...
///
/// Virtual column statistics are not reduced: their column ids are local to each segment
/// and only meaningful together with that segment's virtual schema, so they cannot be
/// merged by id across segments (see `merge_statistics_mut`).
...
pub struct ReducedSegmentStats {
    ...
    /// reduced, because reducing `None` cluster statistics with anything yields `None`.
    segment_count: usize,
}
