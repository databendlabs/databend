...
impl ReadBlockContext {
    pub fn create(
        ...
        Ok(Arc::new(Self {
            ...
            storage_format,
            block_read_ctx: block_reader.read_context(),
            block_format,
            block_reader,
            index_reader,
            virtual_reader,
    ...
    pub async fn read_data(&self, part: PartInfoPtr) -> Result<ParquetDataSource> {
        ...
        if let Some(data_block) = self.read_page_range_data(fuse_part).await? {
            return Ok(ParquetDataSource::PageRange(data_block));
        }
    ...
    async fn read_page_range_data(&self, part: &FuseBlockPartInfo) -> Result<Option<DataBlock>> {
        ...
        read_parquet_page_range_data(&self.block_reader, &self.read_settings, part).await
    }
