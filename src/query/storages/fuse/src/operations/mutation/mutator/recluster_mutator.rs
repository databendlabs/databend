...
impl ReclusterMutator {
    ...
    fn build_recluster_tasks_for_indices(
        ...
        // The split loop cannot emit a task with fewer than two blocks, so the
        // first two selected blocks are the minimum possible task. Check that
        // minimal task against the per-node memory budget before planning tasks.
        let minimal_task_bytes = selected_idx.iter().take(2).fold(0usize, |total, idx| {
            total.saturating_add(blocks[*idx].meta.block_size as usize)
        });
        if minimal_task_bytes > self.memory_threshold {
            debug!(
                "recluster: level selection detail level={} block_count={} selected_count={} minimal_task_bytes={} selected_bytes={} memory_threshold={} skip_reason=minimal_task_exceeds_memory_budget",
                group,
                block_count,
                selected_block_count,
                minimal_task_bytes,
