        ...
        let selected = if selected_blocks_idx.is_empty() {
            let unordered = || {
                blocks.windows(2).any(|w| {
                    let left = self.build_cluster_stats_for_recluster(
                        w[0].1.cluster_stats.as_ref(),
                        &w[0].1.col_stats,
                    );
                    let right = self.build_cluster_stats_for_recluster(
                        w[1].1.cluster_stats.as_ref(),
                        &w[1].1.col_stats,
                    );
                    sort_by_cluster_stats(&Some(left), &Some(right), self.cluster_key_id)
                        == Ordering::Greater
                })
            };
