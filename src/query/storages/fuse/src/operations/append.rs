...
impl FuseTable {
    ...
    pub fn get_cluster_stats_gen(
        ...
        let partition_keys = self
            ...
            .map(|keys| bind_key_exprs(ctx.clone(), table_meta.clone(), keys))
            .transpose()?
            .unwrap_or_default();
        ...
        let mut cluster_key_index = Vec::with_capacity(if is_hilbert {
            0
        } else {
            cluster_key_exprs.len()
        });
        let mut stats_key_offsets = Vec::with_capacity(cluster_key_exprs.len());
        let mut source_column_ids = HashMap::with_capacity(cluster_key_exprs.len());
        ...
        for (key_index, cluster_key_expr) in cluster_key_exprs.iter().enumerate() {
            ...
            if !is_vector_key {
                ...
                if let Expr::ColumnRef(ColumnRef { id, .. }) = cluster_key_expr {
                    source_column_ids.insert(index, table_schema.field(*id).column_id());
                }
            }
        }
