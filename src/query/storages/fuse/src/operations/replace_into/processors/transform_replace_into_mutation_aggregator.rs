...
struct TransformReplaceIntoMutationAggregator {
    inner: ReplaceIntoOperationAggregator,
    input: Arc<InputPort>,
...
impl TransformReplaceIntoMutationAggregator {
    fn create(
        input: Arc<InputPort>,
        output: Arc<OutputPort>,
        inner: ReplaceIntoOperationAggregator,
...
impl Processor for TransformReplaceIntoMutationAggregator {
    fn name(&self) -> String {
        "ReplaceIntoMutationAggregator".to_string()
    }
    ...
    fn event(&mut self) -> Result<Event> {
        ...
        if self.input.has_data() {
            let data = self.input.pull_data().ok_or_else(|| {
                ErrorCode::Internal("replace-into aggregator input reported data but returned none")
            })??;
    ...
    async fn async_process(&mut self) -> Result<()> {
        match std::mem::replace(&mut self.state, State::Consume) {
            State::Accumulate(data) => {
                self.inner
                    .accumulate(ReplaceIntoOperation::try_from(data)?)
                    .await
            }
            State::Prepare => {
            ...
            _ => Err(ErrorCode::Internal(
                "invalid replace-into aggregator async state",
            )),
        }
    }
}
...
impl ReplaceIntoOperationAggregator {
    pub fn into_pipe_item(self) -> PipeItem {
        ...
        let processor =
            TransformReplaceIntoMutationAggregator::create(input.clone(), output.clone(), self);
