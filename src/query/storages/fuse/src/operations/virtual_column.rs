...
/// Maps an owner storage prefix to its table id.
type StoragePrefixes = HashMap<String, u64>;

/// Resolve the owner using the longest matching storage prefix.
fn owner_of_path(storage_prefixes: &StoragePrefixes, path: &str) -> Option<u64> {
    storage_prefixes
        .iter()
...
async fn vacuum_virtual_column_orphans(
    ...
    //
    // This cleanup intentionally only sweeps owners that are still retainable in the current
    // branch family. If a referenced segment / virtual file belongs to an owner that is no longer
        ...
        collect_snapshot_referenced_segments(ctx.clone(), fuse_table).await?
    {
        for segment in base_segments {
            let Some(owner_id) = owner_of_path(&storage_prefixes, &segment.0) else {
                warn!(
                    "vacuum virtual columns: skip segment with unknown owner, table: {}, segment: {}",
                    fuse_table.get_table_info().desc,
                    segment.0
                );
                continue;
    ...
    // Branch segments.
    for branch_table in owners.values() {
        ...
        for segment in branch_segments {
            let Some(owner_id) = owner_of_path(&storage_prefixes, &segment.0) else {
                warn!(
                    "vacuum virtual columns: skip segment with unknown owner, branch: {}, segment: {}",
    ...
    for (owner_id, segments) in &segments_by_owner {
        ...
        for vl in virtual_locations {
            let Some(vl_owner) = owner_of_path(&storage_prefixes, &vl) else {
                warn!(
                    "vacuum virtual columns: skip virtual file with unknown owner, owner_id: {}, location: {}",
                    owner_id, vl
                );
                continue;
            };
...
pub async fn cleanup_vacuum_virtual_column_files(
    ...
    vacuum_virtual_column_orphans(ctx, fuse_table).await
}
