...
mod tests {
    ...
    fn test_build_table_lvt_check_uses_gc_safe_timestamp() {
        ...
        assert_eq!(
            FuseTable::build_table_lvt_check(&table_info, Some(gc_safe_time)).unwrap(),
            None
        );
        ...
        let check = FuseTable::build_table_lvt_check(&table_info, Some(gc_safe_time)).unwrap();
        assert_eq!(check.unwrap().time, gc_safe_time);

        let err = FuseTable::build_table_lvt_check(&table_info, None).unwrap_err();
