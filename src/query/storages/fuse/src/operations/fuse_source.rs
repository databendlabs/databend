...
impl Processor for FuseTableSource {
    ...
    fn event(&mut self) -> Result<Event> {
        ...
        match self.state {
            ...
            State::Generated(_, _) => Err(ErrorCode::InternalError("It's a bug.")),
        }
    }
        ...
        match std::mem::replace(&mut self.state, State::Finish) {
            State::Deserialize(part, chunks, prewhere_data) => {
                ...
                }) = prewhere_data
                {
                    let block = if chunks.is_empty() {
                        ...
                        return Err(ErrorCode::InternalError("It's a bug. Need remain reader"));
                    };
            ...
            State::ReadDataRemain(part, prewhere_data) => {
                if let Some(remain_reader) = self.remain_reader.as_ref() {
                    ...
                    Err(ErrorCode::InternalError("It's a bug. No remain reader"))
                }
            }
