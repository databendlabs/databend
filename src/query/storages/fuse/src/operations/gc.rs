...
impl From<Arc<SegmentInfo>> for LocationTuple {
    fn from(value: Arc<SegmentInfo>) -> Self {
        ...
        let mut delete_location = HashSet::new();
        for block_meta in &value.blocks {
            ...
            if let Some(delete_loc) = &block_meta.delete_mask_location {
                delete_location.insert(delete_loc.0.clone());
            }
        }
        Self {
            block_location,
...
impl FuseTable {
    ...
    async fn get_block_locations(
        ...
        for (idx, location_tuple) in results.into_iter().enumerate() {
            ...
            deletes.extend(location_tuple.delete_location.into_iter());
        }
