...
impl TableMetaFunc for ClusteringStatistics {
    ...
    async fn apply(
        ...
        let calculate_depth = cluster_keys.is_linear();
        ...
        // block_depth is a global overlap metric for linear keys, so collect all scalar ranges
        // before LIMIT is applied. Hilbert and vector depth are intentionally unavailable here;
        // for those layouts, collect only the rows that will be returned.
