...
impl RuntimeFilterPruneContext {
    pub fn try_create(
        ...
        scan_id: IndexType,
        table_schema: TableSchemaRef,
        enable_dynamic_block_prune: bool,
    ) -> Result<Option<Self>> {
        let runtime_filter_ready = Self::statistics_ready(&ctx.get_runtime_filter_ready(scan_id));
        let dynamic_block_prune_filters = if enable_dynamic_block_prune {
            ctx.get_dynamic_block_prune_filters(scan_id)
        } else {
            Vec::new()
        };
    ...
    pub fn has_dynamic_block_prune_filters(&self) -> bool {
        !self.dynamic_block_prune_filters.is_empty()
    }

    pub fn log_dynamic_block_prune(
        &self,
        segment_idx: usize,
        blocks_before: usize,
        blocks_after: usize,
    ) {
        if self.dynamic_block_prune_filters.is_empty() {
            return;
        }

        info!(
            "[FUSE-PRUNER] dynamic block prune enabled: scan_id={}, segment_idx={}, \
             blocks_before={}, blocks_after={}, blocks_pruned={}",
            self.scan_id,
            segment_idx,
            blocks_before,
            blocks_after,
