...
impl AsyncSink for ColumnOrientedBlockPruneSink {
    ...
    async fn consume(&mut self, mut data: DataBlock) -> Result<bool> {
        ...
        let dynamic_block_locations = match self.runtime_filter_prune_context.as_ref() {
            Some(context) if context.has_dynamic_block_prune_filters() => {
                context.dynamic_block_locations().await?
            }
            _ => Vec::new(),
        };
        ...
        let mut blocks_after_dynamic_prune = 0;
