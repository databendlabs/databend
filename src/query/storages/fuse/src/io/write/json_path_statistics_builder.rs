...
impl JsonPathStatisticsBuilder {
    ...
    pub fn observe_path(&mut self, source_column_id: ColumnId, key_paths: &[KeyPath<'_>]) {
        ...
        let path = encode_virtual_path(
            &key_paths
                .iter()
                .map(virtual_path_segment)
                .collect::<Vec<_>>(),
        );
    ...
    pub fn finalize(&mut self) -> Vec<DraftVirtualColumnPathStatistics> {
        ...
        for (source_field, paths) in self.variant_fields.iter().zip(source_paths) {
            ...
            let mut path_counts = paths
                .into_iter()
                .map(|(path, value_count)| DraftVirtualPathCount { path, value_count })
                .collect::<Vec<_>>();
            let complete = path_counts.len() <= self.max_path_statistics_per_source;
            ...
            path_counts.truncate(self.max_path_statistics_per_source);
...
fn virtual_path_segment(path: &KeyPath<'_>) -> VirtualPathSegment {
    match path {
        KeyPath::Index(index) => VirtualPathSegment::Index(*index),
