...
pub async fn load_spatial_index_meta(dal: Operator, path: &str) -> Result<Arc<SpatialIndexMeta>> {
    ...
    async move {
        let reader = MetaReaders::spatial_index_meta_reader(dal);
        let version = 0;
...
#[fastrace::trace]
pub async fn load_spatial_index_files<'a>(
    operator: Operator,
    ...
    // 1. load index meta
    let spatial_index_meta = load_spatial_index_meta(operator.clone(), location).await?;

    // 2. build index schema
    let spatial_index_fields: Vec<_> = spatial_index_meta
        .columns
        .iter()
        .map(|col| Field::new(col.0.clone(), arrow::datatypes::DataType::Binary, false))
        .collect();
    let spatial_index_schema = Schema::new(Fields::from(spatial_index_fields));

    let spatial_index_schema_desc =
        Arc::new(ArrowSchemaConverter::new().convert(&spatial_index_schema)?);

    // 3. collect column metas that needed to build spatial index
    let spatial_column_chunk_metas = &spatial_index_meta.columns;

    let mut column_indices = Vec::with_capacity(column_names.len());
    for column_name in column_names {
        let column_index = spatial_index_schema.index_of(column_name)?;
        column_indices.push(column_index);
    }

    let projected_column_metas: Vec<_> = spatial_column_chunk_metas
        .iter()
        .enumerate()
    ...
    let spatial_index_file_cache = CacheManager::instance().get_spatial_index_file_cache();
    for (i, (name, col_meta)) in column_indices
        .iter()
        .zip(projected_column_metas.into_iter())
    {
        let cache_key = cache_key_of_column(location, name);
        if let Some(cache_file) = spatial_index_file_cache.get_sized(&cache_key, col_meta.len) {
    ...
    if !ranges.is_empty() {
        ...
        for (i, (chunk_idx, range)) in &merge_io_result.columns_chunk_offsets {
            ...
            let file = SpatialIndexFile::create(name.clone(), data.to_vec().into());

            // add index file to cache
            spatial_index_file_cache.insert(cache_key, file.clone());
            column_data.insert(*i as usize, Arc::new(file));
        }
    }

    // 5. deserialize raw data to spatial index data
    let mut builder =
        RowGroupImplBuilder::new(1, &spatial_index_schema_desc, TableCompression::Zstd.into());
    ...
    let field_levels = parquet_to_arrow_field_levels(
        spatial_index_schema_desc.as_ref(),
        ProjectionMask::leaves(&spatial_index_schema_desc, column_indices),
        None,
    )?;
    ...
    let mut spatial_bytes_len = 0;
    let mut spatial_columns = Vec::with_capacity(4);
    for i in 0..record.num_columns() {
        let spatial_binary = record.column(i).clone();
        let column = Column::from_arrow_rs(
            spatial_binary,
            &databend_common_expression::types::DataType::Binary,
        )?;
        spatial_bytes_len += column.memory_size(false);
        spatial_columns.push(column);
    }

    let elapsed = start.elapsed().as_millis() as u64;
    // Perf.
    {
        metrics_inc_block_spatial_index_read_bytes(spatial_bytes_len as u64);
        metrics_inc_block_spatial_index_read_milliseconds(elapsed);
    }
