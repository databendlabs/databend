...
fn page_fetch_lock(location: &str, file_id: usize, policy: RangeCachePolicy) -> Arc<FetchLock> {
    ...
    fetch_lock(format!("page:{location}:{domain}:{file_id}"))
}
...
impl RemoteBundleFileHandle {
    ...
    async fn read_cached_lookup_file(&self, range: Range<usize>) -> io::Result<OwnedBytes> {
        ...
        if data.is_none() {
            let fetch_lock = page_fetch_lock(&self.location, self.file_id, self.cache_policy);
    ...
    async fn read_cached_pages(&self, range: Range<usize>) -> io::Result<OwnedBytes> {
        ...
        if request_pages.len() != last_page - first_page + 1 {
            ...
            while page_no <= last_page {
                ...
                for (fetched_page_no, page) in self.fetch_page_group(missing_start, page_no).await?
                {
