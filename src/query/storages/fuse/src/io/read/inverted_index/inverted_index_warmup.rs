...
impl InvertedIndexWarmupInfo {
    pub fn try_create(query: &dyn Query, fallback_fields: &[Field]) -> Result<Self> {
        let mut actions = Vec::new();
        collect_warmups(query, fallback_fields, &mut actions)?;
        Ok(Self {
...
fn collect_warmups(
    ...
    if let Some(term_query) = query.downcast_ref::<TermQuery>() {
        ...
        for (_, subquery) in boolean_query.clauses() {
            collect_warmups(subquery.as_ref(), fallback_fields, actions)?;
        }
        ...
        // Prefix expansion of `.idx` / `.pos` is deferred to a follow-up PR.
        add_full_field_warmups(query, fallback_fields, true, actions);
    } else if query.downcast_ref::<FuzzyTermQuery>().is_some() {
        // Automaton expansion of `.idx` is deferred to a follow-up PR.
        add_full_field_warmups(query, fallback_fields, false, actions);
    } else if query.downcast_ref::<BoostQuery>().is_some()
        || query.downcast_ref::<ConstScoreQuery>().is_some()
    {
        // Child query is not visible. Load the complete inverted index until a follow-up PR
        // expands the inner query after warming `.term`.
        add_full_field_warmups(
            query,
            fallback_fields,
            query_needs_positions(query),
            actions,
        );
    ...
    {
        // Opaque composites such as DisjunctionMaxQuery may hide a range or automaton child.
        // Native recursion is deferred to a follow-up PR.
        add_full_field_warmups(
            query,
            fallback_fields,
            query_needs_positions(query),
...
fn query_needs_positions(query: &dyn Query) -> bool {
