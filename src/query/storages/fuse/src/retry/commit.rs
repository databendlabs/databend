...
async fn compute_table_segments_diffs(
    ...
    for (update_table_meta_req, _) in &req.update_table_metas {
        ...
        let dal = new_table.get_operator();
        ...
        for (branch_id, base_location) in base_snapshot_locations {
            let base_snapshot =
                FuseTable::read_snapshot_with_opt_location(dal.clone(), base_location).await?;
            ...
            let new_snapshot_location = if let Some(branch_id) = branch_id {
                ...
                new_table.snapshot_loc()
            };
            let new_snapshot =
                FuseTable::read_snapshot_with_opt_location(dal.clone(), new_snapshot_location)
                    .await?;
...
async fn load_branch_table_and_snapshot(
    ...
    if let Some(id) = branch_id {
        ...
        let snapshot =
            FuseTable::read_snapshot_with_location(latest_table.get_operator(), location).await?;
        ...
        let snapshot = latest_table.read_table_snapshot().await?;
