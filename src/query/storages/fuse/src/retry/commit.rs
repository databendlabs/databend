...
async fn try_rebuild_req(
    ...
    for (tid, seq, table_meta) in update_failed_tbls {
        ...
        let stats_row_delta = additional_stats_row_delta(&base_summary, &new_summary);
...
fn additional_stats_row_delta(base_summary: &Statistics, new_summary: &Statistics) -> u64 {
    ...
    match base_summary.additional_stats_meta.as_ref() {
        ...
        None => new_summary
            .row_count
            .saturating_sub(base_summary.row_count)
            .min(new_stats_meta.row_count),
    }
}
...
mod tests {
    ...
    #[test]
    fn test_additional_stats_row_delta_from_stats_meta() {
        let base_summary = summary(100, Some(100));
        let new_summary = summary(110, Some(110));

        assert_eq!(additional_stats_row_delta(&base_summary, &new_summary), 10);
    }

    #[test]
    fn test_additional_stats_row_delta_without_base_stats_meta() {
    ...
    #[test]
    fn test_additional_stats_row_delta_without_new_stats_meta() {
        let base_summary = summary(100, Some(100));
        let new_summary = summary(110, None);

        assert_eq!(additional_stats_row_delta(&base_summary, &new_summary), 0);
    }

    #[test]
    fn test_additional_stats_row_delta_can_differ_from_table_row_delta() {
        let base_summary = summary(100, Some(100));
        let new_summary = summary(100, Some(130));

        assert_eq!(additional_stats_row_delta(&base_summary, &new_summary), 30);
    }
}
