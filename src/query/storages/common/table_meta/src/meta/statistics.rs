...
impl ColumnTopN {
    ...
    fn merge_compatible(&mut self, mut other: ColumnTopN) -> bool {
        ...
        loop {
            match (lhs_iter.peek(), rhs_iter.peek()) {
                (Some(lhs), Some(rhs)) => {
                    match stats_scalar_cmp(&lhs.scalar.as_ref(), &rhs.scalar.as_ref())
                        .expect("ColumnTopN scalar compatibility was checked before merge")
                    {
                        Ordering::Less => {
    ...
    fn find(&self, scalar: &ScalarRef<'_>) -> Option<std::result::Result<usize, usize>> {
        ...
        Some(self.values.binary_search_by(|entry| {
            stats_scalar_cmp(&entry.scalar.as_ref(), scalar)
                .expect("ColumnTopN scalar compatibility was checked before binary search")
        }))
    }
