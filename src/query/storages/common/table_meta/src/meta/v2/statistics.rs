    ...
    pub fn widen_decimal_dimension(
        ...
        if let Some(pages) = &mut self.pages {
            if let Some(page) = pages.get_mut(index) {
                widen_decimal_scalar(page, size)?;
            }
        }
        Ok(())
    }
...
mod tests {
    ...
    fn widens_decimal_statistics_without_changing_values() {
        ...
        cluster.pages = Some(vec![Scalar::Decimal(DecimalScalar::Decimal128(500, old))]);
        ...
        assert_eq!(
            cluster.pages,
            Some(vec![Scalar::Decimal(DecimalScalar::Decimal128(500, new))])
        );
