...
pub enum InvertedIndexBundleVersion {
    ...
    /// sibling objects referenced by suffix. Version 1 (inline only, never released) is rejected.
    #[default]
    V2 = 2,
}
...
impl InvertedIndexBundleFooter {
        ...
        B: AsRef<[u8]>,
    {
        ...
        output.extend_from_slice(&(InvertedIndexBundleVersion::V2 as u32).to_le_bytes());
...
mod tests {
    ...
    fn test_parse_footer_from_tail() {
        ...
        assert_eq!(footer.bundle_version, InvertedIndexBundleVersion::V2);
    ...
    #[test]
    fn test_footer_and_trailer_versions_are_v2() {
        let bytes = build_bundle();
        let footer = InvertedIndexBundleFooter::open(&bytes).unwrap();
    ...
    fn test_bundle_rejects_unknown_footer_version() {
        ...
        bytes[footer_start + 4..footer_start + 8].copy_from_slice(&1u32.to_le_bytes());
        assert_eq!(
            InvertedIndexBundleFooter::open(&bytes).unwrap_err().kind(),
            io::ErrorKind::InvalidData
        );
