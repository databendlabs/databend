...
fn get_table_operator_fro_drop_operation(table_meta: &TableMeta) -> Result<Operator> {
...
pub async fn drop_table_by_id(
    ...
    match engine.as_str() {
        "FUSE" => {
            ...
            let op = get_table_operator_fro_drop_operation(&table_meta)?;
...
pub async fn drop_all_temp_tables(
    ...
    for (dir, table_meta) in fuse_table_data {
        ...
        match get_table_operator_fro_drop_operation(&table_meta) {
            Ok(op) => {
