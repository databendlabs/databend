...
impl RowDecoder for NdJsonDecoder {
    fn add(&self, state: &mut BlockBuilderState, batch: RowBatchWithPosition) -> Result<()> {
        ...
        for (row_id, row) in data.iter().enumerate() {
            ...
            if !row.is_empty() {
                if let Err(e) = self.read_row(row, columns, &null_if, state.file_full_path.clone())
                {
...
fn map_json_error(err: serde_json::Error, data: &[u8], file_full_path: String) -> FileParseError {
