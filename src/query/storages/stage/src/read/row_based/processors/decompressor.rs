...
impl DecompressorCore {
    ...
    fn accept_compressed_input(&mut self, batch: BytesBatch) -> Result<()> {
        ...
        // to signal EOF. Gzip may continue with another member, while XZ may have
        // zero-byte Stream Padding after Done.
        if matches!(decoder_state, DecompressState::Done) {
            if matches!(self.current_algo, Some(CompressAlgorithm::Gzip)) && !batch.data.is_empty()
            {
                let decoder = self.decoder.as_mut().unwrap();
                decoder
                    .reinit()
                    .map_err(|e| Self::compression_error(self.path.as_ref(), e))?;
                decoder.fill(&batch.data);
    ...
    fn drain_decoder_output(&mut self) -> Result<Option<DataBlock>> {
        ...
        loop {
            ...
            if matches!(state, DecompressState::Done) {
                if matches!(self.current_algo, Some(CompressAlgorithm::Gzip))
                    && self.decoder.as_ref().unwrap().has_buffered_input()
                {
                    self.decoder
                        .as_mut()
                        .unwrap()
            ...
            match state {
                ...
                DecompressState::Done => unreachable!(),
            }
        }
...
mod tests {
    ...
    #[test]
    fn test_decompressor_processor_concatenated_gzip_members() -> Result<()> {
        // RFC 1952 permits a gzip file to contain a sequence of independently
        // compressed members. Exercise both reader shapes: members delivered in
        // one input batch and a new member arriving after the first was drained.
        let first = original_bytes(PROCESSOR_OUTPUT_SIZE * 2 + 17);
        let mut second = original_bytes(PROCESSOR_OUTPUT_SIZE * 2 + 31);
        second.reverse();

        let first_compressed = compress(CompressAlgorithm::Gzip, &first)?;
        let second_compressed = compress(CompressAlgorithm::Gzip, &second)?;
        ...
        let input_cases = [
            vec![bytes_batch_with_path(
                concatenated,
                true,
                PROCESSOR_TEST_PATH,
            )],
            vec![
                bytes_batch_with_path(first_compressed, false, PROCESSOR_TEST_PATH),
                bytes_batch_with_path(second_compressed, true, PROCESSOR_TEST_PATH),
            ],
        ];

        for inputs in input_cases {
            let (outputs, _) =
                run_decompressor_processor(CompressAlgorithm::Gzip, PROCESSOR_OUTPUT_SIZE, inputs)?;
    ...
    fn test_decompressor_rejects_data_after_stream_end() -> Result<()> {
        ...
        assert!(
            err.code() == ErrorCode::INVALID_COMPRESSION_DATA,
            "unexpected error: {err}"
        );

        let err = run_decompressor_processor(CompressAlgorithm::Gzip, PROCESSOR_OUTPUT_SIZE, vec![
            bytes_batch_with_path(compressed, false, PROCESSOR_TEST_PATH),
            bytes_batch_with_path(b"trailing".to_vec(), true, PROCESSOR_TEST_PATH),
        ])
