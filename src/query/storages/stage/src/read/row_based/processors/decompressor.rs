...
impl DecompressorCore {
    ...
    fn accept_compressed_input(&mut self, batch: BytesBatch) -> Result<()> {
        ...
        // to signal EOF. Only that EOF marker may follow.
        if matches!(decoder.state(), DecompressState::Done) {
            if !batch.data.is_empty() {
                return Err(ErrorCode::BadBytes(format!(
                    "got {} more bytes after the compressed stream of file {} already ended",
                    batch.data.len(),
                    batch.path
                )));
            }
    ...
    fn drain_decoder_output(&mut self) -> Result<Option<DataBlock>> {
        ...
        if is_done {
            let buffered = self.decoder.as_ref().unwrap().buffered_input_len();
            if buffered != 0 {
                return Err(ErrorCode::BadBytes(format!(
                    "got {buffered} more bytes after the compressed stream of file {} already ended",
                    path.as_deref().unwrap_or("")
                )));
            }
        }
