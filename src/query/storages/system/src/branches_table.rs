        ...
        impl BranchesHistoryAware for BranchesTable<$with_history> {
                ...
                M: RefApi + ?Sized,
            {
                if with_history {
                    ...
                    meta_api.list_table_branches(table_id).await
                }
                .map_err(Into::into)
            }
        }
...
where BranchesTable<WITH_HISTORY>: BranchesHistoryAware
{
    ...
    where
        M: RefApi + ?Sized,
    {
        let branches = Self::list_branches(meta_api, table.get_id(), WITH_HISTORY).await?;
