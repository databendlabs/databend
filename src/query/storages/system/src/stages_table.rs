...
impl AsyncSystemTable for StagesTable {
    ...
    async fn get_full_data(
        ...
        let mut storage_type = Vec::with_capacity(stages.len());
        let mut urls: Vec<Option<String>> = Vec::with_capacity(stages.len());
        ...
        for stage in stages.into_iter() {
            ...
            if stage.stage_type == StageType::External {
                ...
                storage_type.push(Some(storage.storage_type()));
                urls.push(storage.url());
                ...
                let storage_json = serde_json::to_string(&storage.redacted_for_display())?;
                ...
                storage_params.push(Some(storage_jsonb.to_vec()));
            } else {
                storage_type.push(None);
                urls.push(None);
                endpoints.push(None);
                has_credentials.push(false);
                has_encryption_key.push(false);
                storage_params.push(None);
            }
