...
use common_expression::{DataBlock, TableSchemaRefExt, TableField, TableDataType, types::{NumberDataType, StringType, UInt64Type}, FromData};
use common_meta_app::schema::{TableInfo, TableIdent, TableMeta};
use common_storages_fuse::TableContext;
use storages_common_cache_manager::CacheManager;


use crate::{SyncSystemTable, SyncOneBlockSystemTable};

pub struct InMemoryCacheTable{
    table_info: TableInfo,
}

impl SyncSystemTable for InMemoryCacheTable {
    const NAME: &'static str = "system.in_memory_caches";
...
impl InMemoryCacheTable {
    pub fn create(table_id:u64) -> Arc<dyn Table> {
        ...
        let table_info = TableInfo {
            desc: "'system'.'in_memory_caches'".to_string(),
            name: "in_memory_caches".to_string(),
