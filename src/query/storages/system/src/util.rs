...
///
/// Logic:
/// 1. Check if database is visible through grants
/// 2. If not, check database ownership - if user owns db, all tables visible
/// 3. For each table, check grants first, then ownership if needed
async fn filter_tables_with_optimized_path(
    ...
    // Check if database is visible through grants
    let db_visible_by_grants =
        grants_checker.check_database_visibility(CATALOG_DEFAULT, db_name, db_id);

    if db_visible_by_grants {
        // Database visible by grants - check each table's grants and ownership
        ...
        // Batch check ownership for tables not visible through grants
        ...
        // Database not visible by grants - check database ownership
        ...
        if let Some(db_owner_info) = db_ownership {
            if is_role_owner(Some(&db_owner_info.role), effective_roles) {
                // User owns database - all tables visible
                return Ok(tables);
            }
        }

        // User doesn't own database - check each table's ownership
        let ownership_objects: Vec<OwnershipObject> = tables
            .iter()
            .map(|t| OwnershipObject::Table {
                catalog_name: catalog_name.to_string(),
                db_id,
