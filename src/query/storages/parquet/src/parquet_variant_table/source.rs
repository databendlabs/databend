...
use crate::parquet_variant_table::parquet_to_variant::batch_to_block;
use crate::parquet_variant_table::parquet_to_variant::read_record_batch;
...
enum State {
    ...
    ReadRowGroup {
        readers: VecDeque<(ParquetRecordBatchReader, u64)>,
        location: String,
    },
        ...
        match std::mem::replace(&mut self.state, State::Init) {
            State::ReadRowGroup {
                ...
                if let Some((reader, mut start_row)) = vs.front_mut() {
                    if let Some(batch) = reader.next() {
                        let mut block = batch_to_block(batch?)?;
            ...
            State::ReadFiles(buffers) => {
                ...
                for (buffer, path) in buffers {
                    let mut block = read_small_file(buffer, self.batch_size)?;
...
impl ParquetVariantSource {
    async fn get_row_group_readers(
    ...
    ) -> Result<VecDeque<(ParquetRecordBatchReader, u64)>> {
        ...
        for (rowgroup_idx, rg) in meta.row_groups().iter().enumerate() {
            ...
            readers.push_back((reader, start_row));
        }
        Ok(readers)
    }
}
