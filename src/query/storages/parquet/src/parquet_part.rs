...
pub enum ParquetPart {
    Parquet2SmallGroup(Parquet2SmallGroupPart),
...
impl ParquetPart {
    pub fn uncompressed_size(&self) -> u64 {
        match self {
            ParquetPart::Parquet2SmallGroup(r) => r.uncompressed_size(),
    ...
    pub fn compressed_size(&self) -> u64 {
        match self {
            ParquetPart::Parquet2SmallGroup(r) => r.compressed_size(),
...
impl PartInfo for ParquetPart {
    ...
    fn hash(&self) -> u64 {
        let path = match self {
            ParquetPart::Parquet2SmallGroup(g) => g.groups.keys().next().unwrap(),
