...
pub trait AggregatingIndexHandler: Sync + Send {
    fn do_query_rewrite(&self, s_expr: &SExpr, indexes: &[SExpr]) -> Result<Option<SExpr>>;
}
...
impl AggregatingIndexHandlerWrapper {
    ...
    pub fn do_query_rewrite(&self, s_expr: &SExpr, indexes: &[SExpr]) -> Result<Option<SExpr>> {
        self.handler.do_query_rewrite(s_expr, indexes)
    }
}
