...
impl<T: SchedulerTask> SchedulerCore<T> {
    ...
    fn enqueue(&self, task: T) {
        let catalog = task.catalog().to_string();
...
mod tests {
    ...
    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    async fn test_same_table_task_is_not_dropped_while_pending() {
        let first_started = Arc::new(Notify::new());
        let release_first = Arc::new(Notify::new());
        ...
        let runner: SchedulerRunner<TestTask> = {
            ...
            Arc::new(move |_task| {
                ...
                Box::pin(async move {
                    let started_index = started.fetch_add(1, Ordering::SeqCst) + 1;
                    if started_index == 1 {
                        first_started.notify_one();
                        release_first.notified().await;
                    }
        ...
        assert_eq!(
            ...
            1,
            "same-table task should wait for the concurrency permit"
        );
    ...
    async fn test_queue_accepts_tasks_beyond_concurrency_limit() {
        ...
        release_first.notify_one();
        wait_until(|| completed.load(Ordering::SeqCst) == 32).await;
        assert_eq!(
            max_running.load(Ordering::SeqCst),
            1,
