...
impl SchedulerTask for TableHookTask {
    fn key(&self) -> TableHookTaskKey {
        TableHookTaskKey {
            ...
            database: self.compact_target.database.clone(),
    ...
    fn merge(&mut self, mut other: Self) {
        ...
        let compaction_num_block_hint = std::cmp::max(
            self.ctx
...
impl TableHookScheduler {
    ...
    async fn execute_task(task: TableHookTask) {
        ...
        if !Self::table_id_matches_target(&task).await {
            return;
        }
    ...
    async fn table_id_matches_target(task: &TableHookTask) -> bool {
        match task
            .ctx
            .get_table(
                &task.compact_target.catalog,
                &task.compact_target.database,
                &task.compact_target.table,
            )
            .await
        {
            Ok(table) if table.get_id() == task.table_id => true,
            Ok(table) => {
                warn!(
                    "Skip async table hook because table id mismatches target, expected_table_id={}, actual_table_id={}, catalog={}, database={}, table={}",
                    task.table_id,
                    table.get_id(),
                    task.compact_target.catalog,
                    task.compact_target.database,
            ...
            Err(e) => {
                warn!(
                    "Skip async table hook because target table cannot be resolved, table_id={}, catalog={}, database={}, table={}, error={}",
                    task.table_id,
                    task.compact_target.catalog,
                    task.compact_target.database,
                    task.compact_target.table,
...
impl<T: SchedulerTask> SchedulerCore<T> {
    ...
    async fn dispatch(receiver: Receiver<TableHookTaskKey>, scheduler: Arc<Self>) {
        while let Ok(key) = receiver.recv().await {
            let permit = match scheduler.semaphore.clone().acquire_owned().await {
                ...
                Err(_) => {
                    warn!(
                        "Skip async table hook because scheduler semaphore is closed, table_id={}, catalog={}, database={}",
                        key.table_id, key.catalog, key.database,
                    );
                    continue;
                }
            };
    ...
    fn try_schedule_key(&self, key: TableHookTaskKey) {
        ...
        warn!(
            "Skip async table hook because scheduler is closed, table_id={}, catalog={}, database={}",
            key.table_id, key.catalog, key.database,
        );
    }
...
mod tests {
    ...
    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    async fn test_table_hook_task_merge_preserves_compaction_hint() -> Result<()> {
        ...
        assert_eq!(task.ctx.get_compaction_num_block_hint("t"), 7);
        Ok(())
    }
}
