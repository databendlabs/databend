...
impl Interpreter for AddTableColumnInterpreter {
    ...
    async fn execute2(&self) -> Result<PipelineBuildResult> {
        ...
        commit_table_meta(&self.ctx, tbl.as_ref(), new_schema, new_table_meta, catalog).await?;
...
pub(crate) async fn commit_table_meta(
    ctx: &QueryContext,
    tbl: &dyn Table,
    new_schema: Arc<TableSchema>,
    mut new_table_meta: TableMeta,
    catalog: Arc<dyn Catalog>,
) -> Result<()> {
    if let Ok(fuse_tbl) = FuseTable::try_from_table(tbl) {
        let mut new_snapshot_location = generate_new_snapshot(ctx, fuse_tbl, &new_schema).await?;
