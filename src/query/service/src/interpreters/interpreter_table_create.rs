...
impl CreateTableInterpreter {
    ...
    async fn create_table_clone(&self) -> Result<PipelineBuildResult> {
        ...
        let anchor_gc_safe_time = if let Some(snapshot) = source_snapshot {
            ...
            let anchor_gc_safe_time = anchor.timestamp;
        ...
        let lvt_check = if anchor_gc_safe_time.is_some() {
            FuseTable::build_table_lvt_check(target.get_table_info(), anchor_gc_safe_time)?
        } else {
            None
        };
