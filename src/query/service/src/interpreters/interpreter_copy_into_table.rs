...
use databend_common_sql::plans::AppendSource;
...
impl CopyIntoTableInterpreter {
    ...
    fn get_copy_into_table_result(
        &self,
        stage_table_info: &StageTableInfo,
    ) -> Result<Vec<DataBlock>> {
        let return_all = !stage_table_info.stage_info.copy_options.return_failed_only;
...
impl Interpreter for CopyIntoTableInterpreter {
    ...
    async fn execute2(&self) -> Result<PipelineBuildResult> {
        ...
        // Build commit insertion pipeline.
        // {
        //     let files_to_copy = self.plan.source.files_to_copy();

        //     let duplicated_files_detected = self.plan.source.duplicated_files_detected();
        ...
        // Execute hook.
        {
            let hook_operator = HookOperator::create(
                self.ctx.clone(),
                self.plan.catalog_name.to_string(),
    ...
    fn inject_result(&self) -> Result<SendableDataBlockStream> {
        // let blocks = match &self.plan.source {
        //     AppendSource::Stage(stage) => self.get_copy_into_table_result(stage)?,
        //     _ => vec![DataBlock::empty_with_schema(self.plan.schema())],
        // };
        todo!()
