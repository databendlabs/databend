// Copyright 2021 Datafuse Labs
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

use std::sync::Arc;

use databend_common_catalog::table::Table;
use databend_common_exception::ErrorCode;
use databend_common_exception::Result;
use databend_common_sql::Planner;
use databend_common_sql::plans::Plan;
use databend_storages_common_table_meta::table::OPT_KEY_MATERIALIZED_VIEW;
use databend_storages_common_table_meta::table::OPT_KEY_MATERIALIZED_VIEW_QUERY;
use databend_storages_common_table_meta::table::OPT_KEY_MATERIALIZED_VIEW_QUERY_CATALOG;
use databend_storages_common_table_meta::table::OPT_KEY_MATERIALIZED_VIEW_QUERY_DATABASE;

use crate::sessions::QueryContext;
use crate::sessions::TableContextTableAccess;

pub fn check_not_materialized_view(table: &dyn Table, db_name: &str) -> Result<()> {
    if table.options().contains_key(OPT_KEY_MATERIALIZED_VIEW) {
        return Err(ErrorCode::TableEngineNotSupported(format!(
            "Cannot alter materialized view `{}`.`{}`",
            db_name,
            table.name()
        )));
    }
    Ok(())
}

struct ContextRestoreGuard<'a> {
    ctx: &'a Arc<QueryContext>,
    catalog: Option<String>,
    database: Option<String>,
}

impl Drop for ContextRestoreGuard<'_> {
    fn drop(&mut self) {
        if let Some(catalog) = self.catalog.take() {
            self.ctx.set_current_catalog_unchecked(catalog);
        }
        if let Some(database) = self.database.take() {
            self.ctx.set_current_database_unchecked(database);
        }
    }
}

/// Plan the MV's stored query in its original catalog/database context.
///
/// The Planner resolves unqualified identifiers (UDFs, sequences, etc.) against
/// the session's current catalog and database. Since REFRESH may run in a
/// different session context than the one the MV was created in, we temporarily
/// switch to the MV's recorded catalog/database, plan the SQL, then restore.
pub async fn plan_sql_in_mv_context(
    ctx: &Arc<QueryContext>,
    query_catalog: String,
    query_database: String,
    query_text: &str,
) -> Result<Plan> {
    let _guard = ContextRestoreGuard {
        ctx,
        catalog: Some(ctx.get_current_catalog()),
        database: Some(ctx.get_current_database()),
    };

    ctx.set_current_catalog(query_catalog).await?;
    ctx.set_current_database(query_database).await?;

    let mut planner = Planner::new(ctx.clone());
    let (plan, _) = planner.plan_sql(query_text).await?;
    Ok(plan)
}

pub async fn plan_materialized_view_query(
    ctx: &Arc<QueryContext>,
    catalog_name: &str,
    db_name: &str,
    view_name: &str,
) -> Result<Plan> {
    let table = ctx.get_table(catalog_name, db_name, view_name).await?;
    let options = table.options();
    if !options.contains_key(OPT_KEY_MATERIALIZED_VIEW) {
        return Err(ErrorCode::TableEngineNotSupported(format!(
            "`{}`.`{}` is not a MATERIALIZED VIEW",
            db_name, view_name
        )));
    }

    let query_text = options
        .get(OPT_KEY_MATERIALIZED_VIEW_QUERY)
        .ok_or_else(|| {
            ErrorCode::Internal(format!(
                "materialized view `{}`.`{}` is missing refresh query definition",
                db_name, view_name
            ))
        })?
        .clone();
    let query_catalog = options
        .get(OPT_KEY_MATERIALIZED_VIEW_QUERY_CATALOG)
        .cloned()
        .unwrap_or_else(|| catalog_name.to_string());
    let query_database = options
        .get(OPT_KEY_MATERIALIZED_VIEW_QUERY_DATABASE)
        .cloned()
        .unwrap_or_else(|| db_name.to_string());

    plan_sql_in_mv_context(ctx, query_catalog, query_database, &query_text).await
}
