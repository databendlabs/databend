...
pub static CREATE_MATERIALIZED_VIEW_OPTIONS: LazyLock<HashSet<&'static str>> =
    LazyLock::new(|| {
        ...
        // option. TEMP_PREFIX routes CREATE TABLE to the session's temporary-table manager and is
        // never valid for a persistent materialized view. Other internal keys remain in the set
        // because the binder adds them after rejecting user-specified reserved keys.
        let mut r = CREATE_FUSE_OPTIONS.clone();
        r.remove(OPT_KEY_COMMENT);
        r.remove(OPT_KEY_TEMP_PREFIX);
