...
use databend_common_sql::plans::RenameWorkloadGroupPlan;
...
pub struct RenameWorkloadGroupInterpreter {
    ...
    plan: RenameWorkloadGroupPlan,
}

impl RenameWorkloadGroupInterpreter {
    pub fn try_create(ctx: Arc<QueryContext>, plan: RenameWorkloadGroupPlan) -> Result<Self> {
        Ok(RenameWorkloadGroupInterpreter { ctx, plan })
    }
}

#[async_trait::async_trait]
impl Interpreter for RenameWorkloadGroupInterpreter {
    fn name(&self) -> &str {
        "RenameWorkloadGroupInterpreter"
    }
