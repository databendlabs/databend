...
impl AccessChecker for PrivilegeAccess {
    ...
    async fn check(&self, ctx: &Arc<QueryContext>, plan: &Plan) -> Result<()> {
        ...
        match plan {
                ...
                match rewrite_kind {
                    ...
                    Some(RewriteKind::ShowTables(catalog, database)) => {
                        ...
                        if self
                            .has_ownership(
                                &session,
                                &GrantObject::Database(catalog.clone(), database.clone()),
                            )
                            .await?
                        {
                            return Ok(());
                        ...
                        let (db_id, table_id) = match self
                            .convert_to_id(&tenant, &catalog, database, None)
                            .await?
                        {
                            ObjectId::Table(db_id, table_id) => (db_id, Some(table_id)),
                            ObjectId::Database(db_id) => (db_id, None),
                        ...
                        let has_priv =
                            has_priv(&tenant, database, None, db_id, table_id, grant_set).await?;
                        return if has_priv {
                            Ok(())
                        } else {
                    ...
                    Some(RewriteKind::ShowStreams(database)) => {
                        ...
                        if self
                            .has_ownership(
                                &session,
                                &GrantObject::Database(catalog_name.clone(), database.clone()),
                            )
                            .await?
                        {
                            return Ok(());
                    ...
                    Some(RewriteKind::ShowColumns(catalog_name, database, table)) => {
                        ...
                        let catalog = self.ctx.get_catalog(catalog_name).await?;
                        let (db_id, table_id) = match self
                            .convert_to_id(&tenant, &catalog, database, Some(table))
                            .await?
                        {
                            ObjectId::Table(db_id, table_id) => (db_id, Some(table_id)),
                            ObjectId::Database(db_id) => (db_id, None),
                        ...
                        let has_priv =
                            has_priv(&tenant, database, Some(table), db_id, table_id, grant_set)
                                .await?;
                        return if has_priv {
                            Ok(())
                        } else {
                ...
                for table in metadata.tables() {
                    if enable_experimental_rbac_check && table.is_source_of_stage() {
                        match table.table().get_data_source_info() {
                            ...
                            | DataSourceInfo::ResultScanSource(_) => {}
                        }
                    }
            ...
            // Task
            // Is it compatible with the permission check of the old version?
            Plan::CreateTask(_) => {
                if self
                    .validate_access(&GrantObject::Global, UserPrivilegeType::CreateTask)
                    .await
                    .is_err()
                {
            ...
            Plan::UndropDatabase(_) | Plan::DropIndex(_) | Plan::DropTableIndex(_) => {
            ...
            Plan::DropStage(plan) => {
                match UserApiProvider::instance()
                    .get_stage(&tenant, &plan.name)
                    .await
                {
                    Ok(stage) => {
                        if enable_experimental_rbac_check {
                            let privileges =
                                vec![UserPrivilegeType::Read, UserPrivilegeType::Write];
            ...
            Plan::UseDatabase(plan) => {
                ...
                if self
                    .has_ownership(
                        &session,
                        &GrantObject::Database(catalog_name.clone(), plan.database.clone()),
                    )
                    .await?
                {
                    return Ok(());
                ...
                let (db_id, _) = match self
                    .convert_to_id(&tenant, &catalog, &plan.database, None)
                    .await?
                {
                    ObjectId::Table(db_id, table_id) => (db_id, Some(table_id)),
                    ObjectId::Database(db_id) => (db_id, None),
                ...
                let has_priv =
                    has_priv(&tenant, &plan.database, None, db_id, None, grant_set).await?;
            ...
            Plan::InsertMultiTable(plan) => {
                ...
                for target in plan.whens.iter().flat_map(|when| when.intos.iter()).chain(
                    plan.opt_else
                        .as_ref()
                        .into_iter()
                        .flat_map(|e| e.intos.iter()),
                ) {
            ...
            Plan::CreateUser(_) => {
                self.validate_access(&GrantObject::Global, UserPrivilegeType::CreateUser)
                    .await?;
            }
            Plan::DropUser(_) => {
                self.validate_access(&GrantObject::Global, UserPrivilegeType::DropUser)
                    .await?;
            }
            Plan::CreateRole(_) => {
                self.validate_access(&GrantObject::Global, UserPrivilegeType::CreateRole)
                    .await?;
            }
            Plan::DropRole(_) => {
                self.validate_access(&GrantObject::Global, UserPrivilegeType::DropRole)
                    .await?;
            }
            ...
            Plan::CopyIntoLocation(plan) => {
                self.validate_stage_access(&plan.stage, UserPrivilegeType::Write)
                    .await?;
            ...
            Plan::CreateDatamaskPolicy(_) | Plan::DropDatamaskPolicy(_) => {
                self.validate_access(&GrantObject::Global, UserPrivilegeType::CreateDataMask)
                    .await?;
            }
