...
impl ModifyTableColumnInterpreter {
    ...
    async fn do_set_data_type(
        ...
        let mut modified_cols = HashMap::with_capacity(field_and_comments.len());
        ...
        for (field, _comment) in field_and_comments {
            if let Some((i, old_field)) = schema.column_with_name(&field.name) {
                ...
                if old_field.data_type != field.data_type {
                    modified_cols.insert(field.name.clone(), &field.data_type);
        ...
        if !modified_cols.is_empty() {
            ...
            if let Some((_, cluster_key)) = table.cluster_key_meta() {
                ...
                for refer_col in referenced {
                    let Some(&typ) = modified_cols.get(&refer_col) else {
                        continue;
                    };
                    let data_type = typ.into();
                    if !Binder::valid_cluster_key_type(&data_type) {
                        return Err(ErrorCode::AlterTableError(format!(
                            "Unsupported data type '{}' for column '{}', because it is referenced by cluster key '{}'",
                            data_type, refer_col, cluster_key,
                        )));
                    }
                }
            }
