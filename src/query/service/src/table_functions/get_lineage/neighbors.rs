    ...
    fn parse(table_args: &TableArgs) -> Result<Self> {
        ...
        if args.len() != 2 {
            return Err(ErrorCode::BadArguments(format!(
                "{GET_LINEAGE_NEIGHBORS_FUNC} requires 2 positioned arguments: object_domain, object_name"
            )));
        }
        let object_domain = ObjectDomain::parse(&string_value(&args[0])?)?;
        Ok(Self {
            object_domain,
            object_name: string_value(&args[1])?,
        })
    }
}
    ...
    async fn collect_block(&mut self, block: &DataBlock) -> Result<()> {
        ...
        for direction in ["UPSTREAM", "DOWNSTREAM"] {
            ...
            for edge in edges {
                ...
                if start.column_name.as_ref().is_some_and(|column_name| {
                    !build_column_neighbors(
                        direction,
                        &edge,
                        &start.object,
                        &neighbor,
...
mod tests {
    ...
    #[test]
    fn test_get_lineage_neighbors_args() -> Result<()> {
        let args = TableArgs::new_positioned(vec![
            Scalar::String("TABLE".to_string()),
            Scalar::String("default.db.t".to_string()),
        ]);
        let parsed = GetLineageNeighborsArgs::parse(&args)?;
        assert_eq!(parsed.object_domain, ObjectDomain::Table);
        assert_eq!(parsed.object_name, "default.db.t");
        Ok(())
    }

    #[test]
    fn test_get_lineage_neighbors_schema() {
        let schema = GetLineageNeighborsTable::schema();
        let names = schema
            .fields()
            .iter()
    ...
    #[test]
    fn test_get_lineage_neighbors_accepts_column_domain() -> Result<()> {
        let args = TableArgs::new_positioned(vec![
            Scalar::String("COLUMN".to_string()),
            Scalar::String("default.db.t.a".to_string()),
        ]);
        let parsed = GetLineageNeighborsArgs::parse(&args)?;
        assert_eq!(parsed.object_domain, ObjectDomain::Column);
        assert_eq!(parsed.object_name, "default.db.t.a");
        Ok(())
    }

    #[test]
    fn test_get_lineage_neighbors_supported_domains_and_arity() -> Result<()> {
        for domain in ["TABLE", "VIEW", "STAGE", "COLUMN"] {
            let args = TableArgs::new_positioned(vec![
                Scalar::String(domain.to_string()),
                Scalar::String("object".to_string()),
            ]);
            assert_eq!(
                GetLineageNeighborsArgs::parse(&args)?.object_domain,
                ObjectDomain::parse(domain)?
            );
        }

        let one_arg = TableArgs::new_positioned(vec![Scalar::String("TABLE".to_string())]);
        let three_args = TableArgs::new_positioned(vec![
            Scalar::String("TABLE".to_string()),
            Scalar::String("db.t".to_string()),
            Scalar::String("extra".to_string()),
        ]);
        assert!(GetLineageNeighborsArgs::parse(&one_arg).is_err());
        assert!(GetLineageNeighborsArgs::parse(&three_args).is_err());
        Ok(())
    }
}
