...
async fn collect_tag_references(
    ...
    let (taggable_object, obj_db, obj_id, obj_name) = match domain_upper.as_str() {
        ...
        "UDF" => {
            ...
            let tokens = databend_common_ast::parser::tokenize_sql(trimmed).map_err(|e| {
                ErrorCode::BadArguments(format!("Invalid UDF name '{}': {}", object_name, e))
            })?;
            let idents =
                databend_common_ast::parser::parse_comma_separated_idents(&tokens, dialect)
                    .map_err(|e| {
            ...
            let udf_ident = if idents.len() == 1 {
                idents[0].clone()
            } else {
                return Err(ErrorCode::BadArguments(
                    "UDF name must be a single identifier",
                ));
            };
