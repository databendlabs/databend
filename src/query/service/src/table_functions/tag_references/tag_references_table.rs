...
async fn collect_tag_references(
    ...
    let object_name = args[0]
        .clone()
        .into_string()
        .map_err(|_| ErrorCode::BadArguments("object_name must be a string"))?;
    let object_domain = args[1]
        .clone()
        .into_string()
        .map_err(|_| ErrorCode::BadArguments("object_domain must be a string"))?;
    ...
    let (taggable_object, obj_db, obj_id, obj_name) = match domain_upper.as_str() {
        ...
        "TABLE" => {
            let (db_name, table_name) = parse_table_name(&ctx, &object_name)?;
    ...
    for (tag, name_opt) in tags.iter().zip(tag_names_result.into_iter()) {
        let name = name_opt
            .map(|seqv| seqv.data.tag_name().to_string())
...
/// Parse table name in format "db.table" or "db.schema.table"
/// Returns (database, table) - schema is ignored as Databend doesn't use it
fn parse_table_name(ctx: &Arc<dyn TableContext>, name: &str) -> Result<(String, String)> {
    ...
    match parts.len() {
        1 => {
            // Just table name, use current database
            let db = ctx.get_current_database();
            Ok((db, parts[0].to_string()))
        }
        2 => {
            // db.table
        ...
        3 => {
            // db.schema.table - ignore schema, Databend doesn't use it
            Ok((parts[0].to_string(), parts[2].to_string()))
        }
        _ => Err(ErrorCode::BadArguments(format!(
            "Invalid table name '{}'. Expected format: table, db.table, or db.schema.table",
            name
        ))),
    }
}
