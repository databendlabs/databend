...
/// Returns (database, table) - catalog is ignored (uses default catalog)
fn parse_table_name(ctx: &Arc<dyn TableContext>, name: &str) -> Result<(String, String)> {
    ...
    match parts.len() {
        ...
        3 => {
            // catalog.db.table - ignore catalog, use default catalog
            Ok((parts[1].to_string(), parts[2].to_string()))
        }
        _ => Err(ErrorCode::BadArguments(format!(
            "Invalid table name '{}'. Expected format: table, db.table, or catalog.db.table",
            name
        ))),
    }
}
