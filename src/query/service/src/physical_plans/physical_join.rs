...
fn find_pushdown_safe_scan_id(s_expr: &SExpr, table_index: usize) -> Option<usize> {
    ...
    // Only cross operators through which that filter can be pushed safely.
    match s_expr.plan() {
        RelOperator::Scan(scan) if scan.table_index == table_index => Some(scan.scan_id),
        RelOperator::EvalScalar(_)
        | RelOperator::Exchange(_)
        | RelOperator::Filter(_)
        | RelOperator::Sort(_) => find_pushdown_safe_scan_id(s_expr.child(0).ok()?, table_index),
        _ => None,
    }
}
