...
impl ContextFunction {
    ...
    pub fn build_args_from_ctx(ctx: Arc<QueryContext>, name: &str) -> Result<Vec<Scalar>> {
        ...
        Ok(match name.to_lowercase().as_str() {
            "database" | "currentdatabase" | "current_database" => {
                vec![Scalar::String(ctx.get_current_database().into_bytes())]
            }
            "version" => vec![Scalar::String(ctx.get_fuse_version().into_bytes())],
            "user" | "currentuser" | "current_user" => vec![Scalar::String(
                ctx.get_current_user()?.identity().to_string().into_bytes(),
            )],
            "current_role" => vec![Scalar::String(
                ctx.get_current_role()
                    .map(|r| r.name)
                    .unwrap_or_else(|| "".to_string())
                    .into_bytes(),
            )],
            "connection_id" => vec![Scalar::String(ctx.get_connection_id().into_bytes())],
            "timezone" => vec![Scalar::String(
