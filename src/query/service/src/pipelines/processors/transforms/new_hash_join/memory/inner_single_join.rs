...
pub struct InnerSingleHashJoin {
...
impl Join for InnerSingleHashJoin {
    ...
    fn probe_block(&mut self, data: DataBlock) -> Result<Box<dyn JoinStream + '_>> {
        ...
        Ok(InnerSingleHashJoinStream::create(
            probe_block,
...
struct InnerSingleHashJoinStream<'a> {
    desc: Arc<HashJoinDesc>,
...
unsafe impl<'a> Send for InnerSingleHashJoinStream<'a> {}
unsafe impl<'a> Sync for InnerSingleHashJoinStream<'a> {}

impl<'a> InnerSingleHashJoinStream<'a> {
    pub fn create(
        ...
        Box::new(InnerSingleHashJoinStream {
            desc,
            join_state,
            probed_rows,
            filter_executor,
    ...
    fn check_left_single_match(counts: &mut [usize], matched_probe: u64) -> Result<()> {
        let probe_idx = matched_probe as usize;
        assume(probe_idx < counts.len());
        counts[probe_idx] += 1;
        if counts[probe_idx] > 1 {
    ...
    fn check_right_single_match(
        single_to_inner: Option<JoinType>,
        join_state: &BasicHashJoinState,
        matched_build: RowPtr,
    ) -> Result<()> {
        if matches!(single_to_inner, Some(JoinType::RightSingle)) {
            mark_right_single_row(join_state, matched_build)?;
        }
        Ok(())
    }
}

impl<'a> JoinStream for InnerSingleHashJoinStream<'a> {
    fn next(&mut self) -> Result<Option<DataBlock>> {
        let Some(probe_data_block) = self.probe_data_block.take() else {
            return Ok(None);
        };

        let mut counts = vec![0_usize; probe_data_block.num_rows()];
        let mut output_probe = Vec::new();
        let mut output_build = Vec::new();
        ...
        loop {
            ...
            if matches!(single_to_inner, Some(JoinType::LeftSingle)) {
                for probe_idx in &self.probed_rows.matched_probe {
                    Self::check_left_single_match(&mut counts, *probe_idx)?;
                }
            }
            ...
            match self.filter_executor.as_mut() {
                None => {
                    ...
                    {
                        Self::check_right_single_match(single_to_inner, &join_state, *row_ptr)?;
                        output_probe.push(*probe_idx);
                        output_build.push(*row_ptr);
                    }
                }
                Some(filter_executor) => {
                    ...
                    for idx in filter_executor.true_selection().iter().take(result_count) {
                        ...
                        let probe_idx = self.probed_rows.matched_probe[idx];
