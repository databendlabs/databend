...
pub trait JoinHashTable: Send + Sync + 'static {
    fn insert(&self, keys: ProjectedBlock, chunk: usize, arena: &mut Vec<u8>) -> Result<()>;
}
...
impl<T: HashtableKeyable + FixedKey + Send + Sync + 'static> JoinHashTable
    for FixedKeyHashJoinHashTable<T>
{
    fn insert(&self, keys: ProjectedBlock, chunk: usize, arena: &mut Vec<u8>) -> Result<()> {
        let num_rows = keys.num_rows();
        let keys_state = self.hash_method.build_keys_state(keys, num_rows)?;
