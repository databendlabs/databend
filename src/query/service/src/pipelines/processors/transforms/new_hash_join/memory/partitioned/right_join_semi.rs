...
impl Join for PartitionedRightSemiJoin {
        ...
        if let Some(filter) = self.filter_executor.as_mut() {
            ...
            if !matched_probe.is_empty() {
                ...
                if !result.is_empty() {
                    ...
                    if count > 0 {
                        // probe_and_mark_visited already marked all matches as visited,
                        // which is correct for semi join (any match suffices)
                    }
                }
            }
        } else {
            // Without filter, probe_and_mark_visited already marks all matched build rows
            let _ = self.build.probe_and_mark_visited(&data)?;
        }

        // Right semi doesn't output during probe phase
    ...
    fn final_probe(&mut self) -> Result<Option<Box<dyn JoinStream + '_>>> {
        ...
        let mut visited_ptrs = Vec::new();
        for i in 1..=self.build.num_rows {
            if self.build.hash_table.is_visited(i) {
                visited_ptrs.push(flat_to_row_ptr(i));
            }
        }

        if visited_ptrs.is_empty() {
            return Ok(None);
        }

        let build_block = self.build.gather_build_block(&visited_ptrs);
        match build_block {
            Some(block) => Ok(Some(Box::new(OneBlockJoinStream(Some(block))))),
            None => Ok(Some(Box::new(OneBlockJoinStream(Some(
                DataBlock::new(vec![], visited_ptrs.len()),
            ))))),
        }
    }
}
