// Copyright 2021 Datafuse Labs
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

use std::sync::Arc;
use std::sync::atomic::AtomicU64;

use databend_common_exception::Result;
use databend_common_pipeline::core::InputPort;
use databend_common_pipeline::core::OutputPort;
use databend_common_pipeline::core::Pipeline;
use databend_common_pipeline::core::ProcessorPtr;
use databend_common_pipeline::core::TransformPipeBuilder;

use crate::physical_plans::AggregateShuffleMode;
use crate::pipelines::processors::transforms::aggregator::AggregateBucketPartitionStream;
use crate::pipelines::processors::transforms::aggregator::AggregateRowPartitionStream;
use crate::pipelines::processors::transforms::aggregator::AggregatorParams;
use crate::pipelines::processors::transforms::aggregator::TransformFinalAggregate;
use crate::servers::flight::v1::exchange::ExchangeTransform;
use crate::servers::flight::v1::partition::PartitionStream;
use crate::sessions::QueryContext;
use crate::sessions::TableContextSettings;

pub fn build_partition_bucket(
    pipeline: &mut Pipeline,
    params: Arc<AggregatorParams>,
    after_worker: usize,
    ctx: Arc<QueryContext>,
    shuffle_mode: AggregateShuffleMode,
    input_partitioned: bool,
) -> Result<()> {
    let base_consumed_bits = shuffle_mode.determine_radix_bits();
    let final_parallelism = if input_partitioned {
        pipeline.output_len()
    } else {
        match shuffle_mode {
            AggregateShuffleMode::Row(_) => ctx.get_settings().get_max_threads()? as usize,
            AggregateShuffleMode::Bucket(hint) => hint as usize,
        }
    };

    if !input_partitioned {
        let partition_streams = (0..final_parallelism)
            .map(|_| match shuffle_mode {
                AggregateShuffleMode::Row(_) => Box::new(AggregateRowPartitionStream {
                    buckets: final_parallelism,
                    aggregate_params: params.clone(),
                }) as Box<dyn PartitionStream>,
                AggregateShuffleMode::Bucket(_) => Box::new(AggregateBucketPartitionStream {
                    buckets: final_parallelism,
                }) as Box<dyn PartitionStream>,
            })
            .collect();
        ExchangeTransform::local_shuffle(
            pipeline,
            &params.spill_schema(),
            partition_streams,
            final_parallelism,
        )?;
    }

    debug_assert_eq!(pipeline.output_len(), final_parallelism);

    let mut builder = TransformPipeBuilder::create();
    let (tx, rx) = async_channel::unbounded();
    let next_task_id = Arc::new(AtomicU64::new(1));
    for id in 0..final_parallelism {
        let input_port = InputPort::create();
        let output_port = OutputPort::create();
        let processor = TransformFinalAggregate::try_create(
            input_port.clone(),
            output_port.clone(),
            params.clone(),
            id,
            base_consumed_bits,
            ctx.clone(),
            tx.clone(),
            rx.clone(),
            next_task_id.clone(),
        )?;
        builder.add_transform(input_port, output_port, ProcessorPtr::create(processor));
    }

    pipeline.add_pipe(builder.finalize());

    pipeline.resize(after_worker, true)?;

    Ok(())
}
