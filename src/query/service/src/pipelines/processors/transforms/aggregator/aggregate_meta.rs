...
pub enum AggregateMeta<Method: HashMethodBounds, V: Send + Sync + 'static> {
    ...
    PartitionedHashTable(HashTablePayload<
...
impl<Method: HashMethodBounds, V: Send + Sync + 'static> AggregateMeta<Method, V> {
    pub fn create_hashtable(v: Method::HashTable<V>, arena_holder: ArenaHolder) -> BlockMetaInfoPtr {
    ...
    pub fn create_partitioned_hashtable(
        ...
        Box::new(AggregateMeta::<Method, V>::PartitionedHashTable(HashTablePayload {
            arena_holder,
            inner: hashtable,
        }))
    }
