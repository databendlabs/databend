...
impl HashJoinProbeState {
    ...
    fn extend_right_data_block(
        ...
        right_single_scan_map: &mut Vec<*mut AtomicBool>,
    ) -> Result<()> {
        ...
        let probe_block = if probe_state.is_probe_projected {
            ...
            let nullable_columns = probe_block
                .columns()
                .iter()
                .map(|c| wrap_true_validity(c, max_block_size, &probe_state.true_validity))
                .collect::<Vec<_>>();
        ...
        let result_block = self.merge_eq_block(probe_block, build_block, max_block_size);
        ...
        H::Key: 'a,
    {
        ...
        let right_single_scan_map = &mut build_state.right_single_scan_map;
