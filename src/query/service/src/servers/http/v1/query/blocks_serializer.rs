...
fn format_geometry_column(
    ...
    format_geo_binary_column(
        ...
        column.iter(),
        |value| format_geometry_value(value, geometry_format),
    )
}
...
fn format_geography_column(
    ...
    Ok(GeographyColumn(format_geo_binary_column(
        ...
        column.iter(),
        |value| format_geography_value(value, geometry_format),
    )?))
}
...
fn format_geometry_value(value: &[u8], geometry_format: GeometryDataType) -> Result<Vec<u8>> {
    match extract_geo_and_srid(databend_common_expression::ScalarRef::Geometry(value)) {
        Ok(Some((geo, _))) => {
            let srid = read_wkb_header(value).ok().and_then(|info| info.srid);
            format_decoded_geo(geo, srid, geometry_format)
        }
        Ok(None) => unreachable!("expected geometry scalar"),
        Err(_) => Ok(lossy_geo_value(value, geometry_format)),
    }
}

fn format_geography_value(
    value: GeographyRef<'_>,
    geometry_format: GeometryDataType,
) -> Result<Vec<u8>> {
    match extract_geo_and_srid(databend_common_expression::ScalarRef::Geography(value)) {
        Ok(Some((geo, _))) => format_decoded_geo(geo, Some(GEOGRAPHY_SRID), geometry_format),
        Ok(None) => unreachable!("expected geography scalar"),
        Err(_) => Ok(lossy_geo_value(value.0, geometry_format)),
    }
}
