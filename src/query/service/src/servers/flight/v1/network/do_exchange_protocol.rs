...
mod tests {
    ...
    #[test]
    fn test_sequence_and_fail_roundtrip() {
        let request =
            DoExchangeRequest::decode(DoExchangeRequest::data(42, FlightData::default()).encode())
                .unwrap();
        ...
        let response = DoExchangeResponse::decode(DoExchangeResponse::ack(42).encode()).unwrap();
        assert!(matches!(response, DoExchangeResponse::Ack { sequence: 42 }));

        let response = DoExchangeResponse::decode(
            DoExchangeResponse::fail(ErrorCode::AbortedQuery("receiver failed")).encode(),
        )
        .unwrap();
        let DoExchangeResponse::Fail(cause) = response else {
            panic!("expected Fail response");
        };
        assert_eq!(cause.code(), ErrorCode::ABORTED_QUERY);
    }
}
