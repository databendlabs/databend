...
impl BlockOutboundSet {
    pub(crate) fn create_with_producers(
        ...
        let mut started_outbounds = Vec::with_capacity(outbounds.len());
        for outbound in outbounds {
            let outbound = outbound.start(slots.clone(), Some(config.max_batch_bytes), runtime);
            started_outbounds.push(Arc::new(outbound));
        }

        Self {
            outbounds: started_outbounds,
    ...
    pub async fn finish_producer(&self) -> Result<()> {
        ...
        let results =
            futures::future::join_all(self.outbounds.iter().map(|outbound| outbound.finish()))
                .await;
...
impl Drop for BlockOutboundSet {
    fn drop(&mut self) {
