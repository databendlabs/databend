...
pub enum DataPacket {
    ...
    FlightControl(FlightControlCommand),
}
...
impl TryFrom<DataPacket> for FlightData {
    ...
    fn try_from(packet: DataPacket) -> Result<Self> {
        Ok(match packet {
            ...
            DataPacket::FlightControl(command) => FlightData {
                app_metadata: vec![0x09],
                data_body: serde_json::to_vec(&command)?,
                data_header: vec![],
                flight_descriptor: None,
            },
        })
...
impl TryFrom<FlightData> for DataPacket {
    ...
    fn try_from(flight_data: FlightData) -> Result<Self> {
        ...
        match flight_data.app_metadata.last().unwrap() {
            ...
            0x09 => {
                let command =
                    serde_json::from_slice::<FlightControlCommand>(&flight_data.data_body)?;
                Ok(DataPacket::FlightControl(command))
            }
