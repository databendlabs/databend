...
async fn test_fuse_purge_normal_orphan_snapshot() -> anyhow::Result<()> {
    ...
    // create orphan snapshot, its timestamp is larger than the current one
    {
        ...
        let orphan_snapshot_location =
            location_gen.gen_snapshot_location(&orphan_snapshot_id, TableSnapshot::VERSION)?;
...
async fn test_fuse_purge_older_version() -> anyhow::Result<()> {
    ...
    // Do compact segment, generate a new snapshot.
    {
        ...
        check_data_dir(&fixture, "", 4, 0, 5, 7, 7, 5, Some(()), None).await?;
    }
