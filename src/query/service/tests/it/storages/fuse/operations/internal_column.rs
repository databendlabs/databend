...
async fn test_dynamic_block_prune_from_in_subquery() -> anyhow::Result<()> {
    ...
    let false_conjunct = format!("{one_block} AND false");
    assert_eq!(
        0,
        query_count(fixture.execute_query(&false_conjunct).await?).await?
    );

    let null_conjunct = format!("{one_block} AND CAST(NULL AS BOOLEAN)");
    assert_eq!(
        0,
        query_count(fixture.execute_query(&null_conjunct).await?).await?
    );

    let limit_probe = format!(
        "SELECT count(*) FROM (\
           SELECT _block_name FROM {db}.{tbl} ORDER BY _block_name LIMIT 1\
    ...
    assert_eq!(
        0,
        query_count(fixture.execute_query(&limit_probe).await?).await?
    );
    let blocks = fixture
        .execute_query(&format!("EXPLAIN PIPELINE {limit_probe}"))
        .await?
