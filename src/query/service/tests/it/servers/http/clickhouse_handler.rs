...
async fn test_settings() -> PoemResult<()> {
    ...
    // unknown setting
    {
        let sql = "select value from settings where name = 'max_block_size'";
    ...
    {
        let sql = "select value from settings where name = 'max_block_size' or name = 'enable_cbo' order by value";
        let (status, body) = server
            .get_response(
                QueryBuilder::new(sql)
                    .settings(HashMap::from([
