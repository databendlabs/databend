...
async fn test_can_perform_task() -> Result<()> {
    let fixture = TestFixture::setup().await?;
    let ctx = fixture.new_query_ctx().await?;
    let graph = create_simple_pipeline(ctx)?;
    graph.can_perform_task(0, 3);
    Ok(())
}
...
//                 assert_eq!(scheduled.sync_queue[0].processor.id().index(), scheduled_result[acc]);
//                 assert_eq!(
...
fn create_simple_pipeline(ctx: Arc<QueryContext>) -> Result<Arc<RunningGraph>> {
    ...
    RunningGraph::create(pipeline, 0)
}
