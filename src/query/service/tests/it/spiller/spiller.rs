...
use databend_query::spiller::Spiller;
use databend_query::spiller::SpillerConfig;
use databend_query::spiller::SpillerType;
...
async fn test_spill_with_partition() -> Result<()> {
    ...
    let data_blocks = spiller.read_spilled_data(&(0 as u8)).await?;
    for block in data_blocks {
        assert_eq!(block.num_rows(), 100);
        assert_eq!(block.num_columns(), 2);
        for (col_idx, col) in block.columns().into_iter().enumerate() {
            for (idx, cell) in col
                .value
