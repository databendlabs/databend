...
impl ResultCacheState {
    pub fn add_partitions_sha(&self, sha: String) {
        let mut shas = self.partitions_shas.write();
    ...
    pub fn partitions_shas(&self) -> Vec<String> {
        let mut shas = self.partitions_shas.read().clone();
        shas.sort();
        shas
    }

    pub fn add_cache_key_extra(&self, extra: String) {
        let mut extras = self.cache_key_extras.write();
    ...
    pub fn cache_key_extras(&self) -> Vec<String> {
        let mut extras = self.cache_key_extras.read().clone();
        extras.sort();
        extras
    }
