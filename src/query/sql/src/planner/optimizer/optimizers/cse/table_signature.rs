...
impl<'a> Visitor<'a> for DeterministicVisitor {
    ...
    fn visit_window_function(
        &mut self,
        _window: &'a WindowFunc,
    ) -> databend_common_exception::Result<()> {
        self.deterministic = false;
        Ok(())
    }

    fn visit_udf_call(&mut self, _udf: &'a UDFCall) -> databend_common_exception::Result<()> {
        self.deterministic = false;
        Ok(())
    }
    ...
    fn visit_udf_lambda_call(
        &mut self,
        _udf: &'a UDFLambdaCall,
    ) -> databend_common_exception::Result<()> {
        self.deterministic = false;
        Ok(())
    }

    fn visit_udaf_call(&mut self, _udaf: &'a UDAFCall) -> databend_common_exception::Result<()> {
        self.deterministic = false;
        Ok(())
    }

    fn visit_async_function_call(
        &mut self,
        _async_func: &'a AsyncFunctionCall,
    ) -> databend_common_exception::Result<()> {
        self.deterministic = false;
        Ok(())
    }
