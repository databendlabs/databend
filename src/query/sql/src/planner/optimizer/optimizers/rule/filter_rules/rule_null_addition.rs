...
///
/// The derived predicates carry `derived_from = Some(DerivedFrom::NullAddition)`:
/// they restate a fact the join already enforces (NULL-key rows never
/// survive), and therefore must never introduce new evaluation errors when
/// evaluated. Their purpose is to seed downstream rules with null-rejection
/// information, e.g. once pushed into a subquery branch,
/// `outer_join_to_inner_join` can eliminate an outer join whose
/// null-supplying side is constrained by the derived `is_not_null`:
///
/// ```text
...
impl Rule for RuleNullAddition {
    ...
    fn apply(&self, s_expr: &SExpr, state: &mut TransformResult) -> Result<()> {
        ...
        for condition in join.equi_conditions.iter() {
            ...
            if filter_left
                && let Some(predicate) = nullable_key_filter(&condition.left, s_expr.child(0)?)
            {
                left_null_predicates.push(predicate);
            }
            if filter_right
                && let Some(predicate) = nullable_key_filter(&condition.right, s_expr.child(1)?)
            {
...
///
/// Two classes of keys are skipped:
///
/// - Keys whose defining `EvalScalar` item is statically non-null (e.g.
///   `CAST(count(x) AS UInt64 NULL)` — a nullable cast of a non-null value):
///   the derived predicate would be a no-op that only folds to `true` during
///   physical planning, leaving pure overhead.
/// - Keys whose defining expression is nondeterministic (e.g. `rand()`):
///   the materialized value is computed once by the `EvalScalar`, but a
///   derived predicate must never tempt a later rule into re-evaluating the
///   expression (inlining it would produce a *different* value), so no
///   predicate is derived at all.
fn nullable_key_filter(key: &ScalarExpr, child: &SExpr) -> Option<ScalarExpr> {
    let ScalarExpr::BoundColumnRef(column_ref) = key else {
        return None;
    };
    if !column_ref.column.data_type.is_nullable_or_null() {
        return None;
    }
    if let RelOperator::EvalScalar(eval_scalar) = child.plan()
        && let Some(item) = eval_scalar
            .items
            .iter()
            .find(|item| item.index == column_ref.column.index)
    {
        if statically_non_null(&item.scalar) || !item.scalar.is_deterministic() {
            return None;
        }
    }
...
/// Shallow static nullability: a value that can never be NULL regardless of
/// input data. Conservative — unknown shapes (function calls, etc.) answer
/// false, which only keeps the (unnecessary but harmless) derivation.
fn statically_non_null(expr: &ScalarExpr) -> bool {
    match expr {
        ScalarExpr::BoundColumnRef(col) => !col.column.data_type.is_nullable_or_null(),
        ScalarExpr::ConstantExpr(c) => !c.value.is_null(),
        ScalarExpr::TypedConstantExpr(c, _) => !c.value.is_null(),
        // A strict cast of a non-null value yields a non-null value even when
        // the target type is nullable (failures throw rather than produce
        // NULL); a try cast may produce NULL.
        ScalarExpr::CastExpr(cast) if !cast.is_try => statically_non_null(&cast.argument),
        _ => false,
    }
}
