        ...
        match (left, right) {
            ...
            | (Expr::Constant(constant), Expr::ColumnRef(column_ref)) => {
                ...
                if matches!(self.constraint_context, ConstraintContext::And) {
                    ...
                    if let Some(selectivity) = self.derive_count_min_sketch_equality_selectivity(
                        column_index,
                        op,
                        &constant.scalar,
                    )? {
                ...
                {
                    Ok(selectivity)
                } else if let Some(selectivity) = self
                    .derive_count_min_sketch_equality_selectivity(
                        column_index,
                        op,
                        &constant.scalar,
                    )?
