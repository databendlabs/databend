...
// Current implementation of outer join to inner join is deprecated since
// we are going to decouple data type with `RelOperator`s.
//
// The `convert_outer_to_inner_join` will be reimplemented with the column
// constraint system.
//
// With which system we can check if a scalar expression is null-rejected,
// and then we can convert outer join to inner join with this information.
