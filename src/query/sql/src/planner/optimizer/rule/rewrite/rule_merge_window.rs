...
// Merge two adjacent `Window`s into one
pub struct RuleMergeWindow {
    id: RuleID,
    matchers: Vec<Matcher>,
}

impl RuleMergeWindow {
    pub fn new() -> Self {
        Self {
            id: RuleID::MergeWindow,
            // Window
            // \
            //  Window
            //  \
...
impl Rule for RuleMergeWindow {
    ...
    fn apply(&self, s_expr: &SExpr, state: &mut TransformResult) -> Result<()> {
        ...
        let down_window: Window = s_expr.child(0)?.plan().clone().try_into()?;
