...
fn source_column_from_symbol(metadata: &Metadata, symbol: Symbol) -> Result<Option<SourceColumn>> {
    ...
    if let Some(source) = metadata.lineage_source_column(symbol) {
        return Ok(Some(SourceColumn {
            table: QueryLineageRelation {
                ...
                kind: if source.relation_is_view {
...
fn relation_info_from_table_index(
    ...
    if let Some(lineage_source) = table.lineage_source_relation() {
...
mod tests {
    ...
    struct FakeTable {
        ...
        lineage_source_table_info: Option<TableInfo>,
    ...
    impl Table for FakeTable {
        ...
        fn lineage_source_table_info(&self) -> Option<&TableInfo> {
            self.lineage_source_table_info.as_ref()
        }
    ...
    fn test_view_lineage_stops_at_view_boundary() -> Result<()> {
        ...
        metadata
            .write()
            .add_lineage_source_column(vx, LineageSourceColumn {
    ...
    fn add_fake_stream_table(
        ...
        metadata.set_table_lineage_source_relation(table_index, LineageSourceRelation {
    ...
    fn fake_table(table_id: u64, table_name: &str, columns: &[&str]) -> Arc<dyn Table> {
        Arc::new(FakeTable {
            ...
            lineage_source_table_info: None,
            stream_columns: vec![],
        })
    }
    ...
    fn fake_stream_table(
        ...
        Arc::new(FakeTable {
            ...
            lineage_source_table_info: Some(table_info(
