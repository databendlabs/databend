...
fn multi_insert_target_table_id(
    ...
    plan.target_tables
        .iter()
        .find(|target| {
            target.catalog == into.catalog
                && target.database == into.database
                && target.table == into.table
                && target.branch == into.branch
        })
        .and_then(|target| (target.table_id != 0).then_some(target.table_id))
}
