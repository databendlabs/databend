...
impl Binder {
    pub(in crate::planner::binder) async fn bind_create_share(
        ...
        Ok(Plan::CreateShare(Box::new(CreateSharePlan {
            ...
            connection: stmt
                .connection
                .as_ref()
                .map(|connection| normalize_identifier(connection, &self.name_resolution_ctx).name),
    ...
    pub(in crate::planner::binder) async fn bind_alter_share(
        ...
        let action = match &stmt.action {
            ...
            AlterShareAction::Set {
                accounts,
                connection,
                comment,
            } => {
                if accounts.is_none() && connection.is_none() && comment.is_none() {
                    return Err(ErrorCode::BadArguments(
                        "ALTER SHARE SET requires ACCOUNTS, CONNECTION or COMMENT",
                    ));
                }
                AlterSharePlanAction::Set {
                    ...
                    connection: connection.as_ref().map(|connection| {
                        normalize_identifier(connection, &self.name_resolution_ctx).name
                    }),
