...
impl Binder {
    ...
    pub(in crate::planner::binder) async fn bind_create_materialized_view(
        ...
        // Reject materialized CTEs in MV definitions — they require a subquery
        // executor during planning which conflicts with the multi-phase planning
        // architecture (privilege check, access log, refresh).
        if let Some(with) = &query.with {
            for cte in &with.ctes {
                if cte.user_specified_materialized {
                    return Err(ErrorCode::SyntaxException(
                        "Materialized CTEs are not supported in materialized view definitions"
                            .to_string(),
                    ));
                }
            }
