...
impl Binder {
    ...
    pub(in crate::planner::binder) async fn bind_create_materialized_view(
        ...
        // Reject materialized CTEs in MV definitions: refresh re-plans stored
        // SQL without the request subquery executor that materialized CTEs need.
        let mut materialized_cte_detector = MaterializedCteDetector::default();
        query.drive(&mut materialized_cte_detector);
        ...
        query.drive(&mut executor_dependent_pivot_detector);
        ...
        if matches!(*create_option, AstCreateOption::CreateOrReplace)
            && let Ok(existing) = target_catalog
                .get_table(&tenant, &database_name, &view_name)
                .await
            && existing.options().contains_key(OPT_KEY_MATERIALIZED_VIEW)
