...
impl Binder {
    ...
    fn materialized_view_cluster_schema(
        physical_schema: &TableSchemaRef,
        logical_schema: &databend_common_expression::TableSchema,
    ) -> TableSchemaRef {
        let fields = physical_schema
            .fields()
            .iter()
            .filter(|physical_field| {
                logical_schema.fields().iter().any(|logical_field| {
                    logical_field.name() == physical_field.name()
                        && logical_field
                            .default_expr()
                            .is_some_and(|expr| expr == logical_field.name())
                })
            })
            .cloned()
            .collect();
    ...
    pub(in crate::planner::binder) async fn bind_create_materialized_view(
        ...
        if let Some(cluster_opt) = cluster_by {
            let cluster_schema =
                Self::materialized_view_cluster_schema(&physical_schema, &logical_schema);
            let keys = self
    ...
    pub(in crate::planner::binder) async fn bind_alter_materialized_view(
        ...
        match action {
            AlterTableAction::AlterTableClusterKey { cluster_by } => {
                // The logical definition is required only for cluster-key binding: it identifies
                // user-visible physical columns and GROUP BY keys while excluding aggregate states
                // and the refresh-only source-row-id column.
                let catalog_handle = self.ctx.get_catalog(&catalog).await?;
                let definition = catalog_handle
                    .get_mv_definition(&tenant, table_id)
                    .await?
                    .ok_or_else(|| {
                ...
                let cluster_schema = Self::materialized_view_cluster_schema(
                    &mv_table.schema(),
                    &definition.data.logical_schema,
                );
