...
/// Whether a bind failure may stop being a failure before the task actually runs.
///
/// Task SQL is validated when it is defined but executed later, so a failure that only
/// reflects the current catalog contents, the current privileges, or a transient
/// environment must not reject the definition. Every other bind failure is deterministic:
/// it would fail the same way at run time, so it is reported to the user immediately.
...
impl Binder {
    ...
    /// their static SQL and expressions. Report every deterministic bind failure, and
    /// tolerate only the failures that may resolve before the task runs (see
    /// [`is_deferrable_bind_error`]).
