...
pub struct InternalColumnBinding {
    ...
    /// Owner table selected during name resolution. Some programmatically-created internal
    /// columns leave this unset and resolve it from the current context when they are bound.
...
impl CteContext {
    ...
    pub fn set_cte_context(&mut self, cte_context: CteContext) {
        ...
        self.virtual_column_outputs = cte_context.virtual_column_outputs;
    }
...
impl BindContext {
    ...
    fn search_bound_columns_in_context(
        ...
        if let Some(internal_column) = INTERNAL_COLUMN_FACTORY.get_internal_column(&column.name) {
            let table_indices = bind_context
                .columns
                .iter()
                .filter(|binding| Self::match_table_binding(database, table, binding))
