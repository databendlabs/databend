        ...
        let mut scalars = scalars
            ...
            .map(|(_, item)| {
                if bind_context.in_grouping {
                    ...
                    if let ScalarExpr::WindowFunction(win) = &item.scalar {
                        if let Some(column) = bind_context
                            .windows
                            .window_functions_map
                            .get(&win.agg_func.display_name)
                        {
                            let window_info = &bind_context.windows.window_functions[*column];
                            let column_binding = ColumnBinding {
                                ...
                                data_type: Box::new(
                                    window_info.aggregate_function.scalar.data_type()?,
                                ),
                                visibility: Visibility::Visible,
                            };
                            let scalar = BoundColumnRef {
                                span: None,
                                column: column_binding,
                            }
                            .into();
                            return Ok(ScalarItem {
                                scalar,
                                index: item.index,
                            });
                        }
