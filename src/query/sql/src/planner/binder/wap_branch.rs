...
impl Binder {
    /// Resolve the configured branch for a DML target table.
    ///
    /// `wap_branch` only supplies an implicit branch for writes. Reads and
    /// non-DML statements use the base table unless they contain an explicit
    /// `table/branch` reference.
