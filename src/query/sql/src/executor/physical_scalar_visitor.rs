...
use super::PhysicalScalar;

/// Controls how the visitor recursion should proceed.
pub enum Recursion<V: PhysicalScalarVisitor> {
...
/// `PhysicalScalarVisitor::accept`, `PhysicalScalarVisitor::visit` is invoked
/// recursively on all nodes of an expression tree. See the comments
/// on `PhysicalScalarVisitor::accept` for details on its use
pub trait PhysicalScalarVisitor: Sized {
    /// Invoked before any children of `expr` are visisted.
    fn pre_visit(self, expr: &PhysicalScalar) -> Result<Recursion<Self>>;

    fn visit(mut self, predecessor_expr: &PhysicalScalar) -> Result<Self> {
    ...
    fn post_visit(self, _scalar: &PhysicalScalar) -> Result<Self> {
        Ok(self)
    }
}
...
impl PhysicalScalar {
    ...
    /// its children, calling [`PhysicalScalarVisitor::pre_visit`] and
    /// `visitor.post_visit`.
    ///
    ...
    pub fn accept<V: PhysicalScalarVisitor>(&self, visitor: V) -> Result<V> {
...
enum RecursionProcessing<'a> {
    Call(&'a PhysicalScalar),
    Ret(&'a PhysicalScalar),
}
...
impl RequireColumnsVisitor {
    pub fn collect_columns_from_expr(expr: &PhysicalScalar) -> Result<HashSet<usize>> {
    ...
    fn pre_visit(self, expr: &PhysicalScalar) -> Result<Recursion<Self>> {
        match expr {
            PhysicalScalar::IndexedVariable { index, .. } => {
                let mut v = self;
                v.required_columns.insert(*index);
