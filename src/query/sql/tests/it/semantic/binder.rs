...
async fn test_binder_grouping_and_srf_paths() -> Result<()> {
    let cases = [
        ...
        SqlTestCase {
            ...
            description: "GROUP BY should resolve an unqualified name to the input column before a same-name SELECT alias by default.",
...
#[tokio::test(flavor = "multi_thread", worker_threads = 1)]
async fn test_binder_enable_group_by_alias_first_setting() -> Result<()> {
    let case = SqlTestCase {
        name: "group_by_alias_first_disabled_prefers_column",
        description: "",
        setup_sqls: &["CREATE TABLE t(i UInt64, j UInt64)"],
        sql: "SELECT 1 AS i, sum(i) FROM t GROUP BY i",
    };
    let ctx = setup_context(&case).await?;
    ctx.get_settings()
        .set_setting("enable_group_by_alias_first".to_string(), "0".to_string())?;
    let plan = ctx.bind_sql(case.sql).await?;
    let plan = plan.format_indent(Default::default())?;
    assert!(
        plan.contains("group items: [t.i (#0) AS (#0)]"),
        "GROUP BY should bind the input column when enable_group_by_alias_first=0:\n{plan}"
    );

    let case = SqlTestCase {
        name: "enable_group_by_alias_first_setting_prefers_alias",
        description: "",
        setup_sqls: &["CREATE TABLE t(i UInt64, j UInt64)"],
        sql: "SELECT 1 AS i, sum(i) FROM t GROUP BY i",
    };
    let ctx = setup_context(&case).await?;
    ctx.get_settings()
        .set_setting("enable_group_by_alias_first".to_string(), "1".to_string())?;
    let plan = ctx.bind_sql(case.sql).await?;
    let plan = plan.format_indent(Default::default())?;
    assert!(
        plan.contains("group items: [1 AS"),
        "GROUP BY should bind the SELECT alias before the input column when enable_group_by_alias_first=1:\n{plan}"
    );

    let case = SqlTestCase {
        name: "group_by_alias_first_disabled_keeps_alias_fallback",
        description: "",
        setup_sqls: &["CREATE TABLE t(i UInt64, j UInt64)"],
    ...
    assert!(
        ...
        "GROUP BY should still bind non-conflicting aliases when enable_group_by_alias_first=0:\n{plan}"
    );
