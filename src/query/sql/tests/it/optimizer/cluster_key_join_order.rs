...
async fn test_cluster_key_join_order_golden() -> Result<()> {
    ...
    for case in [
        ...
        JoinOrderCase {
            name: "limit_preserves_base_cluster_keys",
            description: "Cluster keys below a limit can discount the first join, but do not propagate through its output.",
            tables: key_tables_with_a_clustered_k1_k2(),
            settings: CLUSTER_KEY_DISCOUNT,
            sql: "
                    SELECT *
        ...
        JoinOrderCase {
            name: "self_join_expression_filter_matches_alias",
            description: "Expression cluster keys bind to the query symbols of the matching self-join instance.",
            tables: vec![JoinOrderTable {
                create_sql: "CREATE TABLE t(join_key BIGINT, column_a STRING, column_b STRING) CLUSTER BY (length(column_a))",
                column_statistics: string_column_statistics,
            }],
            settings: CLUSTER_KEY_DISCOUNT,
            sql: "SELECT * FROM t AS l JOIN t AS r ON l.join_key = r.join_key WHERE length(l.column_a) = 3 AND length(r.column_b) = 3",
        },
        JoinOrderCase {
