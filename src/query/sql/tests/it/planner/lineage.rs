...
async fn test_query_lineage_aggregate_arguments_from_sql() -> Result<()> {
    ...
    let lineage = query_lineage_from_sql(
        &ctx,
        "CREATE TABLE dst AS SELECT count(*) AS all_count, sum(a) AS sum_a FROM src",
    )
    .await?;

    assert_lineage_sources(&lineage, QueryLineageKind::Ctas, "dst", "all_count", &[]);
    assert_lineage_sources(&lineage, QueryLineageKind::Ctas, "dst", "sum_a", &["src.a"]);
    Ok(())
}
        ...
        .spawn(|| {
            tokio::runtime::Builder::new_multi_thread()
                ...
                .block_on(async {
                    ...
                    // Keep an expression above the view output so optimized-plan
                    // extraction must preserve the view lineage boundary.
                    let lineage =
                        query_lineage_from_sql(&ctx, "INSERT INTO dst SELECT vx + 1 FROM v")
                            .await?;
                    let expected = expected_query_lineage(
