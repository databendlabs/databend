...
pub fn alter_table_action(i: Input) -> IResult<AlterTableAction> {
    ...
    // NOTE: `AT (TAG => ...)` travel-point syntax is only supported when
    ...
    let create_table_branch = map(
        rule! {
            CREATE ~ BRANCH ~ #ident ~ ( AT ~ ^(#travel_point | #at_table_tag) )? ~ (RETAIN ~ #literal_duration)?
        },
    ...
    let create_table_tag = map(
        rule! {
            CREATE ~ TAG ~ #ident ~ ( AT ~ ^(#travel_point | #at_table_tag) )? ~ (RETAIN ~ #literal_duration)?
        },
