...
pub fn statement_body(i: Input) -> IResult<Statement> {
    ...
    // Materialized views only support linear clustering. Hilbert clustering adds an internal
    // predicate column to the stored schema, while an MV's backing schema must remain identical
    // to the bound query output.
            ...
            Ok(Statement::CreateMaterializedView(
                CreateMaterializedViewStmt {
                    ...
                    sync_creation: opt_creation_mode
                        .is_some_and(|mode| mode.text().eq_ignore_ascii_case("sync")),
                    catalog,
                    database,
                    view,
                    cluster_by: opt_cluster_by.map(|(_, _, _, _, exprs, _)| ClusterOption {
...
AS
  <sql>`"
                ...
                | #create_materialized_view : "`CREATE [OR REPLACE] [ASYNC | SYNC] MATERIALIZED VIEW [IF NOT EXISTS] [<database>.]<view> [CLUSTER BY [LINEAR] (...)] AS SELECT ...`"
