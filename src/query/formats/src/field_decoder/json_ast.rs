...
impl FieldJsonAstDecoder {
    ...
    pub fn read_field(&self, column: &mut ColumnBuilder, value: &Value) -> Result<()> {
        match column {
            ...
            ColumnBuilder::TimestampTimezone(c) => self.read_timestamp_timezone(c, value),
    ...
    fn read_timestamp_timezone(
        ...
        match value {
            Value::String(s) => {
                let ts_tz = string_to_timestamp_timezone(s.as_bytes(), || &self.jiff_timezone)?;
                column.push(ts_tz);
                Ok(())
            }
            _ => Err(ErrorCode::BadBytes(
                "Incorrect TimestampTimezone value, must be string",
            )),
        }
    }
