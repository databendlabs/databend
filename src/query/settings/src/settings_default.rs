        ...
        Ok(Arc::clone(DEFAULT_SETTINGS.get_or_try_init(|| -> Result<Arc<DefaultSettings>> {
            ...
            let default_settings = HashMap::from([
                ...
                ("enable_spatial_join", DefaultSettingValue {
                    value: UserSettingValue::UInt64(0),
                    desc: "Enables spatial index join for supported inner spatial joins. The smaller side is indexed locally with an R-tree. The setting is off by default.",
                    mode: SettingMode::Both,
                    scope: SettingScope::Both,
                    range: Some(SettingRange::Numeric(0..=1)),
                }),
                ("spatial_join_max_build_rows", DefaultSettingValue {
                    ...
                    desc: "Maximum estimated rows allowed on the spatial index join build side.",
                    mode: SettingMode::Both,
                    scope: SettingScope::Both,
                    range: Some(SettingRange::Numeric(0..=u64::MAX)),
                }),
                ("spatial_join_max_build_bytes", DefaultSettingValue {
                    value: UserSettingValue::UInt64(256 * 1024 * 1024),
                    desc: "Maximum estimated raw bytes allowed on the spatial index join build side. Admission requires: bytes <= max_build_bytes AND bytes * overhead_factor <= max_indexed_build_bytes.",
                    mode: SettingMode::Both,
                    scope: SettingScope::Both,
                    range: Some(SettingRange::Numeric(0..=u64::MAX)),
                }),
                ("spatial_join_max_indexed_build_bytes", DefaultSettingValue {
                    value: UserSettingValue::UInt64(128 * 1024 * 1024),
                    desc: "Maximum estimated R-tree memory for spatial index join. Admission requires: bytes * overhead_factor <= this value. With defaults (factor=4), effective raw byte limit is 32MB.",
                    mode: SettingMode::Both,
                    scope: SettingScope::Both,
                    range: Some(SettingRange::Numeric(0..=u64::MAX)),
                }),
                ("spatial_join_index_overhead_factor", DefaultSettingValue {
                    value: UserSettingValue::UInt64(4),
                    desc: "Multiplier applied to build-side scan bytes to estimate R-tree memory. Used in admission: bytes * this_factor <= max_indexed_build_bytes.",
                    mode: SettingMode::Both,
                    scope: SettingScope::Both,
                    range: Some(SettingRange::Numeric(1..=u64::MAX)),
                }),
