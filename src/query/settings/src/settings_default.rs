        ...
        Ok(Arc::clone(DEFAULT_SETTINGS.get_or_try_init(|| -> Result<Arc<DefaultSettings>> {
            ...
            let default_settings = HashMap::from([
                ...
                ("join_runtime_filter_probe_ratio_threshold", DefaultSettingValue {
                    value: UserSettingValue::UInt64(3),
                    desc: "Minimum probe/build rows ratio required to enable join runtime filters. Filters are enabled only when (probe_rows / build_rows) >= threshold. Default 3 means probe side must be at least 3x larger.",
                    mode: SettingMode::Both,
                    scope: SettingScope::Both,
                    range: Some(SettingRange::Numeric(1..=u64::MAX)),
                }),
