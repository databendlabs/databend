...
pub struct TableSchema {
    ...
    column_id_set: Option<BTreeSet<u32>>,
}
...
pub struct TableField {
    ...
    // column_ids include all the column id(including child column ids) of this field in DFS order,
    // column_ids will be calculate every time init a TableField, so it need not to be serialize.
    #[serde(skip_serializing)]
...
impl TableSchema {
    pub fn empty() -> Self {
        Self {
            ...
            column_id_set: Some(BTreeSet::new()),
        }
    }
    ...
    fn build_members_from_fields(
        fields: &[TableField],
        next_column_id: u32,
    ) -> (u32, BTreeSet<u32>, Vec<TableField>) {
        let (has_inited, mut new_next_column_id) = if next_column_id > 0 {
            (true, next_column_id)
        } else {
            (false, 0)
        };
        let mut column_id_set = BTreeSet::new();
        let mut new_fields = Vec::with_capacity(fields.len());

        for (i, field) in fields.iter().enumerate() {
            // only when i == 0 or has_inited, use TableField.column_id to calc TableField.column_ids,
            // otherwise TableField.column_id is generated by Schema.next_column_id.
            let new_field = field.build_column_ids(&mut new_next_column_id, i == 0 || has_inited);
        ...
        new_fields.iter().for_each(|field| {
            for column_id in field.column_ids() {
                column_id_set.insert(*column_id);
            }
        });

        // assert if Schema.next_column_id has been init, then it will never change
        // when calc TableField column_ids.
        if has_inited {
            assert_eq!(next_column_id, new_next_column_id);
        }
        // check next_column_id value cannot fallback
        assert!(new_next_column_id >= next_column_id);

        (new_next_column_id, column_id_set, new_fields)
    }

    pub fn new(fields: Vec<TableField>) -> Self {
        let (next_column_id, column_id_set, new_fields) =
            Self::build_members_from_fields(&fields, 0);
        Self {
            fields: new_fields,
            metadata: BTreeMap::new(),
    ...
    pub fn new_from_column_ids(
        ...
        let (next_column_id, column_id_set, new_fields) =
            Self::build_members_from_fields(&fields, next_column_id);
        Self {
            fields: new_fields,
            metadata,
            next_column_id: Some(next_column_id),
    ...
    pub fn column_id_set(&self) -> &BTreeSet<u32> {
    ...
    pub fn add_columns(&mut self, fields: &[TableField]) -> Result<()> {
        for f in fields {
            ...
            let field = f.build_column_ids(self.next_column_id.as_mut().unwrap(), false);
            for column_id in field.column_ids() {
                self.column_id_set.as_mut().unwrap().insert(*column_id);
            }
    ...
    pub fn drop_column(&mut self, column: &str) -> Result<()> {
        ...
        for column_id in self.fields[i].column_ids() {
        ...
        } = &field.data_type
        {
            ...
            let fields = fields_name
                ...
                .map(|(name, ty)| {
                    ...
                    field.build_column_ids(&mut next_column_id, false)
                })
                .collect::<Vec<_>>();
    ...
    pub fn project_by_fields(&self, fields: &BTreeMap<usize, TableField>) -> Self {
        ...
        for (index, f) in fields.iter() {
            ...
            let field = f.build_column_ids(&mut column_id, false);
...
impl TableField {
        ...
        match data_type {
            TableDataType::Tuple {
                ...
                for inner_type in fields_type {
                    Self::build_from_data_type(inner_type, column_ids, next_column_id);
                }
            }
            TableDataType::Array(a) => {
                Self::build_from_data_type(a.as_ref(), column_ids, next_column_id);
            }
            _ => {
                *next_column_id += 1;
            }
    ...
    pub fn build_column_ids(&self, next_column_id: &mut u32, use_field_column_id: bool) -> Self {
        let column_id = *next_column_id;
        let data_type = self.data_type();
        let mut new_next_column_id = match self.column_id {
            Some(column_id) => {
                if column_id > 0 || use_field_column_id {
                    column_id
                } else {
                    *next_column_id
                }
        ...
        Self::build_from_data_type(data_type, &mut column_ids, &mut new_next_column_id);
        ...
        *next_column_id = new_next_column_id;
        Self {
            ...
            column_id: match self.column_id {
                Some(field_column_id) if field_column_id > 0 || use_field_column_id => {
                    Some(field_column_id)
                }
                _ => Some(column_id),
            },
            column_ids: Some(column_ids),
    ...
    pub fn leaf_column_ids(&self) -> Vec<u32> {
        let h: BTreeSet<u32> =
            BTreeSet::from_iter(self.column_ids.as_ref().unwrap().iter().cloned());
        h.into_iter().sorted().collect()
    }

    pub fn column_ids(&self) -> &Vec<u32> {
        self.column_ids.as_ref().unwrap()
    }
