...
impl TableSchema {
    ...
    fn build_members_from_fields(
        ...
        for (i, field) in fields.iter().enumerate() {
            // only when i == 0 or has_inited, use TableField.column_id to calc TableField.column_id,
            // otherwise TableField.column_id is generated by Schema.next_column_id.
            let new_field = field.build_column_id(&mut new_next_column_id, i == 0 || has_inited);
        ...
        // when calc TableField column_id.
        if has_inited {
    ...
    pub fn add_columns(&mut self, fields: &[TableField]) -> Result<()> {
        for f in fields {
            ...
            let field = f.build_column_id(self.next_column_id.as_mut().unwrap(), false);
        ...
        } = &field.data_type
        {
            ...
            let fields = fields_name
                ...
                .map(|(name, ty)| {
                    ...
                    field.build_column_id(&mut next_column_id, false)
                })
                .collect::<Vec<_>>();
    ...
    pub fn project_by_fields(&self, fields: &BTreeMap<usize, TableField>) -> Self {
        ...
        for (index, f) in fields.iter() {
            ...
            let field = f.build_column_id(&mut column_id, false);
...
impl TableField {
    ...
    pub fn build_column_id(&self, next_column_id: &mut u32, use_field_column_id: bool) -> Self {
        let column_id = *next_column_id;
        let data_type = self.data_type();

        let mut new_next_column_id = match self.column_id {
            Some(field_column_id) => {
                if field_column_id > 0 || use_field_column_id {
                    *next_column_id = field_column_id;
                    return self.clone();
                } else {
                    *next_column_id
                }
