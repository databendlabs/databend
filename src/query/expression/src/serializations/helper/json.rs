...
pub fn write_json_string(bytes: &[u8], buf: &mut Vec<u8>, settings: &FormatSettings) {
    let mut start = 0;
    let len = bytes.len();
    let table = if settings.json_escape_forward_slashes {
        ESCAPE_WITH_FS
    } else {
        ESCAPE
    };
    ...
    for (i, &byte) in bytes.iter().enumerate() {
        ...
        if escape == QU {
            ...
            if settings.json_quote_denormals {
                buf.push(QU);
            } else {
                buf.push(b'\\');
            }
