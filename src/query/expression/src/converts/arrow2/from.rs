...
impl Column {
    pub fn from_arrow(
        ...
        match (data_type, arrow_col.data_type()) {
            ...
            (DataType::Number(NumberDataType::UInt16), ArrowDataType::UInt16) => {
                Column::Number(NumberColumn::UInt16(
                    arrow_col
                        ...
                        .expect("fail to read `UInt16` from arrow: array should be `UInt16Array`")
                        .values()
                        .clone(),
                ))
            }
            (DataType::Number(NumberDataType::UInt32), ArrowDataType::UInt32) => {
                Column::Number(NumberColumn::UInt32(
                    arrow_col
                        ...
                        .expect("fail to read `UInt32` from arrow: array should be `UInt32Array`")
                        .values()
                        .clone(),
                ))
            }
            (DataType::Number(NumberDataType::UInt64), ArrowDataType::UInt64) => {
                Column::Number(NumberColumn::UInt64(
                    arrow_col
                        ...
                        .expect("fail to read `UInt64` from arrow: array should be `UInt64Array`")
                        .values()
                        .clone(),
                ))
            }
            ...
            (DataType::Binary, ArrowDataType::Binary) => {
                let arrow_col = arrow_col
                    ...
                    .expect("fail to read `Binary` from arrow: array should be `BinaryArray<i32>`");
            ...
            (DataType::Binary, ArrowDataType::LargeBinary) => {
                let arrow_col = arrow_col
                    ...
                    .expect("fail to read `Binary` from arrow: array should be `BinaryArray<i64>`");
            ...
            (DataType::String, ArrowDataType::Binary) => {
                let arrow_col = arrow_col
                    .as_any()
                    .downcast_ref::<databend_common_arrow::arrow::array::BinaryArray<i32>>()
                    .expect("fail to read `String` from arrow: array should be `BinaryArray<i32>`");
                let offsets = arrow_col
                    .offsets()
                    .buffer()
                    .iter()
            ...
            (DataType::String, ArrowDataType::LargeBinary) => {
                let arrow_col = arrow_col
                    .as_any()
                    .downcast_ref::<databend_common_arrow::arrow::array::BinaryArray<i64>>()
                    .expect("fail to read `String` from arrow: array should be `BinaryArray<i64>`");
                let offsets = arrow_col.offsets().clone().into_inner();
                let offsets = unsafe { std::mem::transmute::<Buffer<i64>, Buffer<u64>>(offsets) };
            ...
            (DataType::String, ArrowDataType::Utf8) => {
                let arrow_col = arrow_col
                    ...
                    .expect("fail to read `String` from arrow: array should be `Utf8Array<i32>`");
                let offsets = arrow_col
                    .offsets()
                    .buffer()
                    .iter()
            ...
            (DataType::String, ArrowDataType::LargeUtf8) => {
                let arrow_col = arrow_col
                    ...
                    .expect("fail to read `String` from arrow: array should be `Utf8Array<i64>`");
                let offsets = arrow_col.offsets().clone().into_inner();
                let offsets = unsafe { std::mem::transmute::<Buffer<i64>, Buffer<u64>>(offsets) };
                Column::String(StringColumn::new(arrow_col.values().clone(), offsets))
            }
            ...
            (DataType::Array(ty), ArrowDataType::List(_)) => {
                let values_col = arrow_col
                    ...
                    .expect("fail to read `Array` from arrow: array should be `ListArray<i32>`");
            ...
            (DataType::Array(ty), ArrowDataType::LargeList(_)) => {
                let values_col = arrow_col
                    ...
                    .expect("fail to read `Array` from arrow: array should be `ListArray<i64>`");
                let values = Column::from_arrow(&**values_col.values(), ty);
            ...
            (DataType::Bitmap, ArrowDataType::Extension(name, box ArrowDataType::Binary, None))
                if name == ARROW_EXT_TYPE_BITMAP =>
            {
                let arrow_col = arrow_col
                    .as_any()
                    .downcast_ref::<databend_common_arrow::arrow::array::BinaryArray<i32>>()
                    .expect("fail to read `Bitmap` from arrow: array should be `BinaryArray<i32>`");
                let offsets = arrow_col
                    .offsets()
                    .buffer()
                    .iter()
            ...
            (
                ...
                let arrow_col = arrow_col
                    .as_any()
                    .downcast_ref::<databend_common_arrow::arrow::array::BinaryArray<i64>>()
                    .expect("fail to read `Bitmap` from arrow: array should be `BinaryArray<i64>`");
                let offsets = arrow_col.offsets().clone().into_inner();
                let offsets = unsafe { std::mem::transmute::<Buffer<i64>, Buffer<u64>>(offsets) };
            ...
            (ty, arrow_ty) => {
                unimplemented!("conversion from arrow type {arrow_ty:?} to {ty:?} is not supported")
            }
        }
    }
}
