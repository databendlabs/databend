...
pub enum ColumnVec {
    ...
    Date(Vec<Buffer<i32>>),
    Interval(Vec<Buffer<(i32, i32, i64)>>),
...
impl Scalar {
    ...
    pub fn is_positive(&self) -> bool {
        match self {
            ...
            Scalar::Date(d) => *d > 0,
            // TODO: interval should check is positive?
    ...
    fn hash<H: std::hash::Hasher>(&self, state: &mut H) {
        match self {
            ...
            ScalarRef::Interval(v) => {
                // TODO: need modify
                // (0,1,0) and (1,0,0) hash is same
                let val = v.0 as i64 + v.1 as i64 + v.2;
                val.hash(state)
            }
            ScalarRef::Array(v) => {
...
impl ColumnBuilder {
    ...
    pub fn repeat_default(ty: &DataType, len: usize) -> ColumnBuilder {
        match ty {
            ...
            DataType::Interval => ColumnBuilder::Interval(vec![(0, 0, 0); len]),
    ...
    pub fn push_default(&mut self) {
        match self {
            ...
            ColumnBuilder::Interval(builder) => builder.push((0, 0, 0)),
