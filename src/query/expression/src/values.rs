...
impl Scalar {
    ...
    pub fn default_value(ty: &DataType) -> Scalar {
        match ty {
            ...
            DataType::Variant | DataType::Geometry => Scalar::Variant(vec![]),
...
impl PartialOrd for ScalarRef<'_> {
    fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
        match (self, other) {
            ...
            (ScalarRef::Geometry(_), ScalarRef::Geometry(_)) => None, /* todo(ariesdevil): support geo compare */
            _ => None,
        }
    }
}
        ...
        match ty {
            ...
            DataType::Geometry => {
                ...
                (0..len).for_each(|_| {
                    ...
                    data.push(serde_json::to_vec(&val).unwrap());
                });
