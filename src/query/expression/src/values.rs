...
use crate::types::geo::GeometryColumnBuilder;
use crate::types::geo::GeometryScalar;
...
impl Scalar {
    pub fn as_ref(&self) -> ScalarRef {
        match self {
            ...
            Scalar::Geometry(geo) => ScalarRef::Geometry(*geo),
        }
    }
    ...
    pub fn default_value(ty: &DataType) -> Scalar {
        match ty {
            ...
            DataType::Geometry(geo_type) => Scalar::Geometry(geo_type.default_scalar()),
...
impl<'a> ScalarRef<'a> {
    pub fn to_owned(&self) -> Scalar {
        match self {
            ...
            ScalarRef::Geometry(geo) => Scalar::Geometry(**geo),
        }
    }
    ...
    pub fn domain(&self, data_type: &DataType) -> Domain {
        ...
        match self {
            ...
            ScalarRef::Geometry(geo) => todo!(),
            ScalarRef::Binary(_) | ScalarRef::Bitmap(_) | ScalarRef::Variant(_) => {
                Domain::Undefined
            }
        }
    }
    ...
    pub fn memory_size(&self) -> usize {
        match self {
            ...
            ScalarRef::Geometry(geo) => geo.memory_size(),
        }
    }
    ...
    pub fn infer_data_type(&self) -> DataType {
        match self {
            ...
            ScalarRef::Geometry(geo) => DataType::Geometry(geo.data_type()),
        }
    }
    ...
    pub fn is_value_of_type(&self, data_type: &DataType) -> bool {
        match (self, data_type) {
            ...
            _ => match (self, data_type.remove_nullable()) {
                ...
                (ScalarRef::Geometry(_), DataType::Geometry(_)) => true,
...
impl Hash for ScalarRef<'_> {
    fn hash<H: std::hash::Hasher>(&self, state: &mut H) {
        match self {
            ...
            ScalarRef::Geometry(g) => with_geometry_type!(|GEO_TYPE| match g {
                GeometryScalar::GEO_TYPE(v) => {
                    v.hash(state)
                }
            }),
        }
...
impl Column {
    ...
    pub fn domain(&self) -> Domain {
        ...
        match self {
            ...
            Column::Geometry(col) => todo!(),
            Column::Binary(_) | Column::Bitmap(_) | Column::Variant(_) => Domain::Undefined,
        }
    }
    ...
    pub fn data_type(&self) -> DataType {
        match self {
            ...
            Column::Geometry(g) => with_geometry_type!(|GEO_TYPE| match g {
                GeometryColumn::GEO_TYPE(_) => DataType::Geometry(GeometryDataType::GEO_TYPE),
            }),
        }
    }
    ...
    pub fn random(ty: &DataType, len: usize) -> Self {
        ...
        match ty {
            ...
            DataType::Geometry(geo) => {
                // todo: maybe add a setting to rand different subtype of geos
                todo!()
            }
    ...
    pub fn serialize_size(&self) -> usize {
        match self {
            ...
            Column::Geometry(geo) => geo.serialize_size(),
        }
    }
...
impl ColumnBuilder {
    pub fn from_column(col: Column) -> Self {
        match col {
            ...
            Column::Geometry(col) => {
                ColumnBuilder::Geometry(GeometryColumnBuilder::from_column(col))
            }
        }
    }
    ...
    pub fn repeat(scalar: &ScalarRef, n: usize, data_type: &DataType) -> ColumnBuilder {
        ...
        match scalar {
            ...
            ScalarRef::Geometry(geo) => {
                ColumnBuilder::Geometry(GeometryColumnBuilder::repeat(*geo, n))
            }
        }
    }
    ...
    pub fn memory_size(&self) -> usize {
        match self {
            ...
            ColumnBuilder::Geometry(builder) => builder.memory_size(),
        }
    }
    ...
    pub fn data_type(&self) -> DataType {
        match self {
            ...
            ColumnBuilder::Geometry(col) => with_geometry_type!(|GEO_TYPE| match col {
                GeometryColumnBuilder::GEO_TYPE(_) =>
                    DataType::Geometry(GeometryDataType::GEO_TYPE),
            }),
        }
    }
    ...
    pub fn with_capacity_hint(
        ...
        match ty {
            ...
            DataType::Geometry(geo_ty) => {
                ColumnBuilder::Geometry(GeometryColumnBuilder::with_capacity(geo_ty, capacity))
            }
            DataType::Generic(_) => {
    ...
    pub fn push_default(&mut self) {
        match self {
            ...
            ColumnBuilder::Geometry(builder) => builder.push_default(),
        }
    }
    ...
    pub fn push_binary(&mut self, reader: &mut &[u8]) -> Result<()> {
        match self {
            ...
            ColumnBuilder::Geometry(builder) => builder.push_binary(reader)?,
        };
    ...
    pub fn push_fix_len_binaries(&mut self, reader: &[u8], step: usize, rows: usize) -> Result<()> {
        match self {
            ...
            ColumnBuilder::Geometry(builder) => {
                for row in 0..rows {
                    let mut reader = &reader[step * row..];
                    builder.push_binary(&mut reader)?;
                }
            }
        }
