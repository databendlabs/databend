...
pub fn cast_scalar_to_variant(
    ...
    let value = match scalar {
        ...
        ScalarRef::Timestamp(ts) => timestamp_to_string(ts, inner_tz, ts_output_format)
            .to_string()
            .into(),
        ScalarRef::Date(d) => date_to_string(d, inner_tz).to_string().into(),
        ScalarRef::Array(col) => {
            let items = cast_scalars_to_variants(col.iter(), tz, ts_output_format);
                ...
                .map(|(k, v)| {
                    let key = match k {
                        ...
                        ScalarRef::Timestamp(v) => {
                            timestamp_to_string(v, inner_tz, ts_output_format).to_string()
                        }
                        ScalarRef::Date(v) => date_to_string(v, inner_tz).to_string(),
                        _ => unreachable!(),
                    };
                    let mut val = vec![];
                    cast_scalar_to_variant(v, tz, ts_output_format, &mut val);
                    (key, val)
                })
                .collect::<Vec<_>>();
        ...
        ScalarRef::Tuple(fields) => {
            let values = cast_scalars_to_variants(fields, tz, ts_output_format);
            jsonb::build_object(
                values
                    .iter()
                    .enumerate()
...
pub fn cast_scalars_to_variants(
    ...
    for scalar in iter {
        cast_scalar_to_variant(scalar, tz, ts_output_format, &mut builder.data);
        builder.commit_row();
    }
    builder.build()
}
