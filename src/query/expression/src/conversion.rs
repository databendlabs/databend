...
fn common_type_for_conversion(left: DataType, right: DataType) -> Option<DataType> {
    ...
    match (left, right) {
        ...
        (DataType::Nullable(left), DataType::Nullable(right)) => Some(DataType::Nullable(
            Box::new(common_type_for_conversion(*left, *right)?),
        )),
        (DataType::Nullable(left), right) => Some(DataType::Nullable(Box::new(
            common_type_for_conversion(*left, right)?,
        ))),
        (left, DataType::Nullable(right)) => Some(DataType::Nullable(Box::new(
            common_type_for_conversion(left, *right)?,
        ))),
        ...
        (DataType::Variant, ty) if is_variant_try_cast_target(&ty) => Some(ty.wrap_nullable()),
        (ty, DataType::Variant) if is_variant_try_cast_target(&ty) => Some(ty.wrap_nullable()),
