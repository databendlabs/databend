...
pub enum Domain {
    ...
    TimestampTimezone(SimpleDomain<timestamp_timezone>),
...
impl Domain {
    ...
    pub fn full(data_type: &DataType) -> Self {
        match data_type {
            ...
            DataType::TimestampTimezone => {
                Domain::TimestampTimezone(TimestampTimezoneType::full_domain())
            }
    ...
    pub fn to_minmax(&self) -> (Scalar, Scalar) {
        match self {
            ...
            Domain::TimestampTimezone(SimpleDomain { min, max }) => (
                Scalar::TimestampTimezone(*min),
                Scalar::TimestampTimezone(*max),
            ),
