...
pub trait ValueVisitor: Sized {
    ...
    fn visit_timestamp_timezone(
        &mut self,
        buffer: Buffer<timestamp_timezone>,
    ) -> Result<Self::U, Self::Error> {
        self.visit_typed_column::<TimestampTimezoneType>(buffer, &DataType::TimestampTimezone)
    }
    ...
    fn default_visit_column<V: ValueVisitor>(
        ...
        match column {
            ...
            Column::TimestampTimezone(buffer) => visitor.visit_timestamp_timezone(buffer),
