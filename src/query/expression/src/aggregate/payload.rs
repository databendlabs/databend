...
impl Page {
    ...
    pub(super) fn row_ref(&self, row: usize, row_size: usize) -> RowRef {
        debug_assert!(row < self.rows);
        debug_assert_eq!(self.data.len(), self.rows * row_size);
        RowRef::new(unsafe { self.data.as_ptr().add(row * row_size) as *const u8 })
    }
...
impl Payload {
    ...
    pub(super) fn data_ptr(&self, page: &Page, row: usize) -> RowRef {
        page.row_ref(row, self.tuple_size)
    }
    ...
    pub(super) fn reserve_append_rows(
        ...
        let address = unsafe {
            ...
            // two wrappers.
            std::mem::transmute::<&mut [RowRef; BATCH_SIZE], &mut [RowMut; BATCH_SIZE]>(address)
        };
