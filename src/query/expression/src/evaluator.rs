...
pub struct Evaluator<'a> {
    ...
    // need origin number of columns to pop `AsyncFunction` generared values
    origin_num_columns: usize,
}
...
impl<'a> Evaluator<'a> {
    pub fn new(
        ...
        Evaluator {
            ...
            func_ctx,
            fn_registry,
            origin_num_columns: 0,
        }
    }

    pub fn new_with_origin_num_columns(
        data_block: &'a DataBlock,
        func_ctx: &'a FunctionContext,
        fn_registry: &'a FunctionRegistry,
        origin_num_columns: usize,
    ...
    pub fn partial_run(
        ...
        let result = match expr {
            ...
            // async function call values has been generated by transformer
            Expr::AsyncFunctionCall { .. } => {
                let num_columns = self.origin_num_columns;
                let entry = self.data_block.get_by_offset(num_columns - 1);
...
impl<'a, Index: ColumnIndex> ConstantFolder<'a, Index> {
    ...
    fn fold_once(&self, expr: &Expr<Index>) -> (Expr<Index>, Option<Domain>) {
        let (new_expr, domain) = match expr {
            ...
            Expr::AsyncFunctionCall { .. } => (expr.clone(), None),
        };
