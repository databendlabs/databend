...
impl GeometryOverlay {
    ...
    fn intersection_points_on_lines(
        ...
        points
            .iter()
            .copied()
            .filter(|point| lines.iter().any(|line| Self::line_has_point(line, *point)))
            .collect()
    }
    ...
    fn segment_is_on_lines(segment: &LineString<f64>, lines: &[LineString<f64>]) -> bool {
        ...
        let point = Point(mid);
        lines.iter().any(|line| Self::line_has_point(line, point))
    }

    fn line_has_point(line: &LineString<f64>, point: Point<f64>) -> bool {
        if line.0.len() < 2 {
            return false;
        }

        line.0.windows(2).any(|segment| {
            let start = segment[0];
            let end = segment[1];
            Self::segment_has_point(start, end, point.0)
        })
    }

    fn segment_has_point(start: Coord<f64>, end: Coord<f64>, point: Coord<f64>) -> bool {
        if Self::coord_eq(start, end) {
            return Self::coord_eq(start, point);
        }

        if Self::coord_eq(start, point) || Self::coord_eq(end, point) {
            return true;
        }
        ...
        if !Self::point_on_line(start, dir, point) {
            return false;
        }

        let Some(t) = Self::line_param(start, end, point) else {
            return false;
        };

        (-Self::LINE_OVERLAY_EPS..=1.0 + Self::LINE_OVERLAY_EPS).contains(&t)
    }
...
mod tests {
    ...
    #[test]
    fn difference_removes_point_at_line_endpoint() {
        let overlay = GeometryOverlay::new(OverlayMode::Difference);
        let result = overlay
            .apply(&point(0.0, 0.0), &line(&[(0.0, 0.0), (2.0, 0.0)]))
            .unwrap();
        assert!(result.is_none());
    }
    ...
    #[test]
    fn intersection_keeps_point_at_line_endpoint() {
        let overlay = GeometryOverlay::new(OverlayMode::Intersection);
        let result = overlay
            .apply(&point(2.0, 2.0), &line(&[(0.0, 0.0), (2.0, 2.0)]))
            .unwrap();
        assert!(result.is_some());

        match result.unwrap() {
            Geometry::Point(point) => {
                assert!(GeometryOverlay::point_eq(point, Point::new(2.0, 2.0)));
            }
            other => panic!("unexpected geometry: {other:?}"),
        }
    }
