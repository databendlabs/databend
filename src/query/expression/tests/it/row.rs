...
fn test_decimal128() {
    let converter = RowConverter::new(vec![SortField::new(
        DataType::Decimal(DecimalDataType(DecimalSize::new_unchecked(38, 7))).wrap_nullable(),
    )])
    .unwrap();
...
fn test_decimal256() {
    let converter = RowConverter::new(vec![SortField::new(
        DataType::Decimal(DecimalDataType(DecimalSize::new_unchecked(76, 7))).wrap_nullable(),
    )])
    .unwrap();
