...
use databend_query::interpreters::InsertInterpreter;
use databend_query::interpreters::MutationInterpreter;
use databend_query::interpreters::QueryFinishHooks;
...
impl<'a> MaterializedViewRefresh<'a> {
    pub async fn create(
            ...
            .is_some_and(|(mv_source_seq, _)| *mv_source_seq > source_seq)
        {
            ...
            return Err(ErrorCode::InvalidMaterializedView(format!(
                ...
                database, view_name, mv_source_seq, source_seq
            )));
        }
        ...
        let source_database_id = source_meta
            ...
            .parse::<u64>()?;
        let source_database = catalog_obj.get_db_name_by_id(source_database_id).await?;
        let source_table_name = catalog_obj
        ...
        let plan = planner
            ...
            .map_err(|error| {
                ErrorCode::InvalidMaterializedView(format!(
                    ...
                    database, view_name, expected_source_table_id, error
                ))
            })?;
    ...
    fn attach_source(
        ...
        self.ctx
            .evict_table_from_cache(catalog, database, table, None)?;
    ...
    fn build_standard_refresh_merge(&self, source: Query) -> Statement {
        ...
        Statement::MergeInto(MergeIntoStmt {
            ...
            table_ident: Identifier::from_name(None, &self.view_name),
