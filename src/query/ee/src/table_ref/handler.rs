...
impl TableRefHandler for RealTableRefHandler {
    ...
    async fn do_create_table_branch(
        ...
        branch_table_meta.options.insert(
            OPT_KEY_BASE_TABLE_ID.to_string(),
        ...
        let (new_snapshot, lvt_check) = if let Some(location) = source_snapshot_location {
            ...
            branch_table_meta.schema = Arc::new(snapshot.schema.clone());
        ...
        let new_snapshot =
            new_snapshot.expect("hidden branch creation must have a source snapshot");
        ...
        catalog
            .commit_table_branch_meta(CommitTableBranchMetaReq {
                ...
                orphan_branch_name: branch_reply
                    .orphan_branch_name
                    .expect("orphan branch name must exist for hidden branch creation"),
