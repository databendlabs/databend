...
impl TableRefHandler for RealTableRefHandler {
    ...
    async fn do_create_table_ref(
        ...
        let info = match plan.ref_type {
            SnapshotRefType::Branch => SnapshotRefInfo::Branch {
                head: new_snapshot_location.clone(),
                anchor: new_snapshot_location,
            },
            SnapshotRefType::Tag => SnapshotRefInfo::Tag(new_snapshot_location),
        };
        let expire_at = plan.retain.map(|v| Utc::now() + v);
        let mut new_table_meta = table_info.meta.clone();
        new_table_meta
            .refs
            .insert(plan.ref_name.clone(), SnapshotRef { info, expire_at });
