...
use super::presentation::like_match;
...
fn revoke_txn_conditions(target: &ShareRevokeTarget) -> Vec<TxnCondition> {
    match target {
        ShareRevokeTarget::Database {
            database_id,
            database_meta_seq,
            ..
        } => vec![txn_cond_eq_seq(
...
impl ShareMgr {
    ...
    pub(crate) async fn revoke_share_object(
        ...
        let conditions = revoke_txn_conditions(&target);
