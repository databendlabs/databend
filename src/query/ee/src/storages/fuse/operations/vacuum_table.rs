...
pub async fn do_vacuum(
    ...
    let retain_branch_tables = collect_retainable_history_branch_tables(fuse_table, &ctx).await?;
    ctx.set_status_info(&format!(
        "do_vacuum: collected retainable history branches for orphan gc, branches:{}, cost:{:?}",
        retain_branch_tables.len(),
        start.elapsed()
    ));
    ...
    if let Some(mut purge_files) = purge_files_opt {
        ...
        if purge_files.len() < dry_run_limit {
            ...
            for branch_table in retain_branch_tables.iter() {
                if purge_files.len() >= dry_run_limit {
                    break;
                }
