...
pub async fn do_vacuum2(
    ...
    // Set the global vacuum watermark before deleting any S3 data.
    // This ensures that concurrent `undrop_table_branch` (and `undrop_table`) operations
    // will be blocked for branches/tables whose drop_on <= this watermark.
