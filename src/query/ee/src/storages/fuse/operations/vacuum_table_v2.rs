...
pub async fn do_vacuum2(
    ...
    // Base table and branches all operate on their own storage prefix, so they can run concurrently.
    type CleanupFut<'a> = futures::future::BoxFuture<'a, Result<Vec<String>>>;
    let mut cleanup_futs: Vec<CleanupFut<'_>> = Vec::new();
    ...
    if let Some(base_gc_state) = base_gc_state {
        ...
        let protected_blocks = protected_blocks_by_table
            .remove(&base_gc_state.table_id)
            .unwrap_or_default();
        cleanup_futs.push(Box::pin(async move {
            cleanup_table_data(
                fuse_table,
                ctx,
                Some((
    ...
    let cleanup_results: Vec<Result<Vec<String>>> = futures::stream::iter(cleanup_futs)
        .buffer_unordered(concurrency)
        .collect()
        .await;
