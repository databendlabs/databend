...
pub async fn do_vacuum2(
    ...
    ctx.set_status_info(&format!(
        "vacuum2: pre-lvt refs scanned, table:{}, protected_snapshots:{}, protected_segments:{}, protected_blocks:{}",
        table_info.desc,
        ref_info.protected_snapshots.len(),
        ref_info.protected_segments.len(),
        ref_info.protected_blocks.len()
    ));
    ...
    if !new_refs_info.is_empty() {
        ...
        ctx.set_status_info(&format!(
            "vacuum2: merged post-lvt refs, table:{}, protected_snapshots:{}, protected_segments:{}, protected_blocks:{}, files_to_gc:{}",
            table_info.desc,
            ref_info.protected_snapshots.len(),
            ref_info.protected_segments.len(),
            ref_info.protected_blocks.len(),
            ref_info.files_to_gc.len()
        ));
    }
