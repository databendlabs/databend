...
pub async fn do_vacuum2(
    ...
    let ref_info = process_snapshot_refs(fuse_table, &ctx, table_info).await?;
    ...
    ctx.set_status_info(&format!(
        "[FUSE-VACUUM2] Read segments for table {}, elapsed: {:?}, total protected blocks: {}",
        table_info.desc,
        start.elapsed(),
        gc_root_blocks.len()
    ));
...
async fn process_snapshot_refs(
    ...
    // Use ByTimePeriod strategy to calculate retention time point
    let now = Utc::now();
    let retention_period = fuse_table.get_data_retention_period(ctx.as_ref())?;
    let retention_time = now - retention_period;
    // Process active refs
    for (ref_name, snapshot_ref) in table_info.meta.refs.iter() {
        if snapshot_ref.expire_at.is_some_and(|v| v < now) {
            expired_refs.push(ref_name);
            continue;
        }
        ...
        match &snapshot_ref.info {
            SnapshotRefInfo::Tag(location) => {
                ...
                let (tag_snapshot, _) =
                    SnapshotsIO::read_snapshot(location.clone(), op.clone(), false).await?;
            ...
            SnapshotRefInfo::Branch { head, anchor } => {
                ...
                .await?
                {
                    ...
                    None => {
                        let (anchor_snap, _) =
                            SnapshotsIO::read_snapshot(anchor.clone(), op.clone(), false).await?;
                        (anchor.clone(), anchor_snap)
                    }
                };

                if head != &gc_root_location {
                    let meta = fuse_table
                        .get_operator_ref()
    ...
    let active_refs = updated_refs.keys().cloned().collect::<Vec<_>>();
    // Step 1: Update table meta if refs changed
    if updated_refs != table_info.meta.refs {
        let start_update = std::time::Instant::now();
        update_table_refs_meta(fuse_table, ctx, table_info, updated_refs).await?;
        ctx.set_status_info(&format!(
            "[FUSE-VACUUM2] Updated table meta for table {}, elapsed: {:?}",
            table_info.desc,
            start_update.elapsed()
        ));
    }

    // Step 2: Purge ref snapshots if not dry_run
    ...
    ctx.set_status_info(&format!(
        "[FUSE-VACUUM2] Processed snapshot refs for table {}, elapsed: {:?}, active_refs: {}, expire_refs: {}, ref_snapshots_to_gc: {}, ref_gc_root_meta_ts: {:?}",
        table_info.desc,
        start.elapsed(),
        slice_summary(&active_refs),
        slice_summary(&expired_refs),
...
/// Update table metadata with updated refs (expired refs removed, branch anchors updated)
#[async_backtrace::framed]
async fn update_table_refs_meta(
    fuse_table: &FuseTable,
    ctx: &Arc<dyn TableContext>,
    table_info: &TableInfo,
    updated_refs: BTreeMap<String, SnapshotRef>,
) -> Result<()> {
    let catalog = ctx.get_default_catalog()?;
    let mut new_table_meta = table_info.meta.clone();

    // Replace refs with updated refs (which already has expired refs removed and branch anchors updated)
