...
pub async fn do_vacuum2(
    ...
    // Step 7b: clean branch-local segment/block files in parallel.
    // Each branch operates on its own storage prefix and protected sets are already partitioned
    // by table id, so these can safely run concurrently.
    type CleanupFut<'a> = futures::future::BoxFuture<'a, Result<Vec<String>>>;
    let mut branch_cleanup_futs: Vec<CleanupFut<'_>> = Vec::new();
    ...
    for (branch_table, gc_state) in retainable_with_gc_root {
        ...
        let protected_blocks = protected_blocks_by_table
            .remove(&gc_state.table_id)
            .unwrap_or_default();
        branch_cleanup_futs.push(Box::pin(async move {
            cleanup_table_data(
                branch_table.as_ref(),
                ctx,
                Some((
    ...
    let branch_cleanup_results: Vec<Result<Vec<String>>> =
        futures::stream::iter(branch_cleanup_futs)
            .buffer_unordered(concurrency)
            .collect()
            .await;
    for result in branch_cleanup_results {
        files_to_gc.extend(result?);
    }
