...
use databend_common_meta_app::schema::OPT_KEY_REFERENCED_BRANCH_IDS;
...
///
/// In this branch-aware vacuum2 path the map is populated only from
/// `FuseTable::parse_storage_prefix_from_table_info()` for the base table and its branches.
/// Those prefixes are treated as canonical owner roots for persisted data, so they are expected
/// to be non-overlapping within the set we build here.
///
/// If a future storage layout introduces overlapping owner prefixes, the path-to-owner
/// resolution in `table_id_by_path()` must be revisited together with this invariant.
type StoragePrefixes = HashMap<String, u64>;
...
/// Block UUIDs (as u128) that must be kept alive during cleanup, grouped by the table that stores them.
type ProtectedBlocksByTable = HashMap<u64, HashSet<u128>>;
...
pub async fn do_vacuum2(
    ...
    let (base_gc_state, base_snapshot_files) =
    ...
    for branch in history_branches {
        ...
        let branch_name = branch
            .branch_name
            .unwrap_or_else(|| format!("history-branch-{}", branch_id));

        let branch_table_info = TableInfo {
            ident: TableIdent {
                table_id: branch_id,
                seq: branch.branch_meta.seq,
            },
            desc: format!("{}/'{}'", table_info.desc, branch_name),
            name: branch_name,
        ...
        let storage_prefix = format!(
            "{}/",
            FuseTable::parse_storage_prefix_from_table_info(&branch_table_info)?
        );
        storage_prefixes.insert(storage_prefix, branch_id);

        let branch_table =
            FuseTable::create_without_refresh_table_info(branch_table_info, s3_storage_class)?;
        ...
        if expire_at.is_some_and(|expire_at| expire_at <= now) {
            if let Err(err) = catalog
                .drop_table_branch(DropTableBranchReq {
                    ...
                    table_id: fuse_table.get_id(),
                    branch_name: branch_table.get_table_info().name.clone(),
                ...
                .await
            {
                warn!(
                    ...
                    branch_table.get_table_info().name,
                    err
                );
            }
        }
    ...
    let gc_results: Vec<(Box<FuseTable>, BranchPhaseResult)> =
        futures::stream::iter(gc_root_candidates.into_iter().map(|branch_table| {
            let ctx = ctx.clone();
            async move {
                let gc_selection =
    ...
    let mut retainable_with_gc_root: Vec<(Box<FuseTable>, BranchGcState)> = Vec::new();
    let mut retainable_without_gc_root: Vec<(Box<FuseTable>, Option<Arc<TableSnapshot>>)> =
        Vec::new();
    ...
    let mut tables_at_risk = HashSet::new();
    if let Some(base_gc_state) = base_gc_state.as_ref() {
        tables_at_risk.insert(base_gc_state.table_id);
    }
    tables_at_risk.extend(
        retainable_with_gc_root
            .iter()
            .map(|(_, gc_state)| gc_state.table_id),
    );
    tables_at_risk.extend(
        beyond_retention_branches
            .iter()
            .map(|branch| branch.get_id()),
    );
    ...
    let mut self_segments_to_scan: Vec<Location> = Vec::new();
    ...
    let external_segments_results: Vec<Result<(ProtectedSegmentsByTable, Vec<Location>)>> =
    ...
    // protection.
    let all_segments: Vec<Location> = protected_segments_by_table
        .values()
    ...
    while let Some(segment) = segment_stream.next().await {
        for block in segment?.block_metas()? {
            let uuid_str = try_extract_uuid_str_from_path(&block.location.0)?;
            let uuid = parse_uuid_hex_to_u128(uuid_str)?;
    ...
    let mut gc_pending_branches: Vec<Box<FuseTable>> = Vec::new();
    let mut final_gc_branches: Vec<Box<FuseTable>> = Vec::new();
    ...
    // Step 7: clean table-local segment/block files serially for all tables with cleanup work.
    if let Some(base_gc_state) = base_gc_state {
        let protected_segment_paths: HashSet<String> = protected_segments_by_table
            .remove(&base_gc_state.table_id)
            .unwrap_or_default()
            .into_iter()
            .map(|(p, _)| p)
            .collect();
    ...
    for branch_table in gc_pending_branches {
        ...
        let protected_blocks = protected_blocks_by_table
            .remove(&branch_table.get_id())
            .unwrap_or_default();
        let snapshots_to_gc = branch_table
            .list_files_for_gc(
                branch_table
...
async fn vacuum_base_table(
    ...
    if snapshot_files_to_gc.is_empty() {
        // Nothing to clean for the base table. Safe to skip — base table never references
        // other branches' files, so it needs no protection and no cleanup participation.
        return Ok((None, vec![]));
    }
...
async fn cleanup_table_data(
        ...
        .await?
    {
        ...
        let uuid = match parse_uuid_hex_to_u128(uuid_str) {
            Ok(v) => v,
            Err(err) => {
                warn!(
        ...
        if !protected_blocks.contains(&uuid) {
...
fn table_id_by_path(storage_prefixes: &StoragePrefixes, path: &str) -> Result<u64> {
    // Match against the canonical owner prefixes captured in `StoragePrefixes`.
    // We intentionally do not parse a fixed `"<db_id>/<table_id>/"` layout here because
    // `parse_storage_prefix_from_table_info()` may also yield a custom storage prefix.
    //
    // `starts_with()` is sufficient for the current branch cleanup design because the prefixes in
    // this map are expected to be non-overlapping owner roots for the base table and its
    // branches. If that invariant changes, this resolution logic must be updated accordingly.
...
/// Parse a UUID hex string (with or without dashes) into u128 without allocation.
fn parse_uuid_hex_to_u128(s: &str) -> Result<u128> {
    ...
    for b in s.bytes() {
        ...
        result = result.checked_shl(4).ok_or_else(|| {
            ErrorCode::Internal(format!("UUID hex string '{}' overflows u128", s))
        })? | digit as u128;
    }
    Ok(result)
}
