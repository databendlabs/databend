...
async fn collect_base_referenced_virtual_locations(
    ...
    let Some(mut segments) = fuse_table
        .get_snapshot_referenced_segments(ctx.clone(), |status| ctx.set_status_info(&status), true)
        .await?
    else {
        return Ok(None);
    };
...
async fn collect_branch_and_base_referenced_virtual_locations(
    ...
    let branch_segment_prefix = branch_table
        .meta_location_generator()
        .segment_location_prefix();
    let (branch_segments, _base_segments) =
    ...
    Ok(Some((branch_virtual_locations, base_virtual_locations)))
}
...
async fn vacuum_virtual_column_orphans(
    ...
    let branch_tables = collect_retainable_history_branch_tables(fuse_table, &ctx).await?;
    let base_segment_prefix = fuse_table
        .meta_location_generator()
        .segment_location_prefix()
        .to_string();
    ...
    for branch_table in branch_tables {
        ...
        let Some((branch_refs, base_refs)) = collect_branch_and_base_referenced_virtual_locations(
            ctx.clone(),
            &branch_table,
