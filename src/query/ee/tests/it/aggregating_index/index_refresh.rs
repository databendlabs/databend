...
async fn test_manual_refresh_agg_index() -> Result<()> {
    let fixture = TestFixture::setup_with_custom(EESetup::new()).await?;
    fixture
        .default_session()
        .get_settings()
        .set_manual_refresh_aggregating_index(true)?;
    fixture
    ...
    let ctx = fixture.new_query_ctx().await?;
    let index_id = create_index(ctx, index_name, original_query, query.as_str()).await?;
...
#[tokio::test(flavor = "multi_thread")]
async fn test_manual_refresh_agg_index_with_limit() -> Result<()> {
    let fixture = TestFixture::setup_with_custom(EESetup::new()).await?;
    fixture
        .default_session()
        .get_settings()
...
async fn test_sync_agg_index_after_update() -> Result<()> {
    ...
    let ctx = fixture.new_query_ctx().await?;
    let index_id0 = create_index(ctx, index_name, original_query, query.as_str()).await?;
...
async fn test_sync_agg_index_after_insert() -> Result<()> {
    ...
    let ctx = fixture.new_query_ctx().await?;
    let index_id1 = create_index(ctx, index_name, original_query, query.as_str()).await?;
