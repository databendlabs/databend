...
async fn test_fuse_do_refresh_vector_index() -> Result<()> {
    ...
    let res = handler.do_create_table_index(catalog.clone(), req1).await;
    assert!(res.is_ok());
    ...
    let res = handler.do_create_table_index(catalog.clone(), req2).await;
    assert!(res.is_ok());
    ...
    let res = handler.do_create_table_index(catalog.clone(), req3).await;
    assert!(res.is_ok());
