...
async fn test_vacuum2_preserves_clone_only_data() -> anyhow::Result<()> {
    ...
    for sql in [
        format!("INSERT INTO {database}.{clone_name} VALUES (3)"),
        format!("INSERT INTO {database}.{source_name} VALUES (4)"),
    ...
    let advanced_clone_lvt = catalog
        .get_table_lvt(&LeastVisibleTimeIdent::new(&tenant, clone_id))
        .await?
        .expect("subsequent ancestor VACUUM must retain the clone LVT");
    assert!(advanced_clone_lvt.data.time >= clone_lvt.data.time);
    assert!(advanced_clone_lvt.seq > clone_lvt.seq);

    fixture
        .execute_command(&format!("VACUUM TABLE {database}.{clone_name}"))
        .await?;
    let own_lvt = catalog
        .get_table_lvt(&LeastVisibleTimeIdent::new(&tenant, clone_id))
        .await?
        .expect("the clone's own VACUUM must publish its LVT");
    assert!(own_lvt.data.time > clone_timestamp);
...
async fn test_clone_tag_scan_after_lvt_publication() -> anyhow::Result<()> {
    ...
    let concurrency = VacuumConcurrency::new(1);
    let mark = source
        .mark_clone_descendants(ctx.clone(), &catalog, &mut segments, &concurrency, |_| {})
        .await?;
    assert!(
        segments.is_empty(),
    ...
    catalog
        ...
        .await?;
    // Tag creation does not touch the head fence, so publication succeeds. The post-scan
    // must protect the tag even though its snapshot now predates the published lower bound.
    ...
    FuseTable::extend_clone_descendant_tag_segments(&catalog, &mark.tag_sources, &mut segments)
        .await?;
    assert_eq!(
        segments,
        tag_snapshot
...
async fn test_clone_forward_commit_keeps_gc_mark_valid() -> anyhow::Result<()> {
    ...
    let concurrency = VacuumConcurrency::new(1);
    let marks = source
        .mark_clone_descendants(ctx.clone(), &catalog, &mut protected, &concurrency, |_| {})
        .await?;
...
-> anyhow::Result<()> {
    ...
    assert!(
        !catalog
            .set_table_lvts(&tenant, &[(source.get_id(), 0, selection.lvt)])
            .await?
    );
    Ok(())
}
...
async fn test_vacuum_incomplete_descendant_mark_does_not_sweep() -> anyhow::Result<()> {
    ...
    fixture
        .default_session()
        .get_settings()
        .set_setting("max_vacuum_threads".into(), "3".into())?;
