...
use databend_enterprise_query::storages::fuse::vacuum_drop_tables_with_safe_clone_tables;
...
async fn test_vacuum_drop_clone_group_reclaims_only_authorized_leaf() -> anyhow::Result<()> {
    ...
    let (_, failed) = vacuum_drop_tables_with_safe_clone_tables(
        1,
        vec![malformed],
        None,
        HashSet::from([source.get_id()]),
    )
    .await?;
    ...
    let (_, failed) =
        vacuum_drop_tables_with_safe_clone_tables(1, vec![source.clone()], None, HashSet::new())
            .await?;
    assert_eq!(failed, HashSet::from([source.get_id()]));
    source_fuse.get_operator_ref().stat(&source_marker).await?;
...
async fn test_vacuum_drop_clone_cleans_metadata() -> anyhow::Result<()> {
    ...
    // The SQL coordinator must discover that the dropped clone is a safe leaf, reclaim only its
    // directory, and then remove its TableMeta and lineage binding.
    assert_eq!(
        clone_fuse
            .get_operator_ref()
            .stat(&clone_marker)
    ...
    fixture
        ...
        .await?;
    // LIMIT expands the selected clone group, allowing its leaf to be reclaimed without starving
    // the deferred source. The leaf's LVT is removed with it; the source LVT remains independent.
    fixture.execute_command("VACUUM DROP TABLE LIMIT 1").await?;
    assert!(meta.get_pb(&TableId::new(source.get_id())).await?.is_some());
