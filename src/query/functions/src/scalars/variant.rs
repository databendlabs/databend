    ...
    registry.register_passthrough_nullable_1_arg::<GenericType<0>, VariantType, _, _>(
        ...
        |val, ctx| match val {
            ValueRef::Scalar(scalar) => {
                let mut buf = Vec::new();
                cast_scalar_to_variant(scalar, ctx.func_ctx.tz, &ctx.func_ctx.tsf, &mut buf);
                Value::Scalar(buf)
            }
            ValueRef::Column(col) => {
                let new_col =
                    cast_scalars_to_variants(col.iter(), ctx.func_ctx.tz, &ctx.func_ctx.tsf);
...
fn json_array_fn(args: &[ValueRef<AnyType>], ctx: &mut EvalContext) -> Value<AnyType> {
    ...
    for idx in 0..cap {
        ...
        for column in &columns {
            ...
            let mut val = vec![];
            cast_scalar_to_variant(v, ctx.func_ctx.tz, &ctx.func_ctx.tsf, &mut val);
            items.push(val);
        }
