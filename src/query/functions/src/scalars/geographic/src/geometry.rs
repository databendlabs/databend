...
pub fn register(registry: &mut FunctionRegistry) {
    ...
    geometry_binary_fn::<GeometryType>("st_union", registry, |l, r, srid| {
        let value = GeometryUnionAggOp::compute(vec![l, r])?
            .unwrap_or_else(|| Geometry::GeometryCollection(GeometryCollection::empty()));
        let ewkb = geo_to_ewkb(value, srid)?;
        Ok(ewkb)
    });

    geometry_binary_fn::<GeometryType>("st_intersection", registry, |l, r, srid| {
        let value = GeometryIntersectionAggOp::compute(vec![l, r])?
            .unwrap_or_else(|| Geometry::GeometryCollection(GeometryCollection::empty()));
        let ewkb = geo_to_ewkb(value, srid)?;
        Ok(ewkb)
    });

    geometry_binary_fn::<GeometryType>("st_difference", registry, |l, r, srid| {
        let value = GeometryDifferenceAggOp::compute(vec![l, r])?
            .unwrap_or_else(|| Geometry::GeometryCollection(GeometryCollection::empty()));
        let ewkb = geo_to_ewkb(value, srid)?;
        Ok(ewkb)
    });

    geometry_binary_fn::<GeometryType>("st_symdifference", registry, |l, r, srid| {
        let value = GeometrySymDifferenceAggOp::compute(vec![l, r])?
            .unwrap_or_else(|| Geometry::GeometryCollection(GeometryCollection::empty()));
        let ewkb = geo_to_ewkb(value, srid)?;
        Ok(ewkb)
    });
