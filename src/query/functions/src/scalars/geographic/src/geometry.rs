...
pub fn register(registry: &mut FunctionRegistry) {
    ...
    geometry_binary_combine_fn::<GeometryType>("st_union", registry, |l, r, srid| {
        let result = GeometryUnionAggOp::binary_compute(l, r)?;
        let ewkb = match result {
            Some(value) => Some(geo_to_ewkb(value, srid)?),
            None => None,
        };
        Ok(ewkb)
    });
    ...
    geometry_binary_combine_fn::<GeometryType>("st_intersection", registry, |l, r, srid| {
        let result = GeometryIntersectionAggOp::binary_compute(l, r)?;
        let ewkb = match result {
            Some(value) => Some(geo_to_ewkb(value, srid)?),
            None => None,
        };
    ...
    geometry_binary_combine_fn::<GeometryType>("st_difference", registry, |l, r, srid| {
        let result = GeometryDifferenceAggOp::binary_compute(l, r)?;
        let ewkb = match result {
            Some(value) => Some(geo_to_ewkb(value, srid)?),
            None => None,
        };
    ...
    geometry_binary_combine_fn::<GeometryType>("st_symdifference", registry, |l, r, srid| {
        let result = GeometrySymDifferenceAggOp::binary_compute(l, r)?;
        let ewkb = match result {
            Some(value) => Some(geo_to_ewkb(value, srid)?),
            None => None,
        };
