...
use databend_common_expression::geographic::GeographyDifferenceAggOp;
use databend_common_expression::geographic::GeographyIntersectionAggOp;
use databend_common_expression::geographic::GeographySymDifferenceAggOp;
use databend_common_expression::geographic::GeographyUnionAggOp;
use databend_common_expression::geographic::geography_best_srid;
use databend_common_expression::geographic::project_geography_to_best;
use databend_common_expression::geographic::unproject_geometry_from_best;
...
pub fn register(registry: &mut FunctionRegistry) {
    ...
    geography_unary_combine_fn::<GeographyType>("st_centroid", registry, |geo| {
        let Some(best) = geography_best_srid(&geo, None)? else {
            return Err(ErrorCode::GeometryError("Cannot find best srid"));
        };
        let proj = project_geography_to_best(geo, &best)?;
        match proj.centroid() {
            Some(point) => {
                let value = unproject_geometry_from_best(Geometry::from(point), &best)?;
                let ewkb = geo_to_ewkb(value, None)?;
    ...
    geography_binary_fn::<BooleanType>("st_contains", registry, |l_geo, r_geo| {
        let Some(best) = geography_best_srid(&l_geo, Some(&r_geo))? else {
            return Ok(false);
        };
        let l_proj = project_geography_to_best(l_geo, &best)?;
        let r_proj = project_geography_to_best(r_geo, &best)?;
        Ok(l_proj.contains(&r_proj))
    });

    geography_binary_fn::<BooleanType>("st_intersects", registry, |l_geo, r_geo| {
        let Some(best) = geography_best_srid(&l_geo, Some(&r_geo))? else {
            return Ok(false);
        };
        let l_proj = project_geography_to_best(l_geo, &best)?;
        let r_proj = project_geography_to_best(r_geo, &best)?;
        Ok(l_proj.intersects(&r_proj))
    });

    geography_binary_fn::<BooleanType>("st_disjoint", registry, |l_geo, r_geo| {
        let Some(best) = geography_best_srid(&l_geo, Some(&r_geo))? else {
            return Ok(false);
        };
        let l_proj = project_geography_to_best(l_geo, &best)?;
    ...
    geography_binary_fn::<BooleanType>("st_within", registry, |l_geo, r_geo| {
        let Some(best) = geography_best_srid(&l_geo, Some(&r_geo))? else {
            return Ok(false);
        };
        let l_proj = project_geography_to_best(l_geo, &best)?;
        let r_proj = project_geography_to_best(r_geo, &best)?;
        Ok(l_proj.is_within(&r_proj))
    });

    geography_binary_fn::<GeographyType>("st_union", registry, |l_geo, r_geo| {
        let value = GeographyUnionAggOp::compute(vec![l_geo, r_geo])?
            .unwrap_or_else(|| Geometry::GeometryCollection(GeometryCollection::empty()));
        let ewkb = geo_to_ewkb(value, None)?;
        Ok(Geography(ewkb))
    });

    geography_binary_fn::<GeographyType>("st_intersection", registry, |l_geo, r_geo| {
        let value = GeographyIntersectionAggOp::compute(vec![l_geo, r_geo])?
            .unwrap_or_else(|| Geometry::GeometryCollection(GeometryCollection::empty()));
        let ewkb = geo_to_ewkb(value, None)?;
        Ok(Geography(ewkb))
    });

    geography_binary_fn::<GeographyType>("st_difference", registry, |l_geo, r_geo| {
        let value = GeographyDifferenceAggOp::compute(vec![l_geo, r_geo])?
            .unwrap_or_else(|| Geometry::GeometryCollection(GeometryCollection::empty()));
        let ewkb = geo_to_ewkb(value, None)?;
        Ok(Geography(ewkb))
    });

    geography_binary_fn::<GeographyType>("st_symdifference", registry, |l_geo, r_geo| {
        let value = GeographySymDifferenceAggOp::compute(vec![l_geo, r_geo])?
            .unwrap_or_else(|| Geometry::GeometryCollection(GeometryCollection::empty()));
        let ewkb = geo_to_ewkb(value, None)?;
        Ok(Geography(ewkb))
    });
