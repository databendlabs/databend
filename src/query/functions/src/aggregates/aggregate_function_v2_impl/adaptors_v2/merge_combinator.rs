...
fn create_from_metadata(
    ...
    if !state.function_name.eq_ignore_ascii_case(nested_name)
        && !nested_aliases
            .iter()
            .any(|alias| state.function_name.eq_ignore_ascii_case(alias))
    {
...
fn create_from_legacy_state(
    ...
    let Some((nested, argument_types)) = find_legacy_signature(
        nested_name,
        params,
        state_type,
        order_by,
    ...
    let result_state_type =
        aggregate_state_data_type(nested_name, params, argument_types, state_type.clone())?;
...
fn build_nested(
    ...
    nested_builder(AggregateFunctionRequest {
        name: nested_name,
        params,
