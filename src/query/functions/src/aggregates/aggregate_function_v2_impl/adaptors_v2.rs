...
pub(super) use name_route::AggregateFunctionNameRoutePath;
...
pub(super) fn build_default_name_route_with_unary_input(
    request: AggregateFunctionRequest<'_>,
    names: &[&str],
    features: FunctionFeatures,
    returns_default_when_only_null: bool,
    build: UnaryAggregateFunctionBuildInputFns,
) -> Result<AggregateFunctionRef> {
    let route = AggregateFunctionNameRoutePath::root(request);
    if let Some(function) = try_build_default_name_route_with_unary_input(
        &route,
        names,
        features,
...
pub(super) fn build_default_name_route_with_multi_arg_build_input(
    request: AggregateFunctionRequest<'_>,
    names: &[&str],
    features: FunctionFeatures,
    returns_default_when_only_null: bool,
    build: MultiArgAggregateFunctionBuildInputFns,
) -> Result<AggregateFunctionRef> {
    let route = AggregateFunctionNameRoutePath::root(request);
    if let Some(function) = try_build_default_name_route_with_multi_arg_build_input(
        &route,
        names,
        features,
        returns_default_when_only_null,
...
pub(super) fn build_default_name_route_with_direct_input(
    request: AggregateFunctionRequest<'_>,
    names: &[&str],
    features: FunctionFeatures,
    returns_default_when_only_null: bool,
    build: DirectAggregateFunctionBuildInputFns,
) -> Result<AggregateFunctionRef> {
    let route = AggregateFunctionNameRoutePath::root(request);
    if let Some(function) = try_build_default_name_route_with_direct_input(
        &route,
        names,
        features,
        returns_default_when_only_null,
...
fn try_build_default_name_route_with_unary_input(
    route: &AggregateFunctionNameRoutePath<'_>,
    names: &[&str],
    features: FunctionFeatures,
    returns_default_when_only_null: bool,
    build: UnaryAggregateFunctionBuildInputFns,
) -> Result<Option<AggregateFunctionRef>> {
    if let Some(route) = route.names(names) {
        if let Some(function) = route.plain_null_argument_result(returns_default_when_only_null)? {
            return Ok(Some(function));
        }
        return route
            .plain_or_null()
            .build_with_unary_input(features, build.plain)
            .map(Some);
    }
    if let Some(route) = route.suffixed_names(names, "if") {
        if let Some(function) = route.null_argument_result(returns_default_when_only_null)? {
            return Ok(Some(function));
        }
        return route
            .if_combinator(
                NullPolicy::returns_default(returns_default_when_only_null),
                true,
            )?
            .build_with_unary_input(features, build.if_combinator)
            .map(Some);
    }
    if let Some(route) = route.suffixed_names(names, "state") {
        ...
        return route
            ...
            .build_with_unary_input(features, build.state)
            .map(Some);
    }
    if let Some(route) = route.suffixed_names(names, "distinct") {
        return match build.distinct {
            UnaryDistinctBuildFn::Combinator(distinct) => route
                .distinct_combinator(
                    NullPolicy::returns_default(returns_default_when_only_null),
                    true,
                )
                .build_with_unary_input(features, distinct)
                .map(Some),
            UnaryDistinctBuildFn::PlainAlias(alias) => {
                if let Some(function) =
                    route.plain_null_argument_result(returns_default_when_only_null)?
                {
                    return Ok(Some(function));
                }
                route
                    .plain_or_null()
                    .build_with_unary_input(features, alias)
                    .map(Some)
            }
        };
    }
...
fn try_build_default_name_route_with_multi_arg_build_input(
    route: &AggregateFunctionNameRoutePath<'_>,
    names: &[&str],
    features: FunctionFeatures,
    returns_default_when_only_null: bool,
    ...
    if let Some(route) = route.names(names) {
        ...
        return route
            .plain_or_null()
            .build_with_multi_arg_input(features, build.plain)
            .map(Some);
    }
    if let Some(route) = route.suffixed_names(names, "if") {
        if let Some(function) =
            route.if_nullable_input_null_argument_result(returns_default_when_only_null)?
        {
            return Ok(Some(function));
        }
        return route
            ...
            .build_with_multi_arg_input(features, build.if_combinator)
            .map(Some);
    }
    if let Some(route) = route.suffixed_names(names, "state") {
        if let Some(function) = route.state_null_argument_result()? {
        ...
        return route
            .state_combinator(state_plan)
            .build_with_multi_arg_input(features, build.state)
            .map(Some);
    }
    if let Some(route) = route.suffixed_names(names, "distinct") {
        return match build.distinct {
            MultiArgDistinctBuildFn::Combinator(distinct) => {
                if let Some(function) =
                    route.null_argument_result(returns_default_when_only_null)?
                {
                    return Ok(Some(function));
                }
                route
                    ...
                    .build_with_multi_arg_input(features, distinct)
                    .map(Some)
            }
            MultiArgDistinctBuildFn::PlainAlias(alias) => {
                if let Some(function) =
                    route.null_argument_result(returns_default_when_only_null)?
                {
                    return Ok(Some(function));
                ...
                route
                    .plain_or_null()
                    .build_with_multi_arg_input(features, alias)
                    .map(Some)
            }
        };
    }
...
fn try_build_default_name_route_with_direct_input(
    route: &AggregateFunctionNameRoutePath<'_>,
    names: &[&str],
    features: FunctionFeatures,
    returns_default_when_only_null: bool,
    ...
    if let Some(route) = route.names(names) {
        ...
        return route
            .plain_or_null()
            .build_with_direct_input(features, build.plain)
            .map(Some);
    }
    if let Some(route) = route.suffixed_names(names, "if") {
        if let Some(function) = route.null_argument_result(returns_default_when_only_null)? {
        ...
        return route
            ...
            .build_with_direct_input(features, build.if_combinator)
            .map(Some);
    }
    if let Some(route) = route.suffixed_names(names, "state") {
        if let Some(function) = route.state_null_argument_result()? {
        ...
        return route
            .state_combinator(state_plan)
            .build_with_direct_input(features, build.state)
            .map(Some);
    }
    if let Some(route) = route.suffixed_names(names, "distinct") {
        return match build.distinct {
            DirectDistinctBuildFn::Combinator(distinct) => route
                .distinct_combinator(
                    NullPolicy::returns_default(returns_default_when_only_null),
                    true,
                )
                .build_with_direct_input(features, distinct)
                .map(Some),
            DirectDistinctBuildFn::PlainAlias(alias) => {
                if let Some(function) =
                    route.plain_null_argument_result(returns_default_when_only_null)?
                {
                    return Ok(Some(function));
                ...
                route
                    .plain_or_null()
                    .build_with_direct_input(features, alias)
                    .map(Some)
            }
        };
    }
...
pub(crate) struct KeepNullNameRoute<'a> {
    pub(crate) names: &'a [&'a str],
    pub(crate) features: FunctionFeatures,
    pub(super) build: DirectAggregateFunctionBuildInputFns,
}

pub(crate) fn build_default_name_route_keep_nulls(
    request: AggregateFunctionRequest<'_>,
    routes: &[KeepNullNameRoute<'_>],
) -> Result<AggregateFunctionRef> {
    let route = AggregateFunctionNameRoutePath::root(request);
    for route_case in routes {
        if let Some(function) = try_build_default_name_route_keep_nulls(&route, route_case)? {
            return Ok(function);
        }
    }
    route.unknown()
...
fn try_build_default_name_route_keep_nulls(
    route: &AggregateFunctionNameRoutePath<'_>,
    route_case: &KeepNullNameRoute<'_>,
) -> Result<Option<AggregateFunctionRef>> {
    if let Some(route) = route.names(route_case.names) {
        return route
            .build_with_direct_input(route_case.features.clone(), route_case.build.plain)
            .map(Some);
    }
    if let Some(route) = route.suffixed_names(route_case.names, "if") {
        return route
            .if_combinator(NullPolicy::Keep, false)?
            .build_with_direct_input(route_case.features.clone(), route_case.build.if_combinator)
            .map(Some);
    }
    if let Some(route) = route.suffixed_names(route_case.names, "state") {
        return route
            .state_combinator(StateCombinatorPlan::default())
            .build_with_direct_input(route_case.features.clone(), route_case.build.state)
            .map(Some);
    }
    if let Some(route) = route.suffixed_names(route_case.names, "distinct") {
        return match route_case.build.distinct {
            DirectDistinctBuildFn::Combinator(distinct) => route
                .distinct_combinator(NullPolicy::Keep, false)
                .build_with_direct_input(route_case.features.clone(), distinct)
                .map(Some),
            DirectDistinctBuildFn::PlainAlias(alias) => route
                .build_with_direct_input(route_case.features.clone(), alias)
                .map(Some),
        };
    }
    Ok(None)
...
impl NullPolicy {
    pub(crate) fn returns_default(returns_default_when_only_null: bool) -> Self {
        if returns_default_when_only_null {
            Self::ReturnsDefaultWhenOnlyNull
        } else {
            Self::Skip
        }
    }
...
#[derive(Clone, Copy)]
pub(super) enum UnaryDistinctBuildFn {
    Combinator(UnaryBuildFn<DistinctCombinator>),
    PlainAlias(UnaryBuildFn<PlainCombinator>),
}

#[derive(Clone, Copy)]
pub(super) enum MultiArgDistinctBuildFn {
    Combinator(MultiArgBuildFn<DistinctCombinator>),
    PlainAlias(MultiArgBuildFn<PlainCombinator>),
}

#[derive(Clone, Copy)]
pub(super) enum DirectDistinctBuildFn {
    Combinator(DirectBuildFn<DistinctCombinator>),
    PlainAlias(DirectBuildFn<PlainCombinator>),
}

pub(super) fn unsupported_unary_distinct(
    build: UnaryBuildContext<'_, DistinctCombinator>,
) -> Result<AggregateFunctionRef> {
...
pub(super) fn unsupported_multi_arg_distinct(
    build: MultiArgBuildContext<'_, DistinctCombinator>,
) -> Result<AggregateFunctionRef> {
    Err(ErrorCode::UnknownAggregateFunction(format!(
        "{} is not registered",
        build.request.name
...
pub(super) fn unsupported_direct_distinct(
    build: DirectBuildContext<'_, DistinctCombinator>,
) -> Result<AggregateFunctionRef> {
    Err(ErrorCode::UnknownAggregateFunction(format!(
        "{} is not registered",
        build.request.name
...
#[derive(Clone, Copy)]
pub(super) struct UnaryAggregateFunctionBuildInputFns {
    pub(super) plain: UnaryBuildFn<PlainCombinator>,
    pub(super) if_combinator: UnaryBuildFn<IfCombinator>,
    pub(super) state: UnaryBuildFn<StateCombinator>,
    pub(super) distinct: UnaryDistinctBuildFn,
}

impl UnaryAggregateFunctionBuildInputFns {
    pub(super) fn new(
        plain: UnaryBuildFn<PlainCombinator>,
        if_combinator: UnaryBuildFn<IfCombinator>,
        state: UnaryBuildFn<StateCombinator>,
        distinct: UnaryDistinctBuildFn,
    ) -> Self {
        Self {
            plain,
            if_combinator,
...
#[derive(Clone, Copy)]
pub(super) struct MultiArgAggregateFunctionBuildInputFns {
    pub(super) plain: MultiArgBuildFn<PlainCombinator>,
    pub(super) if_combinator: MultiArgBuildFn<IfCombinator>,
    pub(super) state: MultiArgBuildFn<StateCombinator>,
    pub(super) distinct: MultiArgDistinctBuildFn,
}

impl MultiArgAggregateFunctionBuildInputFns {
    pub(super) fn new(
        plain: MultiArgBuildFn<PlainCombinator>,
        if_combinator: MultiArgBuildFn<IfCombinator>,
        state: MultiArgBuildFn<StateCombinator>,
        distinct: MultiArgDistinctBuildFn,
    ) -> Self {
        Self {
            plain,
            if_combinator,
...
#[derive(Clone, Copy)]
pub(super) struct DirectAggregateFunctionBuildInputFns {
    pub(super) plain: DirectBuildFn<PlainCombinator>,
    pub(super) if_combinator: DirectBuildFn<IfCombinator>,
    pub(super) state: DirectBuildFn<StateCombinator>,
    pub(super) distinct: DirectDistinctBuildFn,
}

impl DirectAggregateFunctionBuildInputFns {
    pub(super) fn new(
        plain: DirectBuildFn<PlainCombinator>,
        if_combinator: DirectBuildFn<IfCombinator>,
        state: DirectBuildFn<StateCombinator>,
        distinct: DirectDistinctBuildFn,
    ) -> Self {
        Self {
            plain,
            if_combinator,
