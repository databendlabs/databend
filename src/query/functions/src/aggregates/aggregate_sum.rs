    ...
    for<'a> T::ScalarRef<'a>: Number + AsPrimitive<N::Scalar>,
{
    ...
    fn merge_result(
        ...
        N::push_item_mut(&mut builder, N::to_scalar_ref(&self.value));
        Ok(())
    }
}
...
where T: Decimal<U64Array: BorshSerialize + BorshDeserialize> + std::ops::AddAssign
{
    ...
    fn merge_result(
        &mut self,
        mut builder: BuilderMut<'_, Vec<T>>,
        _function_data: Option<&dyn FunctionData>,
    ) -> Result<()> {
...
impl UnaryState<IntervalType, IntervalType> for IntervalSumState {
    ...
    fn merge_result(
        &mut self,
        mut builder: BuilderMut<'_, Vec<months_days_micros>>,
        _function_data: Option<&dyn FunctionData>,
    ) -> Result<()> {
        IntervalType::push_item_mut(&mut builder, IntervalType::to_scalar_ref(&self.value));
        Ok(())
    }
}
