...
use databend_common_expression::geographic::GeographyIntersectionAggOp;
use databend_common_expression::geographic::GeographyUnionAggOp;
...
fn try_create_aggregate_geographic_agg_function<GeomState, GeogState>(
    display_name: &str,
    params: Vec<Scalar>,
...
where
    ...
    GeogState: ScalarStateFunc<GeographyType>,
{
    ...
    match arg_type {
        ...
        DataType::Geography => {
            let return_type = DataType::Nullable(Box::new(DataType::Geography));
            AggregateGeographicAggFunction::<GeographyType, GeogState>::try_create(
                display_name,
                return_type,
            )
        }
        _ => Err(ErrorCode::BadDataValueType(format!(
            "The argument of aggregate function {display_name} must be Geometry or Geography",
        ))),
    }
}
...
pub fn aggregate_st_union_agg_function_desc() -> AggregateFunctionDescription {
    ...
    type GeogState = GeographicAggState<GeographyType, GeographyUnionAggOp>;
    AggregateFunctionDescription::creator_with_features(
        Box::new(try_create_aggregate_geographic_agg_function::<GeomState, GeogState>),
...
pub fn aggregate_st_intersection_agg_function_desc() -> AggregateFunctionDescription {
    ...
    type GeogState = GeographicAggState<GeographyType, GeographyIntersectionAggOp>;
    AggregateFunctionDescription::creator_with_features(
        Box::new(try_create_aggregate_geographic_agg_function::<GeomState, GeogState>),
        AggregateFunctionFeatures {
            ..Default::default()
...
pub fn aggregate_st_envelope_agg_function_desc() -> AggregateFunctionDescription {
    AggregateFunctionDescription::creator_with_features(
        Box::new(|display_name, params, arguments, sort_descs| {
            let _ = sort_descs;
            assert_params(display_name, params.len(), 0)?;
            assert_unary_arguments(display_name, arguments.len())?;
            let arg_type = arguments[0].remove_nullable();
            // st_envelope_agg function don't support Geography type.
            if arg_type != DataType::Geometry && arguments[0] != DataType::Null {
                return Err(ErrorCode::BadDataValueType(format!(
                    "The argument of aggregate function {display_name} must be Geometry",
                )));
            }
            let return_type = DataType::Nullable(Box::new(DataType::Geometry));
            AggregateGeographicAggFunction::<GeometryType, GeometryAggState<EnvelopeAggOp>>::try_create(
                display_name,
                return_type,
            )
        }),
...
pub fn aggregate_st_collect_function_desc() -> AggregateFunctionDescription {
    ...
    type GeogState = GeographicAggState<GeographyType, CollectAggOp>;
    AggregateFunctionDescription::creator_with_features(
        Box::new(try_create_aggregate_geographic_agg_function::<GeomState, GeogState>),
        AggregateFunctionFeatures {
            ..Default::default()
