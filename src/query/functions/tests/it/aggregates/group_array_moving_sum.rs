...
fn run_group_array_moving_sum_cases(file: &mut impl Write, simulator: impl AggregationSimulator) {
    ...
    write_aggregate_expr_case(
        file,
        "group_array_moving_sum(const_null)",
        columns,
        simulator,
        vec![],
    );
    ...
    write_aggregate_expr_case(
        file,
        "group_array_moving_sum(all_null)",
        columns,
        simulator,
        vec![],
    );
...
#[test]
fn test_group_array_moving_sum_group_by() {
    let mut mint = Mint::new("tests/it/aggregates/testdata");
    let file = &mut mint
