...
fn get_geography_example() -> Vec<(&'static str, BlockEntry)> {
    [
        (
            "geo_point",
            StringType::from_data(vec![
        ...
        (
            "geo_point_null",
            StringType::from_data_with_validity(
                vec!["POINT(10 10)", "", "POINT(30 30)", "POINT(40 40)"],
                vec![true, false, true, true],
            ),
        ),
        (
...
fn test_agg_st_union_agg(file: &mut impl Write, simulator: impl AggregationSimulator) {
    ...
    run_agg_ast(
        file,
        "st_union_agg(to_geography(geo_point))",
        get_geography_example().as_slice(),
        simulator,
        vec![],
    );
    run_agg_ast(
        file,
        "st_union_agg(to_geography(geo_polygon))",
        get_geography_example().as_slice(),
        simulator,
        vec![],
    );
    run_agg_ast(
        file,
        "st_union_agg(to_geography(geo_point_null))",
        get_geography_example().as_slice(),
        simulator,
        vec![],
    );
...
fn test_agg_st_intersection_agg(file: &mut impl Write, simulator: impl AggregationSimulator) {
    ...
    run_agg_ast(
        file,
        "st_intersection_agg(to_geography(geo_polygon))",
        get_geography_example().as_slice(),
        simulator,
        vec![],
    );
    run_agg_ast(
        file,
        "st_intersection_agg(to_geography(geo_polygon_null))",
        get_geography_example().as_slice(),
        simulator,
        vec![],
    );
...
fn test_agg_st_collect(file: &mut impl Write, simulator: impl AggregationSimulator) {
    ...
    run_agg_ast(
        file,
        "st_collect(to_geography(geo_point))",
        get_geography_example().as_slice(),
        simulator,
        vec![],
    );
    run_agg_ast(
        file,
        "st_collect(to_geography(geo_polygon))",
        get_geography_example().as_slice(),
        simulator,
        vec![],
    );
    run_agg_ast(
        file,
        "st_collect(to_geography(geo_point_null))",
        get_geography_example().as_slice(),
        simulator,
        vec![],
    );
