...
fn run_kurtosis_cases(file: &mut impl Write, simulator: impl AggregationSimulator) {
    ...
    write_aggregate_expr_case(file, "kurtosis(a)", columns, simulator, vec![]);
    write_aggregate_expr_case(file, "kurtosis(dec)", columns, simulator, vec![]);
    write_aggregate_expr_case(file, "kurtosis(flat)", columns, simulator, vec![]);
    write_aggregate_expr_case(file, "kurtosis(NULL)", columns, simulator, vec![]);
    write_aggregate_expr_case(file, "kurtosis(x_null)", columns, simulator, vec![]);
}
