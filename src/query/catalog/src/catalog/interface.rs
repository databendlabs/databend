...
pub trait Catalog: DynClone + Send + Sync + Debug {
    ...
    /// Update one table from its current metadata, routing temporary tables to the session catalog.
    /// Persistent schema changes also invalidate MV source bindings in the same meta transaction.
    async fn update_single_table_meta(
            ...
            .contains_key(OPT_KEY_TEMP_PREFIX)
        {
            ...
            let mut new_table_info = current_table_info.clone();
            new_table_info.meta = req.new_table_meta.clone();
            update_table_metas.push((req, new_table_info));
        }
        self.update_multi_table_meta(UpdateMultiTableMetaReq {
            update_table_metas,
            update_mv_source_bindings,
