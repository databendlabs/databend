...
pub trait StateMachineApiExt: StateMachineApi {
    ...
    async fn upsert_kv_primary_index(
        ...
        upsert_kv: &UpsertKV,
        cmd_ctx: &CmdContext,
    ) -> Result<(SeqMarkedMetaValue, SeqMarkedMetaValue), io::Error> {
        let kv_meta = upsert_kv.value_meta.as_ref().map(|m| m.to_kv_meta(cmd_ctx));

        let prev = self
            .map_ref()
            .str_map()
            .get(UserKey::from_str(&upsert_kv.key))
            .await?
            .clone();
    ...
    async fn list_kv(&self, prefix: &str) -> Result<IOResultStream<(String, SeqV)>, io::Error> {
        ...
        let strm = if let Some(right) = prefix_right_bound(&p) {
            ...
            self.map_ref().str_map().range(UserKey::new(&p)..).await?
        };
    ...
    async fn update_expire_index(
        ...
        removed: &SeqMarked<(Option<KVMeta>, Vec<u8>)>,
        added: &SeqMarked<(Option<KVMeta>, Vec<u8>)>,
    ) -> Result<(), io::Error> {
    ...
    async fn get_maybe_expired_kv(&self, key: &String) -> Result<Option<SeqV>, io::Error> {
        let got = self.map_ref().str_map().get(key.as_ref()).await?;
...
fn marked_to_seqv(k: UserKey, marked: MarkedOf<UserKey>) -> Option<(String, SeqV)> {
