    ...
    pub fn import_raft_store_entry(&mut self, entry: RaftStoreEntry) -> Result<(), io::Error> {
        match entry {
            ...
            RaftStoreEntry::Logs { key: _, value } => {
                ...
                // Ensure the next log index is last.index + 1
                // This is done only once.
                let last = self.raft_log.log_state().last().map(Cw::to_inner);
                if last.is_none() && log_id.index != 0 {
                    let mut state = self.raft_log.log_state().clone();

                    let mut prev = log_id.clone();
                    prev.index -= 1;
                    state.set_last(Some(Cw(prev)));
                    self.raft_log.update_state(state)?;
                }
