...
async fn test_single_level() -> anyhow::Result<()> {
    ...
    // Delete a3
    let (prev, result) = l.str_map_mut().set(user_key("a3"), None).await?;
    ...
    // Range
    let strm = l.str_map().range(user_key("")..).await?;
    ...
    // Get
    let got = l.str_map().get(&user_key("a2")).await?;
    assert_eq!(got, SeqMarked::new_normal(2, (None, b("b2"))));

    let got = l.str_map().get(&user_key("a3")).await?;
...
async fn test_two_levels() -> anyhow::Result<()> {
    ...
    let it = l.str_map().range(user_key("")..).await?;
    ...
    let (prev, result) = l.str_map_mut().set(user_key("a1"), None).await?;
    assert_eq!(prev, SeqMarked::new_normal(1, (None, b("b1"))));
    ...
    let strm = immutables.str_map().range(user_key("")..).await?;
    let got = strm.try_collect::<Vec<_>>().await?;
    assert_eq!(got, vec![
        //
        (user_key("a1"), SeqMarked::new_normal(1, (None, b("b1")))),
...
async fn build_3_levels() -> anyhow::Result<LeveledMap> {
    ...
    l.freeze_writable();
    // internal_seq: 4
    l.str_map_mut().set(user_key("b"), None).await?;
    l.str_map_mut()
    ...
    l.freeze_writable();
    // internal_seq: 6
    l.str_map_mut().set(user_key("c"), None).await?;
    l.str_map_mut()
...
async fn test_three_levels_get_range() -> anyhow::Result<()> {
    ...
    let got = l.str_map().get(&user_key("a")).await?;
    assert_eq!(got, SeqMarked::new_normal(1, (None, b("a0"))));

    let got = l.str_map().get(&user_key("b")).await?;
    assert_eq!(got, SeqMarked::new_tombstone(4));

    let got = l.str_map().get(&user_key("c")).await?;
    assert_eq!(got, SeqMarked::new_tombstone(6));

    let got = l.str_map().get(&user_key("d")).await?;
    assert_eq!(got, SeqMarked::new_normal(7, (None, b("d2"))));

    let got = l.str_map().get(&user_key("e")).await?;
    assert_eq!(got, SeqMarked::new_normal(6, (None, b("e1"))));

    let got = l.str_map().get(&user_key("f")).await?;
    assert_eq!(got, SeqMarked::new_tombstone(0));
...
async fn test_three_levels_delete() -> anyhow::Result<()> {
    ...
    let (prev, result) = l.str_map_mut().set(user_key("a"), None).await?;
    assert_eq!(prev, SeqMarked::new_normal(1, (None, b("a0"))));
    assert_eq!(result, SeqMarked::new_tombstone(7));

    let (prev, result) = l.str_map_mut().set(user_key("b"), None).await?;
    assert_eq!(prev, SeqMarked::new_tombstone(4));
    assert_eq!(result, SeqMarked::new_tombstone(7));

    let (prev, result) = l.str_map_mut().set(user_key("c"), None).await?;
    assert_eq!(prev, SeqMarked::new_tombstone(6));
    assert_eq!(result, SeqMarked::new_tombstone(7));

    let (prev, result) = l.str_map_mut().set(user_key("d"), None).await?;
    assert_eq!(prev, SeqMarked::new_normal(7, (None, b("d2"))));
    assert_eq!(result, SeqMarked::new_tombstone(7));

    let (prev, result) = l.str_map_mut().set(user_key("e"), None).await?;
    assert_eq!(prev, SeqMarked::new_normal(6, (None, b("e1"))));
    assert_eq!(result, SeqMarked::new_tombstone(7));

    let (prev, result) = l.str_map_mut().set(user_key("f"), None).await?;
    assert_eq!(prev, SeqMarked::new_tombstone(0));
