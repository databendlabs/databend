...
impl ValueConvert<Vec<u8>> for MetaValue {
    fn conv_to(self) -> Result<Vec<u8>, io::Error> {
        let (meta, value) = self;
    ...
    fn conv_from(data: Vec<u8>) -> Result<Self, io::Error> {
        // version, meta, value
...
mod tests {
    ...
    fn t_try_from(marked: MetaValue, seq_marked: Vec<u8>) {
        let got: Vec<u8> = marked.clone().conv_to().unwrap();
        assert_eq!(seq_marked, got);

        let got = MetaValue::conv_from(got).unwrap();
    ...
    fn t_invalid(data: Vec<u8>, want_err: impl ToString) {
        let res = MetaValue::conv_from(data);
