...
/// A staged snapshot-based branch must remain publishable for at least this long.
///
/// The same lower bound is used by both vacuum final-GC eligibility and
/// commit_table_branch_meta() retention gating so a staged orphan cannot be
/// reclaimed and then republished concurrently.
pub const STAGED_BRANCH_MIN_LIFETIME: TimeDelta = TimeDelta::minutes(10);
