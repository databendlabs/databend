...
/// Reverse index keyed by tag id first so drops can guard references transactionally.
///
/// Key format: `__fd_tag_object_ref/<tenant>/<tag_id>/<object_type>/<object_id>`.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct TagRefByIdName {
    pub tag_id: u64,
    pub object: TaggableObject,
}
...
impl KeyCodec for TagRefByIdName {
    fn encode_key(&self, b: KeyBuilder) -> KeyBuilder {
...
pub type TagRefByIdIdent = TIdent<ByIdResource, TagRefByIdName>;
pub type TagRefByIdIdentRaw = TIdentRaw<ByIdResource, TagRefByIdName>;
...
mod tests {
    ...
    fn test_tag_ref_by_id_ident_roundtrip() {
        ...
        let name = TagRefByIdName::new(42, TaggableObject::Table {
            db_id: 10,
            table_id: 22,
        });
