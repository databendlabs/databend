...
impl FromToProto for mt::ShareGrantObject {
    ...
    fn to_pb(&self) -> Result<pb::ShareGrantObject, Incompatible> {
        let object = match self {
            ...
            mt::ShareGrantObject::View(_table_id) => {
                unreachable!()
            }
        };
...
impl FromToProto for mt::ShareMetaV1 {
    type PB = pb::ShareMetaV1;
    fn get_pb_ver(p: &Self::PB) -> u64 {
        p.ver
    }
    fn from_pb(p: pb::ShareMetaV1) -> Result<Self, Incompatible>
    where Self: Sized {
        reader_check_msg(p.ver, p.min_reader_ver)?;
    ...
    fn to_pb(&self) -> Result<pb::ShareMetaV1, Incompatible> {
