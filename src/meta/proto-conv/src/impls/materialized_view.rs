...
impl FromToProto for mt::MVSourceIndex {
    type PB = pb::MvSourceIndex;
    ...
    fn from_pb(p: Self::PB) -> Result<Self, Incompatible> {
        ...
        let mv_ids = p.mv_ids.into_iter().map(mt::MVId::new).collect();
        Self::try_from_ids(mv_ids).ok_or_else(|| {
            Incompatible::new("MVSourceIndex.mv_ids must be sorted and unique".to_string())
        })
    }
    ...
    fn to_pb(&self) -> Self::PB {
        pb::MvSourceIndex {
            ver: VER,
            min_reader_ver: MIN_READER_VER,
            mv_ids: self.mv_ids().iter().map(|id| **id).collect(),
        }
    }
}
