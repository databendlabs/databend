// Copyright 2021 Datafuse Labs
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

//! This mod is the key point about compatibility.
//! Everytime update anything in this file, update the `VER` and let the tests pass.

use databend_common_meta_app as mt;
use databend_common_protos::pb;

use crate::reader_check_msg;
use crate::FromToProto;
use crate::Incompatible;
use crate::MIN_READER_VER;
use crate::VER;

impl FromToProto for mt::principal::OwnershipInfo {
    type PB = pb::OwnershipInfo;
    fn get_pb_ver(p: &Self::PB) -> u64 {
        p.ver
    }
    fn from_pb(p: pb::OwnershipInfo) -> Result<Self, Incompatible>
    where Self: Sized {
        reader_check_msg(p.ver, p.min_reader_ver)?;

        Ok(mt::principal::OwnershipInfo {
            role: p.role.clone(),
            object: mt::principal::OwnershipObject::from_pb(p.object.ok_or_else(|| {
                Incompatible {
                    reason: "Object can not be None".to_string(),
                }
            })?)?,
        })
    }

    fn to_pb(&self) -> Result<pb::OwnershipInfo, Incompatible> {
        Ok(pb::OwnershipInfo {
            ver: VER,
            min_reader_ver: MIN_READER_VER,
            role: self.role.clone(),
            object: Some(self.object.to_pb()?),
        })
    }
}

impl FromToProto for mt::principal::OwnershipObject {
    type PB = pb::OwnershipObject;
    fn get_pb_ver(p: &Self::PB) -> u64 {
        p.ver
    }
    fn from_pb(p: pb::OwnershipObject) -> Result<Self, Incompatible>
    where Self: Sized {
        reader_check_msg(p.ver, p.min_reader_ver)?;

        match p.object {
            Some(pb::ownership_object::Object::Database(
                pb::ownership_object::OwnershipDatabaseObject { catalog, db },
            )) => Ok(mt::principal::OwnershipObject::Database(catalog, db)),
            Some(pb::ownership_object::Object::Table(
                pb::ownership_object::OwnershipTableObject { catalog, db, table },
            )) => Ok(mt::principal::OwnershipObject::Table(catalog, db, table)),
            Some(pb::ownership_object::Object::Udf(pb::ownership_object::OwnershipUdfObject {
                udf,
            })) => Ok(mt::principal::OwnershipObject::UDF(udf)),
            Some(pb::ownership_object::Object::Stage(
                pb::ownership_object::OwnershipStageObject { stage },
            )) => Ok(mt::principal::OwnershipObject::Stage(stage)),
            _ => Err(Incompatible {
                reason: "OwnershipObject cannot be None".to_string(),
            }),
        }
    }

    fn to_pb(&self) -> Result<pb::OwnershipObject, Incompatible> {
        let object = match self {
            mt::principal::OwnershipObject::Database(catalog, db) => {
                Some(pb::ownership_object::Object::Database(
                    pb::ownership_object::OwnershipDatabaseObject {
                        catalog: catalog.clone(),
                        db: *db,
                    },
                ))
            }
            mt::principal::OwnershipObject::Table(catalog, db, table) => Some(
                pb::ownership_object::Object::Table(pb::ownership_object::OwnershipTableObject {
                    catalog: catalog.clone(),
                    db: *db,
                    table: *table,
                }),
            ),
            mt::principal::OwnershipObject::UDF(udf) => Some(pb::ownership_object::Object::Udf(
                pb::ownership_object::OwnershipUdfObject { udf: udf.clone() },
            )),
            mt::principal::OwnershipObject::Stage(stage) => Some(
                pb::ownership_object::Object::Stage(pb::ownership_object::OwnershipStageObject {
                    stage: stage.clone(),
                }),
            ),
        };
        Ok(pb::OwnershipObject {
            ver: VER,
            min_reader_ver: MIN_READER_VER,
            object,
        })
    }
}
