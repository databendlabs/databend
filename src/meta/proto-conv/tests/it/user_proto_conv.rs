...
fn test_user_stage_info_gcs() -> mt::UserStageInfo {
    mt::UserStageInfo {
...
fn test_user_stage_pb_from_to() -> anyhow::Result<()> {
    ...
    let test_user_stage_info_gcs = test_user_stage_info_gcs();
    let test_user_stage_info_pb = test_user_stage_info_gcs.to_pb()?;
    let got = mt::UserStageInfo::from_pb(test_user_stage_info_pb)?;
    assert_eq!(got, test_user_stage_info_gcs);
...
fn test_build_user_pb_buf() -> anyhow::Result<()> {
    ...
    {
        let gcs_stage_info = test_user_stage_info_gcs();
...
fn test_load_old_user() -> anyhow::Result<()> {
    ...
    {
        let user_stage_info_v4: Vec<u8> = vec![
        ...
        let p: pb::UserStageInfo =
            common_protos::prost::Message::decode(user_stage_info_v4.as_slice())
                .map_err(print_err)?;
    ...
    {
        // Stage on Google Cloud Storage
        let user_stage_info_gcs = vec![
        ...
        let p: pb::UserStageInfo =
            common_protos::prost::Message::decode(user_stage_info_gcs.as_slice())
                .map_err(print_err)?;
        ...
        let want = test_user_stage_info_gcs();

        assert_eq!(want, got);
    }

    {
        // legacy user stage info
