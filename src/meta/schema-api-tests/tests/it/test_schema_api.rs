...
//! Test SchemaApiTestSuite against metasrv (single node and cluster).
...
#[fastrace::trace]
async fn test_schema_api_single_node() -> anyhow::Result<()> {
...
#[test(harness = meta_service_test_harness)]
#[fastrace::trace]
async fn test_schema_api_cluster() -> anyhow::Result<()> {
    let builder = MetaSrvBuilder {
        test_contexts: Arc::new(Mutex::new(vec![])),
    };
