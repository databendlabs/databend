...
async fn test_meta_store_restart() -> anyhow::Result<()> {
    ...
    info!("--- new meta store");
    {
        ...
        sto.log.clone().blocking_append([Entry::new_blank(log_id(1, 2, 1))])
            .await?;

        sto.log.clone().save_committed(Some(log_id(1, 2, 2))).await?;

        sto.state_machine.clone().apply([Entry::new_blank(log_id(1, 2, 2))]).await?;
    }
    ...
    info!("--- reopen meta store");
    {
        ...
        assert_eq!(Some(log_id(1, 2, 2)), sto.log.clone().read_committed().await?);
        assert_eq!(
            None,
...
async fn test_meta_store_current_snapshot() -> anyhow::Result<()> {
    ...
    let curr_snap = sto.state_machine.clone().get_current_snapshot().await?.unwrap();
    assert_eq!(Some(new_log_id(1, 0, 9)), curr_snap.meta.last_log_id);
...
async fn test_meta_store_install_snapshot() -> anyhow::Result<()> {
    ...
    info!("--- reopen a new metasrv to install snapshot");
    {
        ...
        {
            sto.state_machine.clone().do_install_snapshot(data.clone()).await?;
        }

        info!("--- check installed meta");
        {
            let mem = sto.state_machine().get_inner().sys_data().last_membership_ref().clone();
            ...
            let last_applied = *sto.state_machine().get_inner().sys_data().last_applied_ref();
