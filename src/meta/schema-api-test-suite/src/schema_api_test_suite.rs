...
impl SchemaApiTestSuite {
    ...
    >(
        ...
        assert!(mt.get_pb(&dropped_key).await?.is_some());
        assert!(
            mt.get_pb(&TableId {
                table_id: branch_id
            })
        ...
        assert_eq!(
            ...
            ErrorCode::from(err).code(),
            "out-of-retention branch must not be undropped"
        );
        ...
        assert!(
            removed > 0,
            "beyond-retention dropped branch should clean up metadata"
        );
        assert!(mt.get_pb(&expired_dropped_key).await?.is_none());
        assert!(
            mt.get_pb(&TableId {
                table_id: expired_branch_id
            })
            .await?
            .is_none()
        );

        let recreated_branch_name = "branch_recreated";
        let old_branch_id = self
            .create_active_branch(
                mt,
                &tenant,
                base_table.as_ref(),
                recreated_branch_name,
                None,
            )
            .await?;
        mt.drop_table_branch(DropTableBranchReq {
            tenant: tenant.clone(),
            table_id: base_table_id,
            branch_name: recreated_branch_name.to_string(),
        ...
        let recreated_branch_key = TableIdBranchName::new(base_table_id, recreated_branch_name);
        let recreated_dropped_key =
            DroppedBranchIdent::new(base_table_id, recreated_branch_name, old_branch_id);
        let removed = mt
            .gc_drop_table_branch(GcDroppedTableBranchReq {
                tenant: tenant.clone(),
                table_id: base_table_id,
        ...
        assert!(
            removed > 0,
            "gc should remove the old dropped generation metadata"
        );
        let recreated_branch = mt.get_pb(&recreated_branch_key).await?.unwrap();
        assert_eq!(
            new_branch_id, recreated_branch.data.branch_id,
            "gc must not remove a recreated branch that reuses the same name"
        );
        assert!(mt.get_pb(&recreated_dropped_key).await?.is_none());
        assert!(
            mt.get_pb(&TableId {
        ...
        let expired_retained_branch_name = "branch_expired_retain";
        let expired_retained_expire_at = Utc::now() - Duration::hours(2);
        let expired_retained_branch_id = self
            .create_active_branch(
                mt,
                &tenant,
                base_table.as_ref(),
        ...
        mt.drop_table_branch(DropTableBranchReq {
            tenant: tenant.clone(),
            table_id: base_table_id,
            branch_name: expired_retained_branch_name.to_string(),
            branch_id: expired_retained_branch_id,
        })
        .await?;
        let err = mt
            .undrop_table_branch(UndropTableBranchReq {
        ...
        assert_eq!(
            ErrorCode::ReferenceExpired("").code(),
            ErrorCode::from(err).code(),
            "expired retained branch must provide a new expiration when undropped"
        );
