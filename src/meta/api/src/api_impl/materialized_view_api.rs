...
//! `bound_source_generation == current_source_generation`. Schema/name DDL can
//! therefore invalidate all dependent MVs in O(1) by advancing only the source
//! generation.
//!
...
//!
//! schema-changing ALTER SOURCE TABLE txn:
...
//!     keep SourceTableMVIdent(tenant, source_id, *)
//!
//! DROP/UNDROP SOURCE TABLE txn:
//!     rewrite MVSourceBindingVersion with the same current_source_generation
//!     keep SourceTableMVIdent(tenant, source_id, *)
//!
//! GC MV txn:
//!     delete MVDefinitionIdent(tenant, mv_id)
