    ...
    Self: kvapi::KVApi<Error = MetaError>,
{
    ...
    async fn gc_drop_table_branch(
        ...
        let delete_ops = remove_branch_id_from_history_with_retry(
            self,
            &branch_id_history_ident,
            req.branch_id,
            req.table_id,
...
/// Remove one branch id from branch history with retry under concurrent updates.
///
/// Returns number of metadata delete operations contributed by history cleanup txn.
async fn remove_branch_id_from_history_with_retry(
    kv_api: &(impl kvapi::KVApi<Error = MetaError> + ?Sized),
    branch_id_history_ident: &BranchIdHistoryIdent,
    branch_id: u64,
    _table_id: u64,
    _branch_name: &str,
    first_txn: Option<TxnRequest>,
) -> Result<usize, KVAppError> {
    let mut trials = txn_backoff(None, func_name!());
    let mut first_txn = first_txn;

    loop {
        trials.next().unwrap()?.await;

        let txn = if let Some(txn) = first_txn.take() {
            txn
        } else {
            let mut txn = TxnRequest::default();
            let seq_id_list = kv_api.get_pb(branch_id_history_ident).await?;
            update_txn_to_remove_branch_history(
                &mut txn,
                branch_id_history_ident,
                seq_id_list,
...
async fn gc_branches_for_dropped_table(
    ...
    // Phase 2: remove branch discovery entries with retry.
    let mut trials = txn_backoff(None, func_name!());
    loop {
        trials.next().unwrap()?.await;

        let mut txn = TxnRequest::default();
        let latest_branch_histories = kv_api
            .list_pb_vec(ListOptions::unlimited(&branch_history_dir))
            .await?;
        for (history_key, _seq_id_list) in &latest_branch_histories {
