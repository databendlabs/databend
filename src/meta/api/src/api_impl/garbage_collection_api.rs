    ...
    Self: kvapi::KVApi<Error = MetaError>,
{
    ...
    async fn gc_drop_table_branch(
        ...
        // Copied-file markers are independent from the main GC txn and can be removed first.
        ...
        loop {
            ...
            // 1) Remove the branch table metadata and branch-owned auxiliary kvs.
            ...
            // 2) Remove the dropped-branch entry.
            ...
            if let Some(seq_dropped) = seq_dropped {
                txn.condition
...
async fn gc_dropped_table_by_id(
    ...
    loop {
        ...
        let Some(mut current_removed) = remove_data_for_dropped_table(
            kv_api,
            tenant,
            catalog,
        ...
        current_removed += num_meta_keys_removed;
        for op in &txn.if_then {
            if let Some(Request::Delete(_) | Request::DeleteByPrefix(_)) = &op.request {
                current_removed += 1;
            }
...
async fn remove_data_for_dropped_table(
    ...
    let dropped_branch_dir = DirName::new_with_level(dropped_branch_prefix, 1);
