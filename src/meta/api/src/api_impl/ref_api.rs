    ...
    Self: kvapi::KVApi<Error = MetaError>,
{
    ...
    async fn create_table_branch(
        ...
        loop {
            ...
            if req.as_dropped {
                // Write orphan dropped-branch entry with TTL so it auto-expires if commit fails.
    ...
    async fn drop_table_branch(&self, req: DropTableBranchReq) -> Result<(), KVAppError> {
        ...
        loop {
            ...
            let dropped_meta = DroppedBranchMeta { drop_on };
    ...
    async fn list_history_table_branches(
        ...
        let active_branches = self
            ...
            .await?;
        let active_map: HashMap<u64, (String, Option<chrono::DateTime<Utc>>)> = active_branches
            .iter()
            .map(|(key, branch)| {
                (
        ...
        let mut dropped_map: HashMap<u64, (String, chrono::DateTime<Utc>)> = HashMap::new();
    ...
    async fn undrop_table_branch(&self, req: UndropTableBranchReq) -> Result<(), KVAppError> {
        ...
        self.do_undrop_table_branch(tenant, table_id, branch_name, branch_id)
            .await
    }
    ...
    async fn undrop_table_branch_by_id(
        ...
        self.do_undrop_table_branch(&req.tenant, req.table_id, &req.branch_name, req.branch_id)
            .await
    }
