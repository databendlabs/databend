...
use crate::util::get_raw_key_value;
...
impl<KV: kvapi::KVApi<Error = MetaError> + ?Sized> SchemaApi for KV {
    ...
    async fn create_table(&self, req: CreateTableReq) -> Result<CreateTableReply, KVAppError> {
        ...
        let mut data = vec![];
        data.push(get_raw_key_value(self, &key_dbid).await?);
        data.push(get_raw_key_value(self, &key_dbid_tbname).await?);
        data.push(get_raw_key_value(self, &key_table_id_list).await?);
        let table_count = {
            let table_count = get_raw_key_value(self, &key_table_count).await?;

            // Initialize table count if it is needed
            if table_count.value.is_none() {
                init_table_count(self, &key_table_count).await?;

                // Refetch to get the latest value.
                get_raw_key_value(self, &key_table_count).await?
            } else {
                table_count
            }
        };
        data.push(table_count);
