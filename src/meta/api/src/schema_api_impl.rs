...
impl<KV: kvapi::KVApi<Error = MetaError>> SchemaApi for KV {
    ...
    async fn create_database(
        ...
        debug!(req = &req as &dyn Debug; "SchemaApi: {}", func_name!());
        ...
        while retry < TXN_MAX_RETRY_TIMES {
            ...
            debug!(db_id_seq = db_id_seq, db_id = db_id, name_key = name_key as &dyn Debug; "get_database");
            ...
            debug!(db_id = db_id, name_key = name_key as &dyn Debug; "new database id");
    ...
    async fn drop_database(&self, req: DropDatabaseReq) -> Result<DropDatabaseReply, KVAppError> {
        ...
        while retry < TXN_MAX_RETRY_TIMES {
            ...
            if removed {
                ...
                debug!(
                    name = tenant_dbname as &dyn Debug,
                    id = &DatabaseId { db_id } as &dyn Debug;
                ...
                debug!(
                    db_id = db_id,
                    name_key = tenant_dbname as &dyn Debug;
                    "drop_database"
                );
    ...
    async fn undrop_database(
        ...
        while retry < TXN_MAX_RETRY_TIMES {
            ...
            debug!(db_id = db_id, name_key = name_key as &dyn Debug; "undrop_database");
    ...
    async fn rename_database(
        ...
        while retry < TXN_MAX_RETRY_TIMES {
            ...
            debug!(
                old_db_id = old_db_id,
                tenant_dbname = tenant_dbname as &dyn Debug;
                "rename_database"
            );
            ...
            // rename database
            {
                ...
                debug!(
                    name = tenant_dbname as &dyn Debug,
                    to = &tenant_newdbname as &dyn Debug,
                    database_id = &old_db_id as &dyn Debug,
                    succ = succ;
                    "rename_database"
                );
    ...
    async fn create_index(&self, req: CreateIndexReq) -> Result<CreateIndexReply, KVAppError> {
        ...
        while retry < TXN_MAX_RETRY_TIMES {
            ...
            debug!(
                ...
                index_id = index_id,
                tenant_index = tenant_index as &dyn Debug;
                "get_index_seq_id"
            );
            ...
            debug!(
                index_id = index_id,
                index_key = tenant_index as &dyn Debug;
                "new index id"
            );

            {
                ...
                debug!(
                    index_name = tenant_index as &dyn Debug,
                    id = &id_key as &dyn Debug,
                    succ = succ;
                    "create_index"
                );
    ...
    async fn drop_index(&self, req: DropIndexReq) -> Result<DropIndexReply, KVAppError> {
        ...
        loop {
            ...
            debug!(index_id = index_id, name_key = tenant_index as &dyn Debug; "drop_index");
            ...
            debug!(
                name = tenant_index as &dyn Debug,
                id = &IndexId { index_id } as &dyn Debug,
                succ = succ;
                "drop_index"
            );
    ...
    async fn get_index(&self, req: GetIndexReq) -> Result<GetIndexReply, KVAppError> {
        ...
        debug!(
            index_id = index_id,
            name_key = tenant_index as &dyn Debug;
            "drop_index"
        );
    ...
    async fn create_virtual_column(
        ...
        loop {
            ...
            // (tenant, table_id) -> virtual_column_meta
            {
                ...
                debug!(
                    "req.name_ident" = &virtual_column_meta as &dyn Debug,
                    succ = succ;
    ...
    async fn drop_virtual_column(
        ...
        loop {
            ...
            // (tenant, table_id) -> virtual_column_meta
            {
                ...
                debug!(
                    "req.name_ident" = &req.name_ident as &dyn Debug,
                    succ = succ;
                    "drop_virtual_column"
                );
    ...
    async fn create_table(&self, req: CreateTableReq) -> Result<CreateTableReply, KVAppError> {
        ...
        while retry < TXN_MAX_RETRY_TIMES {
            ...
            debug!(
                table_id = table_id,
                name = tenant_dbname_tbname as &dyn Debug;
                "new table id"
            );
    ...
    async fn undrop_table(&self, req: UndropTableReq) -> Result<UndropTableReply, KVAppError> {
        ...
        while retry < TXN_MAX_RETRY_TIMES {
            ...
            debug!(
                ident = &tbid as &dyn Display,
                name = tenant_dbname_tbname as &dyn Display;
                "undrop table"
            );
    ...
    async fn rename_table(&self, req: RenameTableReq) -> Result<RenameTableReply, KVAppError> {
        ...
        while retry < TXN_MAX_RETRY_TIMES {
...

            {
                ...
                debug!(
                    name = tenant_dbname_tbname as &dyn Debug,
                    to = &newdbid_newtbname as &dyn Debug,
                    table_id = &table_id as &dyn Debug,
                    succ = succ;
                    "rename_table"
                );
    ...
    async fn get_table(&self, req: GetTableReq) -> Result<Arc<TableInfo>, KVAppError> {
        ...
        debug!(
            ident = &tbid as &dyn Display,
            name = tenant_dbname_tbname as &dyn Display,
            table_meta = &tb_meta as &dyn Debug;
            "get_table"
        );
    ...
    async fn get_table_history(
        ...
        for c in keys.chunks(DEFAULT_MGET_SIZE) {
            ...
            for (tb_id_list_seq, tb_id_list_opt) in tb_id_list_seq_vec {
                ...
                debug!(
                    name = &table_id_list_key as &dyn Display;
                    "get_table_history"
                );
    ...
    async fn get_table_by_id(
        ...
        debug!(req = &table_id as &dyn Debug; "SchemaApi: {}", func_name!());

        let tbid = TableId { table_id };

        let (tb_meta_seq, table_meta): (_, Option<TableMeta>) = get_pb_value(self, &tbid).await?;

        debug!(ident = &tbid as &dyn Display; "get_table_by_id");
    ...
    async fn truncate_table(
        ...
        let ctx = &func_name!();
        debug!(req = &req as &dyn Debug; "SchemaApi: {}", ctx);
    ...
    async fn upsert_table_option(
        ...
        loop {
            ...
            debug!(ident = &tbid as &dyn Display; "upsert_table_option");
    ...
    async fn update_table_meta(
        ...
        loop {
            ...
            debug!(ident = &tbid as &dyn Display; "update_table_meta");
    ...
    async fn create_catalog(
        ...
        let catalog_id = loop {
            ...
            debug!(catalog_id = catalog_id, name_key = name_key as &dyn Debug; "new catalog id");
...
fn db_has_to_not_exist(
    ...
    if seq == 0 {
        ...
        debug!(seq = seq, name_ident = name_ident as &dyn Debug; "exist");
