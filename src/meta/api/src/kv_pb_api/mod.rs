...
mod tests {
    ...
    async fn test_list_pb_values_with_limit() -> anyhow::Result<()> {
        ...
        // List with limit 3
        let res: Vec<_> = foo.list_pb_values(&dir, Some(3)).await?.try_collect().await?;
        assert_eq!(res.len(), 3);
        for v in &res {
            assert_eq!(v.catalog_option, catalog_meta.catalog_option);
        }

        // List with limit 0
        let res: Vec<_> = foo.list_pb_values(&dir, Some(0)).await?.try_collect().await?;
        assert!(res.is_empty());
