...
impl SchemaApiTestSuite {
    ...
    async fn database_create_get_drop<MT: SchemaApi + kvapi::KVApi<Error = MetaError>>(
        ...
        info!("--- get db1");
        {
            let util = Util::new(mt, tenant_name, "db1", "", "");
            let res = util.get_database().await;
        ...
        info!("--- get db2");
        {
            let util = Util::new(mt, tenant_name, "db2", "", "");
            let res = util.get_database().await?;
...
where MT: SchemaApi + kvapi::KVApi<Error = MetaError>
{
    ...
    async fn get_database_by_name(
        ...
        let req = GetDatabaseReq::new(self.tenant(), db_name);
        let res = self.mt.get_database(req).await?;
        Ok(res)
    }
    ...
    async fn get_table_by_names(
        &self,
        db_name: &str,
        table_name: &str,
    ) -> anyhow::Result<std::sync::Arc<databend_common_meta_app::schema::TableInfo>> {
        let req = GetTableReq::new(&self.tenant(), db_name, table_name);
        let res = self.mt.get_table(req).await?;
        Ok(res)
    }
