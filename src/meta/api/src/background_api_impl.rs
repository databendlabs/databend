...
impl<KV: kvapi::KVApi<Error = MetaError>> BackgroundApi for KV {
    async fn create_background_job(
        ...
        debug!(req = &req as &dyn Debug; "BackgroundApi: {}", func_name!());
        ...
        let id = loop {
            ...
            debug!(seq = seq, id = id, name_key = name_key as &dyn Debug; "create_background_job");
    ...
    async fn update_background_task(
        ...
        debug!(name_key = name_key as &dyn Debug; "update_background_task");
...
pub fn background_job_has_to_exist(
    ...
    if seq == 0 {
        debug!(seq = seq, name_ident = name_ident as &dyn Debug; "background job does not exist");
...
async fn update_background_job<F: FnOnce(&mut BackgroundJobInfo) -> bool>(
    ...
    debug!(req = name as &dyn Debug; "BackgroundApi: {}", func_name!());
