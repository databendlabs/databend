...
//! - `meta` key is planned to store the capacity of the semaphore, representing
...
//!   - `permits`: The amount of resource this entry consumes.
...
//!
//! If the sum of `permits` fields in `acquired` is less than `capacity`, new entries from the watch
...
//! acquired.  For example, consider entries `a:(seq=2,permits=1)` and `b:(seq=1,permits=1)` where
...
//!    -  It automatically cancels the two tasks when dropped.
...
//!       It gets Ready with an error [`ConnectionClosed`] if the watcher fails.
//!
//!    **Applications should poll the [`Permit`] as a Future to detect removal or watcher failure**
//! ```text
//! | Semaphore -> spawn()-. (1)
//! |     |                |
//! |     |                |
