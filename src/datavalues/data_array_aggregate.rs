...
pub fn array_sum(value: DataArrayRef) -> Result<DataValue> {
    Ok(match value.data_type() {
        ...
        _ => {
            return Err(Error::Unsupported(format!(
                "Unsupported array_sum() for data type: {:?}",
                value.data_type()
            )))
        }
    })
...
pub fn array_max(value: DataArrayRef) -> Result<DataValue> {
    Ok(match value.data_type() {
        ...
        _ => {
            return Err(Error::Unsupported(format!(
                "Unsupported array_max() for data type: {:?}",
                value.data_type()
            )))
        }
    })
