...
impl SmallValues<'_> {
    ...
    fn to_small_bitmap(self) -> SmallBitmap {
        match self {
            SmallValues::Owned(values) => values,
            SmallValues::Serialized(bytes) => bytes
...
impl HybridBitmap {
    ...
    fn bitor_assign_small(&mut self, rhs: SmallValues<'_>) {
        match self {
            ...
            HybridBitmap::Small(lhs_set) => {
                small_union(lhs_set, rhs.to_small_bitmap().as_slice());
    ...
    fn bitand_assign_small(&mut self, rhs: SmallValues<'_>) {
        match self {
            ...
            HybridBitmap::Small(lhs_set) => match rhs {
                ...
                values => {
                    let mut values = values.to_small_bitmap();
    ...
    fn bitxor_assign_small(&mut self, rhs: SmallValues<'_>) {
        match self {
            ...
            HybridBitmap::Small(lhs_set) => {
                small_symmetric_difference(lhs_set, rhs.to_small_bitmap().as_slice());
                if lhs_set.len() >= LARGE_THRESHOLD {
                    let _ = self.promote_to_tree();
                }
            }
    ...
    fn sub_assign_small(&mut self, rhs: SmallValues<'_>) {
        match self {
            ...
            HybridBitmap::Small(lhs_set) => {
                let result = small_difference(lhs_set.as_slice(), rhs.to_small_bitmap().as_slice());
                *lhs_set = result;
            }
        }
    }
...
mod tests {
    ...
    fn bitand_assign_serialized_small_rhs_uses_small_result() {
        ...
        lhs.bitand_assign_rhs(BitmapRhs::Serialized(&rhs_buf)).unwrap();
