...
fn apply_specifier(
    ...
    if specifier_str.is_empty() {
        result.micros = result.micros.checked_add(number).ok_or("Overflow")?;
        result.micros = result.micros.checked_add(fraction).ok_or("Overflow")?;
        return Ok(());
    }
    ...
    match specifier {
        ...
        DatePartSpecifier::Month => {
            result.months = result
                .months
                .checked_add(number.try_into().map_err(|_| "Overflow")?)
                .ok_or("Overflow")?;
        }
        ...
        DatePartSpecifier::Milliseconds => {
            result.micros = result
                .micros
                .checked_add(number.checked_mul(MICROS_PER_MSEC).ok_or("Overflow")?)
                .ok_or("Overflow")?;
        }
        DatePartSpecifier::Second => {
            result.micros = result
                .micros
                .checked_add(number.checked_mul(MICROS_PER_SEC).ok_or("Overflow")?)
                .ok_or("Overflow")?;
        }
        DatePartSpecifier::Minute => {
            result.micros = result
                .micros
                .checked_add(number.checked_mul(MICROS_PER_MINUTE).ok_or("Overflow")?)
                .ok_or("Overflow")?;
        }
        DatePartSpecifier::Hour => {
            result.micros = result
                .micros
                .checked_add(number.checked_mul(MICROS_PER_HOUR).ok_or("Overflow")?)
                .ok_or("Overflow")?;
        }
    }
    Ok(())
...
impl<T> BufferReadIntervalExt for Cursor<T>
where T: AsRef<[u8]>
{
    fn read_interval_text(&mut self) -> Result<Interval> {
        // Can be refactor to other file.
        todo!()
    }
}
...
mod tests {
    ...
    fn test_interval_from_string() {
        let interval = Interval::from_string("1 year 2 months 3 days").unwrap();
        ...
        let interval = Interval::from_string("1 year 2 months 3 days ago").unwrap();
        assert_eq!(interval, Interval {
            months: -14,
            days: -3,
            micros: 0
        ...
        let interval = Interval::from_string("1day").unwrap();
        assert_eq!(interval, Interval {
            months: 0,
            days: 1,
            micros: 0
        ...
        let interval = Interval::from_string("1 hour").unwrap();
        assert_eq!(interval, Interval {
            months: 0,
            days: 0,
            micros: 3600000000
        ...
        let interval = Interval::from_string("1 hours 1 second").unwrap();
        assert_eq!(interval, Interval {
            months: 0,
            days: 0,
            micros: 3601000000
        ...
        let result = Interval::from_string("1 day 01:23:45").unwrap();
        ...
        let result = Interval::from_string("2 hours 30 minutes").unwrap();
        assert_eq!(result, Interval {
            months: 0,
            days: 0,
