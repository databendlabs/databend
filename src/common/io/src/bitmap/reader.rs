...
impl<'a> TreemapReader<'a> {
    ...
    pub fn max(&self) -> io::Result<Option<u64>> {
        ...
        for bitmap_result in self.iter() {
            ...
            if let Some(low16) = container.max() {
                let prefix = bitmap.prefix() as u64;
                last_result =
                    Some((prefix << 32) | ((container.key() as u64) << 16) | low16 as u64);
            }
        }
        Ok(last_result)
    }
    ...
    pub fn len_above(&self, threshold: usize) -> io::Result<bool> {
        let mut sum = 0;
        for bitmap in self.iter() {
            let bitmap = bitmap?;
            for i in 0..bitmap.containers() {
