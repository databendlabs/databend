...
impl Roman {
    ...
    fn try_from_i64(value: i64) -> Option<Self> {
        (1..=3999).contains(&value).then(|| Self(value as u16))
    }
...
fn num_processor(nodes: &[FormatNode], desc: NumDesc, num_part: NumPart) -> Result<String> {
    ...
    if np.desc.flag.contains(NumFlag::Roman) {
        let mut out = String::from_iter(np.number.iter());
        if nodes.iter().any(|n| {
            matches!(
                n,
        ...
        return if np.desc.flag.contains(NumFlag::FillMode) {
            Ok(out)
        } else {
            Ok(format!("{out:>width$}", width = Roman::MAX_LEN))
        };
    }
