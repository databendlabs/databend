...
fn u16_as_u8_overflow() {
    ...
    let b = cast(&array, &DataType::UInt8, CastOptions {
        wrapped: true,
        ..Default::default()
    })
    .unwrap();
...
fn binary_to_i32_partial() {
    ...
    let b = cast(&array, &DataType::Int32, CastOptions {
        partial: true,
        ..Default::default()
    })
    .unwrap();
...
fn fixed_size_binary_to_binary() {
    ...
    // large-binary
    let b = cast(&array, &DataType::LargeBinary, CastOptions {
        ..Default::default()
    })
    .unwrap();
    ...
    // binary
    let b = cast(&array, &DataType::Binary, CastOptions {
        ..Default::default()
    })
    .unwrap();
    let c = b.as_any().downcast_ref::<BinaryArray<i32>>().unwrap();
