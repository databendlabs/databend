...
impl<O: Offset> ListArray<O> {
    ...
    /// * The `data_type`'s [`crate::arrow::datatypes::PhysicalType`] is not equal to either [`crate::arrow::datatypes::PhysicalType::List`] or [`crate::arrow::datatypes::PhysicalType::LargeList`].
    ...
    pub fn try_new(
        ...
        if child_data_type != values_data_type {
            return Err(Error::oos(format!(
                "ListArray's child's DataType must match. However, the expected DataType is {child_data_type:?} while it got {values_data_type:?}."
            )));
        }
