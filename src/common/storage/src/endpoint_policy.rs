...
fn normalize_host(host: &str) -> String {
    // Url::host_str() wraps IPv6 literals in `[ ]`; strip a single outer pair
    // here. Any other bracket characters are left intact rather than trimmed
    // greedily so the function's contract stays narrow.
    let host = host.trim();
    let host = host
        .strip_prefix('[')
        .and_then(|s| s.strip_suffix(']'))
