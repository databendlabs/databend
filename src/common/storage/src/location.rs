...
pub fn parse_uri_location(l: &UriLocation) -> Result<(StorageParams, String)> {
    ...
    // WARNING:
    // If your protocol requires the `endpoint_url`'s corresponding value stored in `l`,
    // and the `endpoint_url` input by user has no scheme
    //
    // Then the parser will automatically prefix it with `https://`.
    //
    // check out `src/query/ast/src/parser/statement.rs::uri_parser` for details.

    let sp = match protocol {
        Scheme::Azblob => StorageParams::Azblob(StorageAzblobConfig {
            endpoint_url: l.connection.get("endpoint_url").cloned().ok_or_else(|| {
                Error::new(
