...
fn feistel_round(x: u64, num_bits: usize, seed: u64, round: usize) -> u64 {
    ...
    let new_right_half = left_half ^ mask_bits(hash(right_half, seed, round), num_bits_left_half);
...
fn transform(x: u64, seed: u64) -> u64 {
    // Keep 0 and 1 as is.
    if x == 0 || x == 1 {
        return x;
    }
...
fn hash(x: u64, seed: u64, round: usize) -> u64 {
    let mut state = std::hash::DefaultHasher::new();
    x.hash(&mut state);
    seed.hash(&mut state);
    round.hash(&mut state);
    state.finish()
}
