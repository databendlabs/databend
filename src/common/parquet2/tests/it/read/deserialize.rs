use parquet2::deserialize::FilteredHybridBitmapIter;
use parquet2::deserialize::FilteredHybridEncoded;
use parquet2::deserialize::HybridEncoded;
...
fn bitmap_complete() {
    ...
    let a = iter.by_ref().collect::<Result<Vec<_>, _>>().unwrap();
    assert_eq!(iter.len(), 0);
    assert_eq!(a, vec![FilteredHybridEncoded::Bitmap {
        values: &[0b01000011],
        offset: 0,
        length: 8,
    }]);
...
fn repeated_complete() {
    ...
    let a = iter.by_ref().collect::<Result<Vec<_>, _>>().unwrap();
    assert_eq!(iter.len(), 0);
    assert_eq!(a, vec![FilteredHybridEncoded::Repeated {
        is_set: true,
        length: 8,
    }]);
}
