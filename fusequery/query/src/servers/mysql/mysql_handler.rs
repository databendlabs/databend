...
impl<W: io::Write> MysqlShim<W> for Session {
    ...
    fn on_query(&mut self, query: &str, writer: QueryResultWriter<W>) -> std::io::Result<()> {
        ...
        fn build_runtime(max_threads: Result<u64>) -> Result<Runtime> {
            max_threads
                .map_err(|_| {
                    ErrorCodes::UnknownSetting(String::from("Missing max_thread settings."))
                })
                .and_then(|v| {
        ...
        fn data_puller(
            interpreter: &'_ Arc<dyn IInterpreter>,
            _statistics: Result<Statistics>
        ) -> DataPuller<'_> {
            return interpreter
                .execute()
                .then(|r_stream| match r_stream {
        ...
        let output = PlanParser::create(self.ctx.clone())
            ...
            .and_then(|interpreter| {
                build_runtime(self.ctx.get_max_threads()).map(|runtime| (runtime, interpreter))
            })
            .and_then(|(runtime, interpreter)| {
                runtime.block_on(data_puller(&interpreter, self.ctx.try_get_statistics()))
            })
            .map(Some)
            .transpose()
            .map(done(writer))
