            ...
            "PrepareQueryStage" => {
                match std::str::from_utf8(&action.body) {
                    ...
                    Ok(prepare_stage_info_str) => {
                        ...
                        let (response_sender, mut receiver) = channel(1);
                        self.dispatcher_sender.send(DispatcherRequest::PrepareQueryStage(PrepareStageInfo::create(action.query_id, action.stage_id, action.plan, action.scatters, action.scatters_plan), response_sender)).await;
