...
async fn test_transform_having() -> anyhow::Result<()> {
    use std::sync::Arc;
    ...
    if let PlanNode::Having(plan) = PlanBuilder::create(test_source.number_schema_for_test()?)
        .having(col("number").eq(lit(2022)))?
        .build()?
    {
