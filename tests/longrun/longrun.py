...
def run_script(args):
    ...
    while run_times < max_iter_limit:
         with open(output_file, "a") as f:
            ...
            p.communicate()
            print("job {}, run {}. duration: {}".format(script, run_times, time.time() - p_start))
            run_times += 1
            env["SEED"] = str(run_times)
    print("finish job {}, run {}, state file: {}".format(script, run_times, output_file))
    if max_iter_limit > 0:
...
def upload_log_file(file):
    ...
    while attempt <= max_attempts:
        try:
            p = subprocess.Popen(['bash', '-c', 'bendsql --dsn "{}" -q "insert into {} values" -d @{}'.format(warehouse_dsn, log_table, file)])
            p.wait()
...
if __name__ == "__main__":
    ...
    parser.add_argument("-d", "--dir", help="Directory path containing scripts", required=True)
    parser.add_argument("-i", "--iter", type=int, help="Maximum number time a job needs to run", required=True)
    parser.add_argument("--logger", help="event logging table", required=False, default="events")
    args = parser.parse_args()
    warehouse_dsn = os.getenv('WAREHOUSE_DSN', 'databend://root:@localhost:8000/?sslmode=disable')
    ...
    before_scripts = sorted([script for script in file_list if script.endswith(".sh") if script.startswith("_before")])
    ...
    scripts_list = [script for script in file_list if script.endswith(".sh") if script.startswith("_") is False]
    if len(scripts_list) > 0:
        ...
        with Pool(processes=len(scripts_list)) as pool:
            pool.map(run_script, [(os.path.join(directory_path, script), maximum_time_limit) for script in scripts_list])
    
    after_scripts = sorted([script for script in file_list if script.endswith(".sh") if script.startswith("_after")])
