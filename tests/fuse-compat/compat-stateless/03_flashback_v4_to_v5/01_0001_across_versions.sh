...
echo "checking that 2 snapshots of version 4 exist"
...
echo "select snapshot_location='${FST_SNAPSHOT_S2_ID_LOC}', format_version=4 from fuse_snapshot('default', 'fuse_test_compaction') where snapshot_id='$FST_SNAPSHOT_S2_ID'" | $BENDSQL_CLIENT_CONNECT
...
echo "select snapshot_location='${FST_SNAPSHOT_S2_ID_LOC}', format_version=4 from fuse_snapshot('default', 'fuse_test_compaction') limit 1" | $BENDSQL_CLIENT_CONNECT
...
echo "check segments after compaction, there should be 2 segments, a version v4 and a version v5"
