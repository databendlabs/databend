...
echo "select snapshot_location='${FST_SNAPSHOT_S2_ID_LOC}', format_version=4 from fuse_snapshot('default', 'fuse_test_compaction') where snapshot_id='$FST_SNAPSHOT_S2_ID'" | $MYSQL_CLIENT_CONNECT
...
echo "select snapshot_location='${FST_SNAPSHOT_S2_ID_LOC}', format_version=4 from fuse_snapshot('default', 'fuse_test_compaction') limit 1" | $MYSQL_CLIENT_CONNECT
