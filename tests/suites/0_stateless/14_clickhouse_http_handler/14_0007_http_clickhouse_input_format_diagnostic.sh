...
cat << EOF > /tmp/databend_test_csv_success.txt
insert into a(a,b,c) format CSV 
'2023-04-08 01:01:01', "Hello", 12345678
'1989-02-03 15:23:23', "World", 123456
...
cat << EOF > /tmp/databend_test_tsv_success.txt
...
curl -s -u 'root:' -XPOST "http://localhost:${QUERY_CLICKHOUSE_HTTP_HANDLER_PORT}/?enable_planner_v2=1" --data-binary @/tmp/databend_test_csv_success.txt
curl -s -u 'root:' -XPOST "http://localhost:${QUERY_CLICKHOUSE_HTTP_HANDLER_PORT}/?enable_planner_v2=1" -d "SELECT * from a"
echo ''
...
curl -s -u 'root:' -XPOST "http://localhost:${QUERY_CLICKHOUSE_HTTP_HANDLER_PORT}/?enable_planner_v2=1" --data-binary @/tmp/databend_test_tsv_success.txt
curl -s -u 'root:' -XPOST "http://localhost:${QUERY_CLICKHOUSE_HTTP_HANDLER_PORT}/?enable_planner_v2=1" -d "SELECT * from a"
echo ''
