...
echo "select count(*)=3  from fuse_snapshot('default', 't17_001')" | $MYSQL_CLIENT_CONNECT
...
echo "select count(*)=4  from t17_001" | $MYSQL_CLIENT_CONNECT

## Drop table.
echo "drop table t17_001 all" | $MYSQL_CLIENT_CONNECT
...
## verify
echo "optimize table t17_001 purge before (TIMESTAMP => '$TIMEPOINT'::TIMESTAMP)" | $MYSQL_CLIENT_CONNECT
