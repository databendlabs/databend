...
SNAPSHOT_ID=$(echo "select snapshot_id from fuse_snapshot('default','t17_0002') where row_count=3" | $BENDSQL_CLIENT_CONNECT)
...
TIMEPOINT=$(echo "select timestamp from fuse_snapshot('default', 't17_0002') where row_count=5" | $BENDSQL_CLIENT_CONNECT)
