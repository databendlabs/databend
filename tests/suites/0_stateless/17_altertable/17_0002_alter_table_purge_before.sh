...
echo "select count(*)=3  from fuse_snapshot('default', 't17_0002')" | $MYSQL_CLIENT_CONNECT

## location the id of 2nd snapshot
SNAPSHOT_ID=$(echo "select snapshot_id from fuse_snapshot('default','t17_0002') where row_count=3" | $MYSQL_CLIENT_CONNECT)
#TIMEPOINT=$(echo "select timestamp from fuse_snapshot('default', 't17_0002') where row_count=3" | $MYSQL_CLIENT_CONNECT)
...
echo "select count(*)=4  from fuse_snapshot('default', 't17_0002')" | $MYSQL_CLIENT_CONNECT

## location the timestamp of latest snapshot
TIMEPOINT=$(echo "select timestamp from fuse_snapshot('default', 't17_0002') where row_count=5" | $MYSQL_CLIENT_CONNECT)
...
# alter table drop a column
echo "alter table drop a column"
echo "alter table t17_0002 drop column a" | $MYSQL_CLIENT_CONNECT
