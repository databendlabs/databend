...
comment "EXPLAIN should show limit pushed down to TableScan (not NONE) when there is a WHERE clause"
echo "EXPLAIN SELECT * FROM rap_pushdown_test WHERE id > 0 LIMIT 1;" | $BENDSQL_CLIENT_CONNECT | grep "push downs:" | grep -v "limit: NONE"
...
comment "EXPLAIN should show both filters and limit pushed down to TableScan"
echo "EXPLAIN SELECT * FROM rap_pushdown_test WHERE id > 0 ORDER BY id LIMIT 1;" | $BENDSQL_CLIENT_CONNECT | grep "push downs:" | grep -v "filters: \\[\\]" | grep -v "limit: NONE"
