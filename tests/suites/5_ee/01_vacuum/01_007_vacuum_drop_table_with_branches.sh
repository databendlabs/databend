...
SNAPSHOT_LOCATION=$(bendsql_connect_root \
	--output tsv \
	--query="select snapshot_location from fuse_snapshot('test_vacuum_drop_table_with_branches','a') limit 1")
...
find /tmp/test_vacuum_drop_table_with_branches/"$PREFIX"/ -type f -print -quit 2>/dev/null | grep -q . && echo present || echo missing
