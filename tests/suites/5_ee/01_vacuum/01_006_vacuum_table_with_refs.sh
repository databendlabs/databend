...
EXPIRED_BRANCH_SNAPSHOT=$(bendsql_connect_root \
	--set enable_experimental_table_ref=1 \
	--output tsv \
	--query="select snapshot_location from fuse_snapshot('test_vacuum_orphan_refs','a/b_exp') where row_count = 5 order by timestamp desc limit 1")
...
stmt_fail "set enable_experimental_table_ref=1; select c from test_vacuum_orphan_refs.a/b_exp order by c"
