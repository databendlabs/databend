...
echo "ALTER VIRTUAL COLUMN (val['k1']) FOR test_attach_only.test_json_read_only" | $MYSQL_CLIENT_CONNECT

echo "drop virtual column should fail"
echo "DROP VIRTUAL COLUMN FOR test_attach_only.test_json_read_only" | $MYSQL_CLIENT_CONNECT

echo "refresh virtual column should fail"
echo "REFRESH VIRTUAL COLUMN FOR test_attach_only.test_json_read_only" | $MYSQL_CLIENT_CONNECT

# vacuum
echo "vacuum table"

echo "vacuum table should fail"
echo "VACUUM TABLE test_attach_only.test_json_read_only RETAIN 0 HOURS;" | $MYSQL_CLIENT_CONNECT
