...
detach_policies() {
    echo "ALTER TABLE ${DB}.t_mask MODIFY COLUMN c UNSET MASKING POLICY" | $BENDSQL_CLIENT_CONNECT > /dev/null 2>&1 || true
    echo "ALTER TABLE ${DB}.t_row DROP ROW ACCESS POLICY ${ROW_POLICY}" | $BENDSQL_CLIENT_CONNECT > /dev/null 2>&1 || true
}
...
comment "time travel and flashback should handle row access policy metadata"
stmt "CREATE OR REPLACE TABLE ${DB}.t_row(a INT, b STRING)"
insert_stmt "INSERT INTO ${DB}.t_row VALUES (1, 'alice'), (2, 'bob')"
ROW_SNAPSHOT_ID=$(snapshot_id "${DB}" "t_row" 2)
stmt "ALTER TABLE ${DB}.t_row ADD COLUMN c INT"
insert_stmt "INSERT INTO ${DB}.t_row VALUES (3, 'charlie', 100), (4, 'dave', 200)"
...
comment "flashback row policy table to saved snapshot"
echo "ALTER TABLE ${DB}.t_row FLASHBACK TO (SNAPSHOT => '$ROW_SNAPSHOT_ID')" | $BENDSQL_CLIENT_CONNECT > /dev/null || exit 1
query "SELECT count(*)=2 FROM ${DB}.t_row"

comment "flashback should preserve masking behavior for re-added columns"
stmt "CREATE OR REPLACE TABLE ${DB}.t_mask(a INT, b STRING)"
insert_stmt "INSERT INTO ${DB}.t_mask VALUES (1, 'hello')"
...
comment "flashback masking table to saved snapshot"
echo "ALTER TABLE ${DB}.t_mask FLASHBACK TO (SNAPSHOT => '$MASK_SNAPSHOT_ID')" | $BENDSQL_CLIENT_CONNECT > /dev/null || exit 1
stmt "ALTER TABLE ${DB}.t_mask ADD COLUMN c STRING"
insert_stmt "INSERT INTO ${DB}.t_mask VALUES (2, 'world', 'visible')"
query "SELECT c FROM ${DB}.t_mask WHERE a = 2"

stmt "CREATE TABLE ${DB}.t_idx(a INT, c INT) STORAGE_FORMAT = 'parquet'"
insert_stmt "INSERT INTO ${DB}.t_idx VALUES (1, 1), (2, 2)"
IDX_SNAPSHOT_ID=$(snapshot_id "${DB}" "t_idx" 2)
stmt "ALTER TABLE ${DB}.t_idx ADD COLUMN b INT"
insert_stmt "INSERT INTO ${DB}.t_idx VALUES (1, 1, 4), (1, 2, 1), (1, 2, 4), (2, 2, 5)"
...
detach_policies
echo "DROP MASKING POLICY IF EXISTS ${MASK_POLICY}" | $BENDSQL_CLIENT_CONNECT > /dev/null 2>&1 || true
echo "DROP ROW ACCESS POLICY IF EXISTS ${ROW_POLICY}" | $BENDSQL_CLIENT_CONNECT > /dev/null 2>&1 || true
stmt "DROP TABLE IF EXISTS ${DB}.t_idx"
stmt "DROP TABLE IF EXISTS ${DB}.t_mask"
stmt "DROP TABLE IF EXISTS ${DB}.t_row"
