...
echo "drop database if exists mydb" | $MYSQL_CLIENT_CONSUMER_CONNECT
...
echo "create consumer db from share"
...
echo "create database mydb from share provider.test_share using ed;" | $MYSQL_CLIENT_CONSUMER_CONNECT
echo "select * from mydb.t order by a;" | $MYSQL_CLIENT_CONSUMER_CONNECT
...
echo "select * from mydb.t order by b;" | $MYSQL_CLIENT_CONSUMER_CONNECT
