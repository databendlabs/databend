...
function run() {
  echo "--$2"
  curl -sS -H "x-databend-query-id:$2" -H "sql:insert into ontime_streaming_load values file_format = ($1)" -F "upload=@/${TESTS_DATA_DIR}/$2" -u root: -XPUT "http://localhost:${QUERY_HTTP_HANDLER_PORT}/v1/streaming_load"
  echo
