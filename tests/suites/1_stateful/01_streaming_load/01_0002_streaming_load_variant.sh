...
function run() {
  curl -sS -H "x-databend-query-id:$2" -H "sql:insert into $3 values file_format = ($1)" -F "upload=@/${TESTS_DATA_DIR}/$2" -u root: -XPUT "http://localhost:${QUERY_HTTP_HANDLER_PORT}/v1/streaming_load"
  echo
}
