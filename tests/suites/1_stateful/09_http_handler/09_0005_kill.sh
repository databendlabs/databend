...
echo "## kill"
curl -s -u root: -XGET -w "%{http_code}\n"  "http://localhost:8000/v1/query/${QID}/${NODE}/kill"
echo "## page"
curl -s -u root: -XGET -w "\n%{http_code}\n" "http://localhost:8000/v1/query/${QID}/${NODE}/page/0" | sed "s/${QID}/QID/g"
echo "## final"
curl -s -u root: -XGET -w "\n" "http://localhost:8000/v1/query/${QID}/${NODE}/final" | jq ".error"
