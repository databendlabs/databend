...
curl -s -u root: -XPOST "http://localhost:8000/v1/query" --header 'Content-Type: application/json' -d '{"sql": "set enable_distributed_copy_into = 0;copy into products (id, name, description) from @s1/", "pagination": { "wait_time_secs": 6}}' -u root: | jq ".error"
