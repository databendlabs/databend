...
curl -sH "insert_sql:insert into a file_format = (type = 'CSV' skip_header = 0)" -u root:  -F "upload=@/tmp/databend_test_csv1.txt"  -F "upload=@/tmp/databend_test_csv2.txt" -XPUT "http://localhost:${QUERY_HTTP_HANDLER_PORT}/v1/streaming_load" | grep -c "Date type"
