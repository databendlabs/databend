...
curl -s -u root: -XPOST "http://localhost:${QUERY_HTTP_HANDLER_PORT}/v1/query" --header 'Content-Type: application/json' --header 'X-DATABEND-DEDUPLICATE_LABEL: insert1' -d '{"sql": "insert into sample (Id, City, Score) values", "stage_attachment": {"location": "@s1/sample.csv"}}' | jq -r '.stats.scan_progress.bytes, .stats.write_progress.bytes, .error'
