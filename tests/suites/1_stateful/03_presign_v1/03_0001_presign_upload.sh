...
curl -s -w "%{http_code}\n" -X PUT -o /dev/null "`echo "PRESIGN UPLOAD @presign_stage/hello_world.txt" | $MYSQL_CLIENT_CONNECT | cut -f 3`" -d "Hello, World!"

# LIST will output file's updated time, so we only take first three of output:
# file_name, file_size, file_md5
echo "LIST @presign_stage/" | $MYSQL_CLIENT_CONNECT | awk '{print $1,$2,$3}';
