...
WRONG_CSV="set max_threads=1;set enable_distributed_copy_into = 1; COPY INTO wrong_csv FROM 'fs://${DATADIR}/' pattern='wrong_sample.*.csv' FILE_FORMAT = (type = CSV field_delimiter = ','  record_delimiter = '\n' skip_header = 0) ON_ERROR=continue"
...
WRONG_NDJSON="set enable_distributed_copy_into = 1;set max_threads=1; COPY INTO wrong_ndjson FROM 'fs://${DATADIR}/' pattern='wrong_sample.*.ndjson'  FILE_FORMAT = (type = ndjson) ON_ERROR=continue"
...
WRONG_TSV="set max_threads=1;set enable_distributed_copy_into = 1; COPY INTO wrong_tsv FROM 'fs://${DATADIR}/' pattern='wrong_sample.*.tsv' FILE_FORMAT = (type = TSV) ON_ERROR=continue"
...
WRONG_XML="set enable_distributed_copy_into = 1;COPY INTO wrong_xml FROM 'fs://${DATADIR}/'  pattern='wrong_sample.*.xml' FILE_FORMAT = (type = xml) ON_ERROR=continue;"
...
WRONG_CSV="set enable_distributed_copy_into = 1;COPY INTO wrong_csv FROM 'fs://${DATADIR}/wrong_sample.csv' FILE_FORMAT = (type = CSV field_delimiter = ','  record_delimiter = '\n' skip_header = 0) ON_ERROR=abort_5"
