...
echo "copy into @s4 from (select number a, number::string b, number::decimal(15,2) c from numbers(500000000)) file_format=(type=parquet) single=true;" | $BENDSQL_CLIENT_CONNECT | cut -d$'\t' -f1,2
