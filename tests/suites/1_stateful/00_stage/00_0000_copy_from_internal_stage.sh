...
echo "drop table if exists iti" | $MYSQL_CLIENT_CONNECT
echo "create table iti (a int, b string, c int)" | $MYSQL_CLIENT_CONNECT
...
do
	echo "---- testing @$STAGE;"
	echo "remove @$STAGE;" | $MYSQL_CLIENT_CONNECT
	echo "truncate table iti;" | $MYSQL_CLIENT_CONNECT
	...
	echo "list @$STAGE" | $MYSQL_CLIENT_CONNECT | awk '{print $1}' | sort
	echo "-- testing copy;"
  echo "copy into iti from @$STAGE FILE_FORMAT = (type = CSV skip_header = 1) force=true;" | $MYSQL_CLIENT_CONNECT
  echo "select count(*) from iti;" | $MYSQL_CLIENT_CONNECT

	echo "-- testing copy with purge;"
	echo "truncate table iti;" | $MYSQL_CLIENT_CONNECT
  echo "copy into iti from @$STAGE FILE_FORMAT = (type = CSV skip_header = 1) purge=true;" | $MYSQL_CLIENT_CONNECT
  echo "select count(*) from iti;" | $MYSQL_CLIENT_CONNECT
done
