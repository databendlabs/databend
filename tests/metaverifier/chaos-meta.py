...
class IoDelayParams(ChaosParams):
    def __init__(self):
        ...
        self.template = open(
            "./tests/metaverifier/templates/node-disk-io-delay-template.yaml", "r"
        ).read()
    ...
    def to_string(self):
        return (
            "type: IoDelay, params delay: "
            + str(self.params["delay"])
            + ", percent:"
            + str(self.params["percent"])
        )
...
class MetaChaos:
    ...
    def get_node(self, get_leader):
        ...
        if len(nodes) == 0:
            ...
            logging.debug(
                "cannot find nodes, get_leader: " + str(get_leader) + " " + str(nodes)
            )
            return nodes
        return nodes
    ...
    def run(self, apply_second, recover_second):
        ...
        while True:
            ...
            logging.debug(
                "loop " + str(count) + " inject chaos rule to node: " + str(node)
            )
            ...
            if self.is_verifier_end():
                logging.debug(
                    "databend-metaverifier has completed, cost:" + str(diff) + "s, exit"
                )
                sys.exit(0)
