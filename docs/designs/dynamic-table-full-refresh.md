...
A full rebuild can be expensive for large source tables. Incremental refresh is intentionally not
implemented in this phase: arbitrary joins, aggregates, deletes, and source snapshot consistency
need a dependency-aware change protocol rather than simply appending source rows. Scheduling and
incremental refresh should be designed together around the persisted source IDs and a pinned
source endpoint.
...
- `source_table_ids`: a sorted, comma-separated list of the source table IDs captured at creation.
  Refresh rebinds the defining query and rejects the refresh if the resolved IDs differ, preventing
  a dropped source followed by same-name table recreation from silently producing incorrect data.
