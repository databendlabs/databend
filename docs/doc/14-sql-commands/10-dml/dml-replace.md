...
```shell
curl -X PUT -T sample_3_replace.csv \
     "https://s3.us-east-2.amazonaws.com/query-storage-53b9412/tn3ftqihs/stage/internal/s1/sample_3_replace.csv?X-Amz-Algorithm=AWS4-HMAC-SHA256&\
     X-Amz-Credential=ASIAT2EUTJTKFPM7OYFZ%2F20230913%2Fus-east-2%2Fs3%2Faws4_request&X-Amz-Date=20230913T023209Z&X-Amz-Expires=3600&\
     X-Amz-Security-Token=IQoJb3JpZ2luX2VjEDsaCXVzL...
```
...
```sql
REPLACE INTO sample (Id, City, Score) ON(Id)
        SELECT $1, $2, $3 FROM @s1 (FILE_FORMAT=>'csv');
