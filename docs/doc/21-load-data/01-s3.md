...
Databend [COPY](../30-reference/30-sql/10-dml/dml-copy-into-table.md) command can read object files(CSV, JSON or Parquet file format) from Amazon S3 buckets.

### Before You Begin

* **AWS Account:**  This Quickstart uses Amazon S3 and requires an AWS account’s *`access key id`* and *`secret access key`*.
* **Databend :** Make sure Databend is running and accessible, please see [Deploy Databend](../10-deploy/00-understanding-deployment-modes.md).
